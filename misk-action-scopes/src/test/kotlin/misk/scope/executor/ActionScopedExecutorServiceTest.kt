...
internal class ActionScopedExecutorServiceTest {
  ...
  fun propagatesScopeIfInScope() {
    val injector = Guice.createInjector(TestActionScopedProviderModule(), ActionScopedExecutorServiceModule())
    ...
    val future = scope.enter(seedData).use { executor.submit(Callable { tester.fooValue() }) }
