...
internal constructor(
  ...
  @Deprecated(message = "don't use runBlocking explicitly", replaceWith = ReplaceWith("use suspending invocation"))
  ...
  /**
   * Converts the action scope into a [CoroutineContext.Element] to maintain the given ActionScope context for
   * coroutines regardless of the actual thread they run on.
   *
   * Example usage:
   * ```
  ...
  fun create(
    ...
    check(!inScope()) { "cannot create an ActionScope.Instance on a thread that is already running in an action scope" }
  ...
  internal fun enter(instance: Instance): ActionScope {
    check(!inScope()) { "cannot begin an ActionScope on a thread that is already running in an action scope" }
  ...
  fun <T> propagate(c: Callable<T>): Callable<T> {
    ...
    return Callable {
      ...
      if (inScope() && currentThreadUUID == threadLocalUUID.get()) {
        ...
        currentInstance.inScope { c.call() }
      }
    }
  }
  ...
  /**
   * Wraps a function or lambda that will be called on another thread, propagating the current scoped data onto that
   * thread
   */
  ...
  private fun providerFor(key: Key<*>): ActionScopedProvider<*> {
    return requireNotNull(providers[key]?.get()) { "no ActionScopedProvider available for $key" }
  }

  class Instance internal constructor(private val lazyValues: Map<Key<*>, Lazy<*>>, private val scope: ActionScope) :
    ...
    fun <T> inScope(block: () -> T): T {
      return scope.enter(this).use { block() }
    }
  ...
  private class WrappedKFunction<T>(
    ...
    override fun call(vararg args: Any?): T {
      ...
      return if (scope.inScope() && threadUUID == threadLocalUUID.get()) {
        ...
        instance.inScope { wrapped.call(*args) }
      }
    }
    ...
    override fun callBy(args: Map<KParameter, Any?>): T {
      ...
      return if (scope.inScope() && threadUUID == threadLocalUUID.get()) {
        ...
        instance.inScope { wrapped.callBy(args) }
      }
    }
