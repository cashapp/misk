...
internal class AsyncCloseableExtensionsTest {
  ...
  class TestCloseable : AsyncCloseable {
    ...
    fun isOpenAsyncFailed(): CompletableFuture<Boolean> = CompletableFuture.failedFuture(TestCloseableException(isOpen))

    override fun closeAsync(): CompletableFuture<Void> =
      CompletableFuture.completedFuture<Void>(null).whenComplete { _, _ -> isOpen = false }
  }

  class TestCloseableException(val isOpen: Boolean) : Exception("test failure")

  private fun newCloseable(): CompletableFuture<TestCloseable> = CompletableFuture.supplyAsync { TestCloseable() }
  ...
  fun `test 'closeOnCompletion' on successfully completed future`() {
    ...
    assertTrue(actual = isOpen, message = "AsyncCloseable is open during use")

    assertTrue(actual = closeable!!.isClosed, message = "AsyncCloseable is closed after use")
  }
  ...
  fun `test 'closeOnCompletion' on exceptionally completed future`() {
    ...
    assertTrue(actual = exception.isOpen, message = "AsyncCloseable is open during use")
  ...
  fun `test 'thenApplyUsing' on successfully completed future`() {
    ...
    val isOpen =
      newCloseable().whenComplete { t, _ -> closeable = t }.thenApplyUsing { it.isOpen }.toCompletableFuture().get()
  ...
  fun `test 'suspendingUse' on successfully failed operation`() = runTest {
    val closeable = TestCloseable()
    val exception =
      assertFailsWith<TestCloseableException> {
  ...
  fun `test 'closeFinallyAsync', with a cause,  when closeAsync throws an exception`() {
    ...
    asyncCloseable.closeFinallyAsync(cause).toCompletableFuture().join()
  ...
  fun `test 'closeFinallyAsync', without a cause,  when closeAsync throws an exception`() {
    ...
    try {
      asyncCloseable.closeFinallyAsync().toCompletableFuture().join()
    } catch (e: Exception) {
  ...
  fun `test 'closeFinallyAsync', without a cause,  when closeAsync succeeds`() {
    ...
    assertTrue(actual = asyncCloseable.isClosed, message = "AsyncCloseable is closed after closeFinallyAsync")
  }
}
