...
internal class ConnectionPoolMetricsTest {
  ...
  fun setUp() {
    ...
    connectionProvider = PooledStatefulRedisConnectionProvider(
  ...
  fun `test connection pool metrics in RedisClientMetrics`() {
    connectionProvider.acquireBlocking(exclusive = true).use {
      ...
      assertEquals(
        ...
        1.0, clientMetrics.activeConnectionsGauge.labels(name, replicationGroupId).get()
      )
      assertEquals(
        "idle connections is 0 after acquiring a connection",
        0.0, clientMetrics.idleConnectionsGauge.labels(name, replicationGroupId).get()
      )
    }
    assertEquals(
    ...
    assertEquals(
      ...
      1.0, clientMetrics.idleConnectionsGauge.labels(name, replicationGroupId).get()
    )
  }
}
