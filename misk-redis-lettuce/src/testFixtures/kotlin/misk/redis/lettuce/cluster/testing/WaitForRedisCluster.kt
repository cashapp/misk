 ...
 *
 * The function polls the cluster status at regular intervals until all conditions are met or the timeout is reached.
 *
        ...
        .thenApply { result ->
          check(
            result
              ...
              .all { clusterNode -> clusterNode.isConnected && !clusterNode.isFailing && clusterNode.hasSlots }
          )
        }
        .thenCompose {
          // Check and return the assigned slots
          async().clusterInfo().toClusterInfo().thenApply { clusterInfo: ClusterInfo ->
