 ...
 *
 * This class manages a containerized Redis Cluster for testing, handling lifecycle operations like startup, shutdown,
 * and test cleanup. It uses Docker to run a multi-node Redis Cluster with a predictable configuration.
 *
 ...
 */
class DockerRedisCluster(private val version: String = "7.0.10") : ExternalDependency {
  ...
  private val redisClient: RedisClusterClient by lazy {
    redisClusterClient(redisURI = redisUri(seedHost, seedPort) { withTimeout(10.seconds.toJavaDuration()) })
  }
  ...
  private val composer =
    Composer(
      CONTAINER_NAME,
      Container {
        val envVars = listOf("IP=0.0.0.0", "INITIAL_PORT=$SEED_PORT", "MASTERS=3", "SLAVES_PER_MASTER=1")
  ...
  override fun startup() {
    ...
    val slots =
      try {
        connection.waitForRedisClusterReady(timeout = 30.seconds, pollInterval = 200.milliseconds).get()
