 ...
 *
 * This provider implements both [ReadWriteStatefulRedisConnectionProvider] and
 * [ReadOnlyStatefulRedisConnectionProvider] interfaces, making it suitable for both read-write and read-only Redis
 * operations. It uses a [BoundedAsyncPool] to efficiently manage and reuse Redis connections.
 *
 * Key features:
...
internal class PooledStatefulRedisConnectionProvider<K : Any, V : Any>(
...
) : ReadWriteStatefulRedisConnectionProvider<K, V>, ReadOnlyStatefulRedisConnectionProvider<K, V> {

  private val sharedConnection: CompletableFuture<StatefulRedisConnection<K, V>> by lazy {
    poolFuture.thenCompose { it.acquire().thenApply { connection -> SharedStatefulRedisConnection(connection) } }
  }
  ...
  override fun acquireAsync(exclusive: Boolean): CompletableFuture<StatefulRedisConnection<K, V>> =
    if (exclusive) {
      ...
      poolFuture.thenCompose { pool ->
        pool.acquire().thenApply { connection -> PooledStatefulRedisConnection(pool, connection) }
      }
    } else {
  ...
  override fun closeAsync(): CompletableFuture<Void> = poolFuture.thenCompose { it.closeAsync() }
}
...
class PooledStatefulRedisConnection<K : Any, V : Any>(
  ...
  override fun closeAsync(): CompletableFuture<Void> = pool.release(connection)
}

class SharedStatefulRedisConnection<K : Any, V : Any>(private val connection: StatefulRedisConnection<K, V>) :
  StatefulRedisConnection<K, V> by connection {
  override fun close() {
    closeAsync().get()
  }

  override fun closeAsync(): CompletableFuture<Void> = CompletableFuture.completedFuture(null) // NOOP
}
