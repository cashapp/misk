 ...
 */
interface StatefulRedisConnectionProvider<K : Any, V : Any> :
  ConnectionProvider<K, V, StatefulRedisConnection<K, V>>
 ...
 * with a read/write connection. It's particularly useful when connecting to a
 * single Redis server.
 */
 ...
 *
 * This specialized provider is used for connections that support both read and write
 * operations. In a replicated environment (like AWS ElastiCache), this typically
 * represents connections to the primary node.

 */
interface ReadWriteStatefulRedisConnectionProvider<K : Any, V : Any> :
  StatefulRedisConnectionProvider<K, V>
 ...
 *
 * This specialized provider is used for connections that only need read access.
 * In a replicated environment (like AWS ElastiCache), this typically represents
 * connections to replica nodes.
 */
interface ReadOnlyStatefulRedisConnectionProvider<K : Any, V : Any> :
  StatefulRedisConnectionProvider<K, V>

/**
 * Type alias for a string-based read-only connection provider.
 * This is a convenience type for the common case of using string keys and values
 * with a Redis  connection.
 */
