 ...
 * [PooledStatefulRedisConnectionProvider]. One [StatefulRedisConnectionProviderModule] should be installed for each
 * desired binding.
 *
...
internal class StatefulRedisConnectionProviderModule<K : Any, V : Any, T : StatefulRedisConnectionProvider<K, V>>(
          ...
          AsyncConnectionPoolSupport.createBoundedObjectPoolAsync(
              object : Supplier<CompletionStage<StatefulRedisConnection<K, V>>> {
                ...
                override fun get(): ConnectionFuture<StatefulRedisConnection<K, V>> {
                  ...
                  return redisClientProvider.get().connectAsync(codec, redisUri).maybeSetClientName(suffix)
                }
              },
  ...
  /**
   * Composer for a [StatefulRedisConnection] [CompletableFuture] that will set the client name once the connection is
   * established and return the original [CompletableFuture]
   */
  @Suppress("UNCHECKED_CAST")
  private fun <K, V> ConnectionFuture<StatefulRedisConnection<K, V>>.maybeSetClientName(
    suffix: String? = null
