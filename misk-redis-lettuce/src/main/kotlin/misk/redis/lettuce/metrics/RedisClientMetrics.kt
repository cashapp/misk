 ...
 *
 * This class provides comprehensive monitoring of Redis client behavior through two main types
 * of metrics:
 *
 ...
 */
@Singleton
internal class RedisClientMetrics @Inject constructor(
  metrics: Metrics
) {
   ...
   *
   * This method sets up gauge metrics to monitor the state of a [BoundedAsyncPool].
   * The metrics are collected on-demand when scraped by the metrics system.
   *
   * Registered metrics:
   ...
   *
   * This metric helps track initial response time for Redis operations, which is useful
   * for monitoring Redis server responsiveness and network latency.
   *
   * The metric is recorded as a histogram with name `redis_client_first_response_time_millis`
   ...
   *
   * This metric tracks the complete duration of Redis operations from initiation
   * to completion, including command execution and response processing time.
   *
   * The metric is recorded as a histogram with name `redis_client_operation_time_millis`
   * and includes the following labels:
   * - `replication_group_id`: Redis replication group identifier
   * - `command`: Redis command type (e.g., GET, SET)
   * - `remote_address`: Redis server address
  ...
  internal val maxTotalConnectionsGauge: ProvidedGauge = metrics.providedGauge(
  ...
  internal val maxIdleConnectionsGauge: ProvidedGauge = metrics.providedGauge(
  ...
  internal val minIdleConnectionsGauge: ProvidedGauge = metrics.providedGauge(
  ...
  internal val activeConnectionsGauge: ProvidedGauge = metrics.providedGauge(
    name = ACTIVE_CONNECTIONS,
    help = "Current number of active connections for the misk-redis2 client connection pool.",
    labelNames = listOf(NAME_LABEL, REPLICATION_GROUP_ID_LABEL),
  )

  internal val idleConnectionsGauge: ProvidedGauge = metrics.providedGauge(
    name = IDLE_CONNECTIONS,
    help = "Current number of idle connections for the misk-redis2 client connection pool.",
    labelNames = listOf(NAME_LABEL, REPLICATION_GROUP_ID_LABEL),
  )

  internal val firstResponseTime: Histogram = metrics.histogram(
  ...
  internal val operationTime: Histogram = metrics.histogram(
    name = OPERATION_TIME,
