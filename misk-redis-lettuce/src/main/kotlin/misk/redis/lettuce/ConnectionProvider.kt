 ...
 *
 * This interface provides type-safe Redis connections with support for custom key and value types. It extends
 * [AsyncCloseable] and should be closed to release resources when no longer needed. In general, misk will bind this
 * interface to an instance of [StatefulRedisConnectionProvider] or [StatefulRedisClusterConnectionProvider] and will
...
interface ConnectionProvider<K : Any, V : Any, T : StatefulConnection<K, V>> : AsyncCloseable {
  ...
  /**
   * Asynchronously acquires a connection from the provider. The returned connection must be closed after use.
   *
  ...
  /**
   * Suspending function to acquire a connection from the provider. The connection must be closed after use.
   *
   * @param exclusive If true, the connection will be acquired exclusively
   * @return A [T] instance
   * @throws NoSuchElementException when no connection is immediately available
  ...
  /**
   * Synchronously acquires a connection from the provider. The connection must be closed after use.
   *
   * @param exclusive If true, the connection will be acquired exclusively
   * @return A [T] instance
   * @throws NoSuchElementException when no connection is immediately available
...
/**
 * Function to generate a [TypeLiteral] for a [ConnectionProvider] with the given key and value types. This is intended
 * to be passed to the [StatefulRedisConnectionProviderModule] or [StatefulRedisClusterConnectionProviderModule] in
 * order specify the type to bind
 */
...
internal inline fun <reified T : ConnectionProvider<*, *, *>> connectionProviderTypeLiteral(
  ...
  TypeLiteral.get(
    object : ParameterizedType {
      ...
      override fun getActualTypeArguments(): Array<Type> = arrayOf(keyType.java, valueType.java)
    }
  ) as TypeLiteral<T>
