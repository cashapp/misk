 ...
 *
 * NOTE: `misk.metrics.v2.Metrics` is NOT backward compatible with `misk.metrics.Metrics`. This is because the metric
 * type of the `histogram(...)` function has changed. If a callsite which used `misk.metrics.Metrics.histogram(...)` is
 * upgraded to use `misk.metrics.v2.Metrics.histogram(...)`, the dashboards and monitors based on the metric will break
 * because the data type of the metric will have changed.
 *
...
interface Metrics {
  ...
  fun counter(name: String, help: String, labelNames: List<String> = listOf()) =
    Counter.build(name, help).labelNames(*labelNames.toTypedArray()).register(getRegistry())
  ...
  fun gauge(name: String, help: String = "", labelNames: List<String> = listOf()) =
    Gauge.build(name, help).labelNames(*labelNames.toTypedArray()).register(getRegistry())

  /**
   * peakGauge creates and registers a new `Gauge` prometheus type that resets to its initial value after each metrics
   * collection.
   *
   * @param name the name of the metric which will be supplied to prometheus. Must be unique across all metric types.
   * @param help human-readable help text that will be supplied to prometheus.
   * @param labelNames the names (a.k.a. keys) of all the labels that will be used for this metric.
   */
  fun peakGauge(name: String, help: String = "", labelNames: List<String> = listOf()) =
    PeakGauge.builder(name, help).labelNames(*labelNames.toTypedArray()).register(getRegistry())

  /**
   * providedGauge creates and registers a new `Gauge` prometheus type that gets its value from a registered value
   * provider
   *
   * @param name the name of the metric which will be supplied to prometheus. Must be unique across all metric types.
   * @param help human-readable help text that will be supplied to prometheus.
   * @param labelNames the names (a.k.a. keys) of all the labels that will be used for this metric.
   */
  fun providedGauge(name: String, help: String = "", labelNames: List<String> = listOf()) =
    ProvidedGauge.builder(name, help).labelNames(*labelNames.toTypedArray()).register(getRegistry())

  /**
   * histogram creates a new `Histogram` prometheus type with the supplied parameters.
   *
   * NOTE: `misk.metrics.v2.Metrics` is NOT backward compatible with `misk.metrics.Metrics`. This is because the metric
   * type of the `histogram(...)` function has changed. If a callsite which used `misk.metrics.Metrics.histogram(...)`
   * is upgraded to use `misk.metrics.v2.Metrics.histogram(...)`, the dashboards and monitors based on the metric will
   * break because the data type of the metric will have changed.
   *
   ...
   *
   * NB: Summaries can be an order of magnitude more expensive than histograms in terms of CPU. Unless you require the
   * specific properties of a summary, consider using [histogram] instead.
   *
   * @param name the name of the metric which will be supplied to prometheus. Must be unique across all metric types.
   * @param help human-readable help text that will be supplied to prometheus.
   * @param labelNames the names (a.k.a. keys) of all the labels that will be used for this metric.
  ...
  fun legacyHistogram(
    ...
    maxAgeSeconds: Long? = null,
  ) = misk.metrics.Histogram.factory(summary(name, help, labelNames, quantiles, maxAgeSeconds))

  companion object {
    fun factory(registry: CollectorRegistry) =
      object : Metrics {
