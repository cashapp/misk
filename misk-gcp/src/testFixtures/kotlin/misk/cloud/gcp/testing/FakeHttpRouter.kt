...
class FakeHttpRouter(val router: (FakeHttpRequest) -> FakeHttpResponse) : HttpTransport() {
  companion object {
    fun respondWithJson(item: Any) = FakeHttpResponse().setStatusCode(200).setJsonContent(item)
    ...
    fun respondWithText(statusCode: Int, text: String) = FakeHttpResponse().setStatusCode(statusCode).setContent(text)
  }
