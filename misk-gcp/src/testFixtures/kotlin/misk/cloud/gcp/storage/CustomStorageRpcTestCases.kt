...
internal abstract class CustomStorageRpcTestCases<T : StorageRpc> {
  ...
  fun createEmpty() {
    ...
    val result = storage.create(BlobInfo.newBuilder(blobId).setContentType("text/plain").build())
  ...
  fun createWithContent() {
    ...
    val blobId = BlobId.of("my_bucket", "my_entry")
    val result =
      storage.create(BlobInfo.newBuilder(blobId).setContentType("text/plain").build(), contents.toByteArray())
  ...
  fun createUpdatesMetagenerationIfMetadataChanged() {
    ...
    val result =
      storage.create(
        BlobInfo.newBuilder(blobId).setContentType("text/plain").setMetadata(mapOf("foo" to "bar")).build(),
        contents.toByteArray(),
      )
    ...
    val result2 =
      storage.create(
        BlobInfo.newBuilder(blobId).setContentType("text/plain").setMetadata(mapOf("foo" to "zed")).build(),
        contents2.toByteArray(),
      )
  ...
  fun progressiveUpload() {
    val blobId = BlobId.of("my_bucket", "my_entry")
    val blob = storage.create(BlobInfo.newBuilder(blobId).setContentType("text/plain").build())
  ...
  fun getIfGenerationMatches() {
    ...
    val blobId = BlobId.of("my_bucket", "my_entry")
    storage.create(BlobInfo.newBuilder(blobId).setContentType("text/plain").build(), contents1.toByteArray())
    ...
    val contents2 = "This is another text"
    storage.create(
      BlobInfo.newBuilder(BlobId.of(blobId.bucket, blobId.name, 1)).setContentType("text/plain").build(),
      contents2.toByteArray(),
    )
    ...
    assertFailsWith<StorageException> { blob1.getContent(BlobSourceOption.generationMatch()) }
    ...
    assertThat(String(blob2.getContent(BlobSourceOption.generationMatch()))).isEqualTo(contents2)
  }
  ...
  fun deleteIfGenerationMatch() {
    ...
    val contents2 = "This is another text"
    val blob2 =
      storage.create(BlobInfo.newBuilder(blob1.blobId).setContentType("text/plain").build(), contents2.toByteArray())
  ...
  fun progressiveDownload() {
    val upload = "This is my text".repeat(1024 * 256)
    val blobId = BlobId.of("my_bucket", "my_entry")
    val blob = storage.create(BlobInfo.newBuilder(blobId).setContentType("text/plain").build(), upload.toByteArray())
  ...
  fun copy() {
    ...
    val sourceBlob =
      storage.create(BlobInfo.newBuilder(sourceBlobId).setContentType("text/plain").build(), data.toByteArray())
  ...
  fun listFolders() {
    storage.create(
      BlobInfo.newBuilder("my_bucket", "notes/storage/merp.txt").setContentType("text/plain").build(),
      "This is a merp".toByteArray(),
    )
    storage.create(
      BlobInfo.newBuilder("my_bucket", "notes/storage/traif.txt").setContentType("text/plain").build(),
      "This is traif".toByteArray(),
    )
    storage.create(
      BlobInfo.newBuilder("my_bucket", "notes/misc/blah.txt").setContentType("text/plain").build(),
      "This is a blah".toByteArray(),
    )
    storage.create(
      BlobInfo.newBuilder("my_bucket", "notes/misc/boop.txt").setContentType("text/plain").build(),
      "This is a boop".toByteArray(),
    )
    storage.create(
      BlobInfo.newBuilder("my_bucket", "notes/top.txt").setContentType("text/plain").build(),
      "This is at the top level of notes".toByteArray(),
    )
    storage.create(
      BlobInfo.newBuilder("my_bucket", "notes/top2.txt").setContentType("text/plain").build(),
      "This is also at the top level of notes".toByteArray(),
    )
    storage.create(
      BlobInfo.newBuilder("my_bucket", "images/storage.txt").setContentType("text/plain").build(),
      "This is about storage".toByteArray(),
    )
    storage.create(
      BlobInfo.newBuilder("my_bucket", "images.txt").setContentType("text/plain").build(),
      "This documents images".toByteArray(),
    )
    storage.create(
      BlobInfo.newBuilder("other_bucket", "protos.txt").setContentType("text/plain").build(),
      "These are protos in another bucket".toByteArray(),
    )

    val all = storage.list("my_bucket")
    assertThat(all.blobIds)
    ...
    assertThat(subDirRecursive.blobIds)
      .containsExactlyInAnyOrder(
        BlobId.of("my_bucket", "notes/storage/merp.txt", 1),
        BlobId.of("my_bucket", "notes/storage/traif.txt", 1),
        BlobId.of("my_bucket", "notes/misc/blah.txt", 1),
    ...
    val partialNameMatch = storage.list("my_bucket", prefix("notes/s"), currentDirectory())
    assertThat(partialNameMatch.blobIds).containsExactlyInAnyOrder(BlobId.of("my_bucket", "notes/storage"))

    val recursivePartialNameMatch = storage.list("my_bucket", prefix("notes/s"))
    assertThat(recursivePartialNameMatch.blobIds)
      .containsExactlyInAnyOrder(
        BlobId.of("my_bucket", "notes/storage/merp.txt", 1),
        BlobId.of("my_bucket", "notes/storage/traif.txt", 1),
      )
