...
class GoogleSpannerEmulatorTest {
  ...
  inner class `#reset` {
    ...
    fun `clears tables on each test when using reusable test module`() {
      ...
      val adminClient =
        spannerClient.instanceAdminClient.getInstance(spannerConfig.instance_id).getDatabase(spannerConfig.database)
      val dataClient =
        spannerClient.getDatabaseClient(
          DatabaseId.of(spannerConfig.project_id, spannerConfig.instance_id, spannerConfig.database)
        )
      val personId = "abc123"
      fun personExists(): Boolean {
        val query =
          dataClient.singleUseReadOnlyTransaction().read("people", KeySet.singleKey(Key.of(personId)), listOf("id"))
      ...
      // Insert a person
      dataClient.write(listOf(Mutation.newInsertBuilder("people").set("id").to(personId).build()))
