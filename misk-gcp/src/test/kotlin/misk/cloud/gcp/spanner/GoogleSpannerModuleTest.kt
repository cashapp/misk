...
class GoogleSpannerModuleTest {
  val spannerConfig =
    SpannerConfig(
      ...
      emulator = SpannerEmulatorConfig(enabled = true, hostname = ContainerUtil.dockerTargetOrLocalHost()),
    )
  ...
  fun `it creates a Spanner client and emulator to develop with`() {
    assertTrue(
      spanner.instanceAdminClient.getInstance(spannerConfig.instance_id).getDatabase(spannerConfig.database).exists()
    )
  }
