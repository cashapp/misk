...
constructor(
   ...
   *
   * If you have special behavior around managing service credentials in production environments, this is the place to
   * configure that.
   * > Note: We will attempt to automatically grab credentials from the environment where applicable, or ignore setting
   * > credentials for emulator development.
   */
  @Redact val credentials: Credentials? = null,
  ...
  /**
   * The maximum amount of time to wait before retrying. I.e. after this value is reached, the wait time will not
   * increase further by the multiplier.
   */
  val max_retry_delay_s: Long? = null,

  /**
   * The previous wait time is multiplied by this multiplier to come up with the next wait time, until the max is
   * reached.
   */
  val retry_delay_multiplier: Double? = null,

  /**
   * Configure RPC and total timeout settings. Timeout for the first RPC call. Subsequent retries will be based off this
   * value.
   */
...
constructor(
   ...
   *
   * By default, Spanner emulators run a gRPC port on 9010 and a REST / HTTP port on 9020. When spinning up an emulator,
   * we will automatically reserve this port "+ 10" as the REST / HTTP port for Docker to bind to.
   */
  val port: Int = 9010,
