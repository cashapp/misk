...
internal class GcpKeyService @Inject internal constructor(kms: CloudKMS, val config: GcpKmsConfig) : KeyService {
  ...
  override fun encrypt(keyAlias: String, plainText: ByteString): ByteString {
    val keyLocation =
      config.key_locations[keyAlias] ?: throw IllegalArgumentException("no location for keyAlias $keyAlias")
  ...
  override fun decrypt(keyAlias: String, cipherText: ByteString): ByteString {
    val keyLocation = config.key_locations[keyAlias] ?: throw IllegalArgumentException("no location for key $keyAlias")
    val resource = "projects/${config.project_id}/${keyLocation.path}"
