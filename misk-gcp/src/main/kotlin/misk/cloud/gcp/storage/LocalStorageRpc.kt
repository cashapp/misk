...
/**
 * Implementation of [StorageRpc] that is backed by local disk storage. Useful for running in development mode on local
 * machines, since there is no GCS emulator available. Files are stored with the generation version appended as an
 * extension suffix, with a symlink existing for the latest generation. To preserve GCS atomicity semantics, progressive
 * uploads are handled by storing the interim uploaded data in a temporary file based off the latest generation, then
 * doing a rename to the new generation once the upload is complete.
 *
 * The implementation uses file locks to support multiple local processes accessing the same storage directory. Whenever
 * a blob is updated, the local store will acquire an exclusive lock on a corresponding lock file, releasing that lock
 * when the update is complete (or when the process dies). It's slightly more complicated since we need to deal with the
 * possibility that writer stops partway through without the process failing; in those cases we don't want to prevent
 ...
 *     - generate an upload id, save the temp file location + target metadata + constraints in-memory associated with
 *       the upload id
 * - on write
 *     - write to the temp file for that upload id
 * - on finish
 *     - acquire a write lock, read the latest metadata for the blob and re-check constraints to make sure nothing
 *       changed underneath (e.g. a concurrent upload for that blob did not complete)
 *     - copy from the temp file to a new file in the content directory, appending the new generation number. This is
 *       done as an atomic move + overwrite
 *     - write updated metadata to the metadata file. This is done as an atomic move + overwrite. Until this is
 *       complete, the new version of the content is not accessible to readers.
 *     - release the write lock
 *     - remove the content file for the prior generation
 *
 * If a writer fails between open and finish, all that happens is we have abandoned temp files If a writer fails after
 * moving the temp file to the contents directory but before updating the metadata, then we have a bad content file for
 * that generation sitting in the content directory. Since the metadata hasn't been updated with the new generation,
 * this content is not readable, and a subsequent write will overwrite it. If a writer fails after updating the metadata
 * but before removing the prior generation content file, we'll have left an abandoned content file. A garbage
 * collection process can be run to clean these up eventually
 *
 * Read process
 * - acquire a read lock
 ...
 *
 * Clients use etags to detect when a blob is updated while a progressive download is in place; the etag returned from a
 * prior read is sent in subsequent reads. We simply use the generation number as the etag value.
 */
class LocalStorageRpc @JvmOverloads constructor(root: Path, moshi: Moshi = defaultKotlinMoshi) :
  ...
  override fun list(bucket: String, options: Map<StorageRpc.Option, *>): Tuple<String, Iterable<StorageObject>> {
    try {
      ...
      val parentMetadataFolder = metadataRoot.resolve(Paths.get(bucket, *parentFolderElements))
      val bucketRoot = metadataRoot.resolve(Paths.get(bucket))
      val filePrefix = prefixElements.last().trim()

      val (folderPaths, filePaths) =
        if (delimiter != null) {
          // We want to find just the files + subfolders in the current folder
          Files.list(parentMetadataFolder)
            .asSequence()
            .filter { filePrefix.isEmpty() || it.fileName.toString().startsWith(filePrefix) }
            .partition { Files.isDirectory(it) }
        } else {
          // We want to find all of the files beneath this sub-folder
          listOf<Path>() to
            parentMetadataFolder.listRecursively().filter {
              filePrefix.isEmpty() || it.getName(parentMetadataFolder.nameCount).toString().startsWith(filePrefix)
            }
        }

      val folders =
        folderPaths.map { bucketRoot.toBlobId(it) }.map { StorageObject().setBucket(it.bucket).setName(it.name) }
  ...
  override fun load(obj: StorageObject, options: Map<StorageRpc.Option, *>): ByteArray =
    try {
      withReadLock(obj.blobId) {
        val metadata =
          getMetadataForReading(obj.blobId, options) ?: throw StorageException(404, "${obj.blobId.fullName} not found")
  ...
  override fun read(
    ...
    try {
      withReadLock(from.blobId) {
        val metadata =
          getMetadataForReading(from.blobId, options)
  ...
  override fun open(obj: StorageObject, options: Map<StorageRpc.Option, *>): String = beginUpload(obj, options).id
  ...
  override fun openRewrite(request: StorageRpc.RewriteRequest): StorageRpc.RewriteResponse {
    return try {
      ...
      val (lock1, lock2) = arrayOf(source.blobId, target.blobId).sortedWith(Comparator { b1, b2 -> b1.compareTo(b2) })

      withWriteLock(lock1) {
        withWriteLock(lock2) {
          val sourceMetadata =
            getMetadataForReading(source.blobId, request.sourceOptions)
              ?: throw StorageException(404, "${source.blobId.fullName} not found")
          val sourceContentFile = contentRoot.resolve(source.blobId.toPath(sourceMetadata.generation))

          val existingTargetMetadata = getMetadataForWriting(target.blobId, request.targetOptions)
          val newTargetMetadata = request.target.nextGenerationMetadata(existingTargetMetadata)
          val targetContentFile = contentRoot.resolve(target.blobId.toPath(newTargetMetadata.generation))
          ...
          existingTargetMetadata?.let {
            val oldTargetContentPath = contentRoot.resolve(target.blobId.toPath(it.generation))
            try {
          ...
          StorageRpc.RewriteResponse(request, request.target, sourceContentSize, true, "token", sourceContentSize)
        }
      }
    } catch (e: IOException) {
      throw StorageException(e)
  ...
  private fun getContentSize(contentPath: Path) =
    try {
  ...
  private fun getMetadataForWriting(blobId: BlobId, options: Map<StorageRpc.Option, *>): BlobMetadata? {
    val existingMetadata = readMetadata(blobId)
    options.generationMatch?.let {
      when {
        it == 0L && existingMetadata != null -> throw StorageException(401, "${blobId.fullName} already exists")
        it != 0L && existingMetadata == null -> throw StorageException(404, "${blobId.fullName} does not exist")
        it != 0L && existingMetadata != null && it != existingMetadata.generation ->
          throw StorageException(401, "generation mismatch: ${existingMetadata.generation} != $it")
        else -> {}
      }
    }
  ...
  private fun getMetadataForReading(blobId: BlobId, options: Map<StorageRpc.Option, *>): BlobMetadata? {
  ...
  private fun beginUpload(obj: StorageObject, options: Map<StorageRpc.Option, *>): Upload {
    try {
      ...
      internalLock.write { uploads[uploadId] = upload }
  ...
  private fun continueUpload(
    ...
    try {
      ...
      internalLock.read {
        val upload = uploads[uploadId] ?: throw StorageException(404, "no such upload $uploadId")
  ...
  private fun <T> withReadLock(blobId: BlobId, f: () -> T): T = internalLock.read { withFileLock(blobId, true, f) }

  private fun <T> withWriteLock(blobId: BlobId, f: () -> T): T = internalLock.write { withFileLock(blobId, false, f) }
  ...
  private class BlobMetadata(
    ...
    fun toStorageObject(bucket: String, name: String, size: Long = 0) = toStorageObject(BlobId.of(bucket, name), size)
  }

  private class Upload(val id: String, val blobId: BlobId, val tempFile: Path, val targetMetadata: BlobMetadata)
  ...
  private fun StorageObject.nextGenerationMetadata(existing: BlobMetadata?): BlobMetadata {
    ...
    return BlobMetadata(newGeneration, newMetaGeneration, newUserProperties, newContentType, newContentEncoding)
  }
}
...
private val StorageObject.blobId
  get() = BlobId.of(bucket, name, generation)

private fun BlobId.toPath(generation: Long) = Paths.get(bucket, *parentPathElements, fileName(generation))

private fun BlobId.toPath() = Paths.get(bucket, *pathElements)

private val BlobId.parentPathElements
  get() = pathElements.dropLast(1).toTypedArray()
private val BlobId.pathElements
  get() = name.split('/').toTypedArray()

private fun BlobId.fileName(generation: Long = 1) = "${pathElements.last()}.$generation"

private val BlobId.fullName
  get() = "$bucket:$name"

private fun Path.toBlobId(childPath: Path): BlobId =
  BlobId.of(fileName.toString(), relativize(childPath).joinToString("/"))

fun <T> FileChannel.withLock(shared: Boolean, action: () -> T) = lock(0, Long.MAX_VALUE, shared).use { action() }
