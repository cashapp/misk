...
class FakeFeatureFlagsModuleTest {
      ...
      Guice.createInjector(
        ...
        object : KAbstractModule() {
          override fun configure() {
            ...
            install(FakeFeatureFlagsOverrideModule { overrideJson(Feature("jsonFeature"), JsonFeature("testValue")) })
          }
        },
      )
  ...
  fun `withOverrides method and FakeFeatureFlagsOverrideModule both work`() {
    val injector =
      Guice.createInjector(
        FakeFeatureFlagsModule().withOverrides { overrideJson(Feature("jsonFeature"), JsonFeature("testValue")) },
        MoshiTestingModule(),
        object : KAbstractModule() {
          override fun configure() {
            install(FakeFeatureFlagsOverrideModule { override(Feature("foo"), 24) })
...
@Qualifier @Target(AnnotationTarget.FUNCTION, AnnotationTarget.FIELD) annotation class AnotherFeatureFlag
