...
/**
 * Install defaults for [FakeFeatureFlags]. This module can be install many times, allowing for feature flag overrides
 * to be modular and scoped to the module the flag is used in.
 *
 * In any module use:
 * ```
 ...
 * ```
 */
class FakeFeatureFlagsOverrideModule
private constructor(
  ...
  ) : this(qualifier, FakeFeatureFlagsOverride(overrideLambda))

  override fun configure() {
    multibind<FakeFeatureFlagsOverride>(qualifier).toInstance(override)
  }

  class FakeFeatureFlagsOverride(val overrideLambda: FakeFeatureFlags.() -> Unit)
}
