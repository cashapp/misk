...
/**
 * A [JobQueue] enqueues jobs for asynchronous execution, possibly in another process. Jobs are enqueued immediately and
 * may involve an RPC to the underlying job queueing system, so should not be done from within a database transaction.
 * Applications that need to enqueue jobs atomically with a local database transaction should use the
 * [TransactionalJobQueue] interface
 */
interface JobQueue {
  fun enqueue(
    queueName: QueueName,
   ...
   * @param body The body of the job; can be any arbitrary string - it is up to the enqueuer and consumer to agree on
   ...
   *   Used for jobs that should delay processing for a period of time. Note that depending on implementation, there may
   *   be an upper limit to this value. For instance, SQS implementation limits `deliveryDelay` to 900s (15m). If a
   *   longer delay is required by applications, use the [TransactionalJobQueue] interface instead.
  ...
  /**
   * throws [BatchEnqueueException] contains the list of jobs successfully enqueued and the list of jobs that could not
   * be enqueued. See [EnqueueErrorResult] for more details on the error information returned
   */
  fun batchEnqueue(queueName: QueueName, jobs: List<JobRequest>)
   ...
   * @param deliveryDelay If specified, the job will only become visible to the consumer after the provided duration.
   *   Used for jobs that should delay processing for a period of time. Note that depending on implementation, there may
   *   be an upper limit to this value. For instance, SQS implementation limits `deliveryDelay` to 900s (15m).
   * @param attributes Arbitrary contextual attributes associated with the job. Implementations may limit the number of
   *   attributes per message.
   */
  ...
  /**
   * Exception Data class containing the list of jobs successfully enqueued and the list of jobs that could not be
   * enqueued. See [EnqueueErrorResult] for more details on the error information returned
   */
