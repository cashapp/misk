...
abstract class TransacterTest {
  ...
  fun happyPath() {
    ...
    transacter.withLock("actorLock") {
      transacter.transaction { session ->
        val luxoJr = queryFactory.newQuery<CharacterQuery>().allowTableScan().name("Luxo Jr.").uniqueResult(session)!!

        session.delete(luxoJr)

        val afterDelete =
          queryFactory.newQuery<CharacterQuery>().allowTableScan().name("Luxo Jr.").uniqueResult(session)
        assertThat(afterDelete).isNull()
      }
    }
  }
  ...
  fun `saves dates properly`() {
    ...
    val jp = transacter.transaction { session -> session.save(DbMovie("Jurassic Park", releaseDate)) }
    transacter.transaction { session -> assertThat(session.load(jp).release_date).isEqualTo(releaseDate) }
  }
  ...
  fun `can do comparison query on ids`() {
    createTestData()
    transacter.replicaRead { session ->
      val characters = queryFactory.newQuery<CharacterQuery>().allowTableScan().idMoreThan(Id(0)).idAsc().list(session)
  ...
  fun `can run consecutive replica reads and transactions`() {
      ...
      if (transacter.config().type.isVitess) {
        ...
        val target =
          session.useConnection { c ->
            c.createStatement().use { it.executeQuery("SHOW VITESS_TARGET").uniqueString() }
          }
    ...
    transacter.replicaRead { session ->
      assertThat(queryFactory.newQuery<CharacterQuery>().allowTableScan().name("Ian Malcolm 2").uniqueResult(session))
        .isNotNull
    }
  }
  ...
  fun loadOrNullMissingEntityReturnsNull() {
    transacter.transaction { session -> assertThat(session.loadOrNull(Id<DbActor>(123))).isNull() }
  }
  ...
  fun constraintViolationCausesTransactionToRollback() {
    val movieId = transacter.transaction { session -> session.save(DbMovie("Cinderella", LocalDate.of(1950, 3, 4))) }

    val actorId1 = transacter.transaction { session -> session.save(DbActor("Lily James")) }

    val actorId2 = transacter.transaction { session -> session.save(DbActor("Holliday Grainger")) }

    val movie =
      transacter.transaction { session -> queryFactory.newQuery<MovieQuery>().id(movieId).uniqueResult(session)!! }

    val actor1 =
      transacter.transaction { session -> queryFactory.newQuery<ActorQuery>().id(actorId1).uniqueResult(session)!! }

    val actor2 =
      transacter.transaction { session -> queryFactory.newQuery<ActorQuery>().id(actorId2).uniqueResult(session)!! }

    transacter.transaction { session -> session.save(DbCharacter("Cinderella", movie, actor1)) }
    ...
    transacter.transaction {
      ...
      assertFailsWith<IllegalStateException> {
        transacter.failSafeRead {
          queryFactory.newQuery<MovieQuery>().allowTableScan().name("Jurassic Park").uniqueResult(it)
        }
      }
    }
  ...
  fun nestedTransactionUnsupported() {
    val exception = assertFailsWith<IllegalStateException> { transacter.transaction { transacter.transaction {} } }
  ...
  fun nestedTransactionUnsupportedWithDerivativeTransacter() {
    val exception =
      assertFailsWith<IllegalStateException> { transacter.transaction { transacter.noRetries().transaction {} } }
    assertThat(exception).hasMessage("Attempted to start a nested session")
  }
  ...
  fun transactionSucceedsAfterRetry() {
    ...
    transacter.transaction { session ->
      session.save(DbMovie("Star Wars", LocalDate.of(1977, 5, 25)))
      assertThat(queryFactory.newQuery<MovieQuery>().allowTableScan().list(session)).isNotEmpty
  ...
  fun readOnlyWontSave() {
    assertFailsWith<IllegalStateException> {
      transacter.readOnly().transaction { session -> session.save(DbMovie("Star Wars", LocalDate.of(1977, 5, 25))) }
    }
    transacter.transaction { session ->
      assertThat(queryFactory.newQuery<MovieQuery>().allowTableScan().list(session)).isEmpty()
    }
  ...
  fun preCommitHooksCalledPriorToCommit() {
    ...
    transacter.allowCowrites().transaction { session ->
      ...
      session.onPreCommit { preCommitHooksTriggered.add("third") }
    }
  ...
  fun errorInPreCommitHookCausesTransactionRollback() {
    ...
    assertThrows<IllegalStateException> {
      transacter.transaction { session ->
        ...
        session.onPreCommit { preCommitHooksTriggered.add("second") }
    ...
    assertThat(transacter.transaction { queryFactory.newQuery<MovieQuery>().id(swid).list(it) }).isEmpty()
  }
  ...
  fun errorInPostCommitHookDoesNotRollback() {
    ...
    val cause =
      assertThrows<PostCommitHookFailedException> {
          transacter.transaction { session ->
  ...
  fun exceptionRaisedFromSessionCloseHook() {
    val cause =
      assertThrows<IllegalStateException> {
        transacter.transaction { session -> session.onSessionClose { throw IllegalStateException("hook failed") } }
      }
  ...
  fun sessionCloseHookInvokedEvenOnRollback() {
    ...
    assertThrows<NonRetryableException> {
      transacter.transaction { session ->
        session.onSessionClose { logs.add("hook invoked") }
        throw NonRetryableException()
      }
    }
  ...
  fun concurrentUpdates() {
    transacter.transaction { session -> session.save(DbMovie("Star Wars", LocalDate.of(1975, 5, 25))) }
    ...
    val asyncWrite = Callable {
      transacter.transaction { session ->
        ...
        val movie = queryFactory.newQuery(MovieQuery::class).name("Star Wars").uniqueResult(session)!!
        movie.release_date = LocalDate.of(movie.release_date!!.year + 1, 5, 25)
        txnExitLatch.countDown()
      }
    }

    val futureResults = Executors.newFixedThreadPool(2).invokeAll(listOf(asyncWrite, asyncWrite)).map { it.get() }

    assertThat(futureResults).hasSize(2)

    val movies = transacter.transaction { session -> queryFactory.newQuery(MovieQuery::class).list(session) }
...
class CockroachTransacterTest : TransacterTest() {
  @MiskTestModule val module = MoviesTestModule(DataSourceType.COCKROACHDB)
}

@MiskTest(startService = true)
class TidbTransacterTest : TransacterTest() {
  @MiskTestModule val module = MoviesTestModule(DataSourceType.TIDB)
}
