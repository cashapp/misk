...
internal class SchemaValidatorTest {
  ...
  inner class TestModule : KAbstractModule() {
    override fun configure() {
          ...
          object : Provider<Transacter> {
            ...
            override fun get(): RealTransacter =
              RealTransacter(
                ...
                hibernateEntities =
                  injector.findBindingsByType(HibernateEntity::class.typeLiteral()).map { it.provider.get() }.toSet(),
              )
          }
        )
        .asSingleton()
      ...
      install(
        object : HibernateEntityModule(qualifier) {
      ...
      bind(keyOf<SessionFactoryService>(qualifier))
        .toProvider {
  ...
  fun findMissingTablesInHibernate() {
    assertThat(schemaValidationErrorMessage)
      .contains("Hibernate missing tables [database_only_table, unquoted_database_table]")
  }

  @Test
  fun findMissingTablesInDb() {
    assertThat(schemaValidationErrorMessage).contains("Database missing tables [hibernate_only_table]")
  }
  ...
  fun itOkWithNotNullableColumnWithDefaults() {
    assertThat(schemaValidationErrorMessage)
      .doesNotContain("ERROR at schemavalidation.nullable_mismatch_table.tbl5_hibernate_null_default:")
  }

  @Test
  fun catchBadTables() {
    assertThat(schemaValidationErrorMessage).contains("\"BAD_identifier_table\" should be in lower_snake_case")
    assertThat(schemaValidationErrorMessage)
      .contains("\"BAD_identifier_table\" should exactly match hibernate \"bad_identifier_table\"")
  }
  ...
  fun catchBadColumnNames() {
    assertThat(schemaValidationErrorMessage).contains("tbl6CamelCase should be in lower_snake_case")
    assertThat(schemaValidationErrorMessage).contains("tbl6_UPPER_UNDERSCORE should be in lower_snake_case")
    assertThat(schemaValidationErrorMessage).contains("tbl6_MixEd_UNDERScore should be in lower_snake_case")
    assertThat(schemaValidationErrorMessage).contains("tbl6-lower-hyphen should be in lower_snake_case")
    assertThat(schemaValidationErrorMessage).contains("tbl6DatabaseCamelcase should be in lower_snake_case")
    assertThat(schemaValidationErrorMessage)
      .contains("tbl6DatabaseCamelcase should exactly match hibernate tbl6_database_camelcase")
    assertThat(schemaValidationErrorMessage)
      .contains("tbl6_hibernate_camelcase should exactly match hibernate tbl6HibernateCamelcase")
  }
  ...
  @Qualifier @Target(AnnotationTarget.FIELD, AnnotationTarget.FUNCTION) annotation class ValidationDb

  @Entity
  @Table(name = "`quoted_basic_table`")
  class DbBasicTestTable : DbUnsharded<DbBasicTestTable>, DbTimestampedEntity {

    @javax.persistence.Id @GeneratedValue override lateinit var id: Id<DbBasicTestTable>

    @Column var tbl1_string_nullable: String? = null

    @Column var tbl1_int_nullable: Int? = null

    @Column var tbl1_bin_nullable: ByteString? = null

    @Column(nullable = false) lateinit var tbl1_string: String

    @Column(nullable = false) var tbl1_int: Int = 0

    @Column(name = "`tbl1_bin`", nullable = false) lateinit var anotherNameForThisBinColumn: ByteString
  ...
  class DbHibernateOnlyTable : DbUnsharded<DbHibernateOnlyTable> {
    @javax.persistence.Id @GeneratedValue override lateinit var id: Id<DbHibernateOnlyTable>

    @Column(nullable = false) lateinit var string_column: String

    @Column(nullable = false) var int_column: Int = 0
  }

  @Entity
  @Table(name = "`missing_columns_table`")
  class DbMissingColumnsTable : DbUnsharded<DbMissingColumnsTable> {
    @javax.persistence.Id @GeneratedValue override lateinit var id: Id<DbMissingColumnsTable>

    @Column(nullable = false) lateinit var tbl4_string_column: String

    @Column(nullable = false) var tbl4_int_column: Int = 0

    @Column(nullable = false) lateinit var tbl4_string_column_hibernate: String

    @Column(nullable = false) var tbl4_int_column_hibernate: Int = 0
  }

  @Entity
  @Table(name = "`nullable_mismatch_table`")
  class DbNullableMismatchTable : DbUnsharded<DbNullableMismatchTable> {
    @javax.persistence.Id @GeneratedValue override lateinit var id: Id<DbNullableMismatchTable>

    @Column(nullable = false) var tbl5_both_notnull: Int = 0

    @Column var tbl5_hibernate_null: Int = 0

    @Column var tbl5_hibernate_null_default: Int = 0

    @Column var tbl5_both_null: Int = 0

    @Column(nullable = false) var tbl5_database_null: Int = 0
  }

  @Entity
  @Table(name = "bad_identifier_table")
  class DbBadIdentifierTable : DbUnsharded<DbBadIdentifierTable> {
    @javax.persistence.Id @GeneratedValue override lateinit var id: Id<DbBadIdentifierTable>

    @Column var tbl6CamelCase: Int = 0

    @Column var tbl6_UPPER_UNDERSCORE: Int = 0

    @Column var tbl6_MixEd_UNDERScore: Int = 0

    @Column(name = "`tbl6-lower-hyphen`") var tbl6LowerHyphen: Int = 0

    @Column var tbl6NotReallyUnique: Int = 0

    @Column var tbl6_not_REALLY_unique: Int = 0

    @Column var tbl6NotReallyUnique2: Int = 0

    @Column var tbl6_not_really_unique2: Int = 0

    @Column var tbl6HibernateCamelcase: Int = 0

    @Column var tbl6_database_camelcase: Int = 0
  }
}
