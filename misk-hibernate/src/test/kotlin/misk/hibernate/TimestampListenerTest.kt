...
abstract class TimestampListenerTest {
  ...
  fun timestampsUpdatedOnUpdate() {
    val createdAt = clock.instant()
    transacter.transaction { session -> session.save(DbMovie("Star Wars", LocalDate.of(1993, 6, 9))) }
      ...
      session.useConnection { c ->
        ...
        c.prepareStatement("select created_at from movies where id = ?").use { s ->
          ...
          s.executeQuery().use { rs ->
            rs.next()
            assertThat(rs.getTimestamp(1).time).isEqualTo(clock.instant().toEpochMilli())
          }
        }
      }
    }
