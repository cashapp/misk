...
class SchemaValidationTest {
  ...
  private val schemaValidationErrorMessage: String by lazy {
    assertThrows<IllegalStateException> {
      val schemaValidation = SchemaValidation(sessionFactory, config.data_source)
  ...
  @Test
  fun basic() {
    println(schemaValidationErrorMessage)
  }

  @Test
  fun findMissingTablesInOrm() {
    assertThat(schemaValidationErrorMessage).contains(
        "The hibernateSchema is missing tables [database_only_table, unquoted_database_table]")
  }

  @Test
  fun findMissingTablesInDb() {
    assertThat(schemaValidationErrorMessage).contains(
        "The databaseSchema is missing tables [hibernate_only_table].")
  }

  @Test
  fun findMissingColumnsInOrm() {
    assertThat(schemaValidationErrorMessage).contains(
  ...
  fun catchBadColumnNames(){
    println(SchemaValidation.normalize("tbl6DatabaseCamelcase"))
    assertThat(schemaValidationErrorMessage).contains(
  ...
  fun catchNotReallyUniqueColumnNames(){
    assertThat(schemaValidationErrorMessage).contains(
        "\"BAD_identifier_table\" has duplicate identifiers [[tbl6NotReallyUnique, tbl6_not_really_unique]]")

    assertThat(schemaValidationErrorMessage).contains(
        "\"bad_identifier_table\" has duplicate identifiers " +
