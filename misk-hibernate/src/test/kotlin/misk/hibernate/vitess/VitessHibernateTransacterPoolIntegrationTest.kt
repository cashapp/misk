...
/**
 * Integration test that verifies Vitess reader transacter properly uses the reader connection pool and routes queries
 * to replica tablets.
 *
...
class VitessHibernateTransacterPoolIntegrationTest {
  ...
  @MiskTestModule val module = MoviesTestModule(type = DataSourceType.VITESS_MYSQL, allowScatters = true)
  ...
  fun `reader transacter can query data written by writer transacter`() {
    ...
    val readMovie =
      readerTransacter.transaction { session -> queryFactory.newQuery<MovieQuery>().id(movieId).uniqueResult(session) }
  ...
  fun `reader transacter can perform complex queries with joins`() {
    ...
    readerTransacter.transaction { session ->
      val characters = queryFactory.newQuery<CharacterQuery>().allowTableScan().list(session)

      assertThat(characters).hasSize(2)
      assertThat(characters.map { it.name }).containsExactlyInAnyOrder("Dom Cobb", "Bruce Wayne")
  ...
  fun `replicaRead on reader transacter works`() {
    ...
    val replicaReadResult =
      readerTransacter.replicaRead { session -> queryFactory.newQuery<MovieQuery>().id(movieId).uniqueResult(session) }
    ...
    val target =
      readerTransacter.replicaRead { session ->
        session.useConnection { c -> c.createStatement().use { it.executeQuery("SHOW VITESS_TARGET").uniqueString() } }
      }
  ...
  fun `replicaRead on writer transacter works`() {
    ...
    val replicaReadResult =
      writerTransacter.replicaRead { session -> queryFactory.newQuery<MovieQuery>().id(movieId).uniqueResult(session) }
  ...
  fun `reader transacter respects read-only mode`() {
    ...
    readerTransacter.transaction { session ->
      val movie = queryFactory.newQuery<MovieQuery>().id(movieId).uniqueResult(session)
      movie!!.name = "Modified Title"
    }

    val verifyMovie =
      writerTransacter.transaction { session -> queryFactory.newQuery<MovieQuery>().id(movieId).uniqueResult(session) }
