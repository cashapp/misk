...
class VitessShardTargetIntegrationTest {
  ...
  @BeforeEach
  fun setup() {
    jp = transacter.save(DbMovie("Jurassic Park", LocalDate.of(1993, 6, 9)))
    sw = transacter.createInSeparateShard(jp) { DbMovie("Star Wars", LocalDate.of(1977, 5, 25)) }
  }
  ...
  fun `target(shard) works in replica reads`() {
    transacter.replicaRead { session ->
      session.target(jp.shard(session)) {
        assertThat(queryFactory.newQuery<MovieQuery>().allowTableScan().name("Jurassic Park").uniqueResult(session))
          .isNotNull

        assertThat(queryFactory.newQuery<MovieQuery>().allowTableScan().name("Star Wars").uniqueResult(session))
          .isNull()
  ...
  fun `readTransacter - target(destination) works in replicaRead`() {
    ...
    readerTransacter.replicaRead { session ->
      ...
      assertThat(queryFactory.newQuery<MovieQuery>().allowTableScan().name("Star Wars").uniqueResult(session)).isNotNull
    }
  }
  ...
  fun assertShardWriteFails(session: Session) {
    assertThrows<IllegalStateException> {
      ...
      .also { ex -> assertThat(ex).hasMessage("Saving isn't permitted in a read only session.") }
  }
}
