...
/**
 * Test suite that verifies that Vitess query hints work as intended in
 * a Vitess environment.
 */
@MiskTest(startService = true)
class VitessQueryHintsIntegrationTest {
  @MiskExternalDependency
  private val dockerVitess = DockerVitess(
    containerName = "vitess_query_hints_integ_test_db",
    enableScatters = false,
    port = 28003)

  @MiskTestModule
  val module = MoviesTestModule(type = DataSourceType.VITESS_MYSQL, allowScatters = false)
  ...
  fun `scatter query will fail by default but succeed with hint`() {
    ...
    assertThat(exception.message).contains("Scatter query detected. Must be opted-in through the `allow scatter` Vitess query hint.")
  ...
  fun `multiple hints with query timeout`() {
    val exception = assertThrows<PersistenceException> {
      transacter.transaction(
        { session ->
          session.hibernateSession.createNativeQuery("SELECT SLEEP(2)")
