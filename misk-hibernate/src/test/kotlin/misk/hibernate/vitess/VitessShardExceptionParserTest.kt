...
class VitessShardExceptionParserTest {
  ...
  @Test fun parseShardInfo_withSqlException_returnsShard() {
  ...
  @Test fun parseShardInfo_withGenericJDBCException_returnsShard() {
  ...
  @Test fun parseShardInfo_withNonShardException_returnsEmpty() {
  ...
  @Test fun parseShardInfo_withSqlExceptionNoShardInfo_returnsEmpty() {
  ...
  @Test fun parseShardInfo_withGenericJDBCExceptionNoShardInfo_returnsEmpty() {
    val cause = SQLException("Generic database error")
    val genericException = GenericJDBCException("Database error", cause)

    val result: Optional<VitessShardExceptionData> =
      parser.parseShardInfo(genericException)

    Assertions.assertThat(result).isEmpty()
  }

  @Test fun parseShardInfo_withNullCause_returnsEmpty() {
  ...
  @Test fun parseShardInfo_withMalformedShardString_returnsEmpty() {
    val sqlException = SQLException("target: malformed.shard.string: vttablet:")

    val result: Optional<VitessShardExceptionData> =
      parser.parseShardInfo(sqlException)

    Assertions.assertThat(result).isEmpty()
  }

  @Test fun parseShardInfo_withRangeShardFormat_returnsShard() {
  ...
  @Test fun parseShardInfo_withGenericJDBCExceptionRangeFormat_returnsShard() {
  ...
  @Test fun parseShardInfo_withOpenRangeShardFormat_returnsShard() {
  ...
  @Test fun parseShardInfo_withGenericJDBCExceptionOpenRangeFormat_returnsShard() {
  ...
  @Test fun parseShardInfo_withSixNestedGenericJDBCExceptionOpenRangeFormat_returnsShard() {
    ...
    val result: Optional<VitessShardExceptionData> =
      parser.parseShardInfo(genericExceptionFive)
  ...
  @Test fun parseShardInfo_withDeadlineExceeded_returnsShard() {
  ...
  @Test fun parseShardInfo_withNotServing_returnsShard() {
  ...
  @Test fun parseShardInfo_withTransactionEnded_returnsShard() {
  ...
  @Test fun parseShardInfo_withNoShardInfoTransactionEnded_returnsEmpty() {
  ...
  @Test fun parseShardInfo_withGenericJDBCExceptionNoShardInfoTransactionEnded_returnsEmpty() {
  ...
  @Test fun parseShardInfo_withNotServingState_returnsShard() {
  ...
  @Test fun parseShardInfo_withGenericJDBCExceptionNotServingState_returnsShard() {
  ...
  @Test fun parseShardInfo_withExcludedMessage_emitsMetricReturnsRowCountExceeded() {
    val excludedMessage =
      "target: sharded_keyspace.-40.primary: vttablet: Row count exceeded 10000"
  ...
  @Test fun parseShardInfo_withReplicaTransactionEnded_emitsMetrics() {
  ...
  @Test fun parseShardInfo_withDuplicateShardPatterns_emitsMetricOnce() {
  ...
  @Test fun parseShardInfo_withSqlException_returnsVitessShardExceptionData() {
    val sqlException = SQLException(SQL_EXCEPTION_MESSAGE)
    ...
    Assertions.assertThat(exception.shard.toString()).isEqualTo(
      EXPECTED_SHARD_ID
    )
    Assertions.assertThat(exception.isShardHealthError).isTrue()
    Assertions.assertThat(exception.isPrimary).isTrue()
    Assertions.assertThat(exception.exceptionMessage).isEqualTo(
      SQL_EXCEPTION_MESSAGE
    )
  }

  @Test fun parseShardInfo_withGenericJDBCException_returnsVitessShardExceptionData() {
    val cause = SQLException(SQL_EXCEPTION_MESSAGE)
    val genericException = GenericJDBCException("Database error", cause)
  ...
  @Test fun parseShardInfo_withResultSetExtractionError_doesNotFindNestedShardError() {
    parser.configureStackDepth(118)
    val genericException =
      createNestedException("Outer exception", NOT_SERVING_STATE_MESSAGE, 119)
  ...
  companion object {
    private const val SQL_EXCEPTION_MESSAGE = ("target: sharded_keyspace.-40.primary: vttablet: "
    ...
    private const val NOT_SERVING_MESSAGE =
      ("target: sharded_keyspace.-40.primary: primary is not serving, "
        + "there is a reparent operation in progress")
    ...
    /**
     * Creates a nested exception chain with the specified depth.
     * The SQLException with the Vitess error message is at the bottom of the stack.
     * All other exceptions are generic SQLExceptions.
     *
     ...
     */
    private fun createNestedException(
      outerMessage: String,
      ...
      for (i in 1..depth) {
        exceptionStack = SQLException(
          "Layer " + (depth - i + 1) + " - " + outerMessage,
          exceptionStack
        )
      }
