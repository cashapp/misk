...
class SecretColumnTest {
  ...
  fun testHappyPath() {
    ...
    transacter.transaction { session ->
      val songRaw =
        queryFactory.newQuery(JerryGarciaSongRawQuery::class).allowTableScan().title("Dark Star").query(session)[0]
      assertThat(songRaw.album).isNotEqualTo(album)
    }
    // test that when retrieving from the database we get the plaintext value
    transacter.transaction { session ->
      val song =
        queryFactory.newQuery(JerryGarciaSongQuery::class).allowTableScan().title("Dark Star").query(session)[0]
  ...
  fun testNullValue() {
    val title = "Ripple"
    val length = 165
    transacter.transaction { session -> session.save(DbJerryGarciaSong(title, length)) }
    transacter.transaction { session ->
      val song = queryFactory.newQuery(JerryGarciaSongQuery::class).allowTableScan().title("Ripple").query(session)[0]
      assertThat(song.title).isEqualTo(title)
      assertThat(song.length).isEqualTo(length)
  ...
  fun testFailedDecryption() {
    ...
    transacter.transaction { session ->
      ...
      assertThatThrownBy {
          queryFactory.newQuery<JerryGarciaSongQuery>().allowTableScan().title(title).query(session)[0]
        }
  ...
  fun testGetRecordByEncryptedIndexedColumnSucceeds() {
    ...
    transacter.transaction { session ->
      ...
      val songs = queryFactory.newQuery<JerryGarciaSongQuery>().allowTableScan().reviewer(reviewer).query(session)
  ...
  fun testGetRecordByEncryptedNonIndexedColumnFails() {
    ...
    transacter.transaction { session ->
      ...
      val songs = queryFactory.newQuery<JerryGarciaSongQuery>().allowTableScan().album(album).query(session)
  ...
  fun testEncryptedIndxedQueryMultiple() {
    ...
    transacter.transaction { session ->
      ...
      val songRaw = queryFactory.newQuery(JerryGarciaSongRawQuery::class).allowTableScan().query(session)
  ...
  @Qualifier @Target(AnnotationTarget.FIELD, AnnotationTarget.FUNCTION) annotation class JerryGarciaDb

  @Entity
  @Table(name = "jerry_garcia_songs")
  class DbJerryGarciaSong : DbUnsharded<DbJerryGarciaSong> {
    @javax.persistence.Id @GeneratedValue override lateinit var id: Id<DbJerryGarciaSong>

    @Column(nullable = false) lateinit var title: String

    @Column(nullable = false) var length: Int = 0

    @Column(nullable = true) @SecretColumn(keyName = "albumKey", indexable = false) var album: ByteArray? = null

    @Column(nullable = true) @SecretColumn(keyName = "reviewerKey") var reviewer: ByteArray? = null
  ...
  class DbJerryGarciaSongRaw : DbUnsharded<DbJerryGarciaSongRaw> {
    @javax.persistence.Id @GeneratedValue override lateinit var id: Id<DbJerryGarciaSongRaw>

    @Column(nullable = false) lateinit var title: String

    @Column(nullable = false) var length: Int = 0

    @Column(nullable = true) var album: ByteArray? = null

    @Column(nullable = true) var reviewer: ByteArray? = null
  ...
  interface JerryGarciaSongQuery : Query<DbJerryGarciaSong> {
    @Constraint(path = "title") fun title(title: String): JerryGarciaSongQuery

    @Constraint(path = "album") fun album(album: ByteArray): JerryGarciaSongQuery

    @Constraint(path = "reviewer") fun reviewer(reviewer: ByteArray): JerryGarciaSongQuery

    @Select fun query(session: Session): List<SongInfo>
  }

  interface JerryGarciaSongRawQuery : Query<DbJerryGarciaSongRaw> {
    @Constraint(path = "title") fun title(title: String): JerryGarciaSongRawQuery
  ...
  data class SongInfo(
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return title == other.title &&
        length == other.length &&
        Arrays.equals(album, other.album) &&
  ...
  class TestModule : KAbstractModule() {
    override fun configure() {
      ...
      install(
        object : HibernateEntityModule(JerryGarciaDb::class) {
