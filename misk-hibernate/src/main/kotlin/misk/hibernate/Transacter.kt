...
interface Transacter {
  ...
  /**
   * Starts a transaction on the current thread, executes [block], and commits the transaction. If the block raises an
   * exception the transaction will be rolled back instead of committed.
   *
   * If retries are permitted (the default), a failed but recoverable transaction will be reattempted after rolling
   * back.
   *
   ...
   * * Consistency is eventual. If your application thread just wrote something in a transaction you may not see that
   *   write with a replica read as the write may not have replicated to the replica yet.
   * * There may be time jumping. As each query may end up at a separate replica that will likely be at a separate point
   *   in the replica stream. That means each query can jump back or forward in "time". (There is some support for
   *   internally consistent replica reads that peg a single replica in Vitess but we're not using that. If you need
   *   that functionality reach out to
   *
   * #vitess)
   * * Full scatters are allowed since you can increase the availability of these by adding more replicas.
   * * If no reader is configured for replica reads when installing the [HibernateModule], this method will throw an
   *   [IllegalStateException].
   * * Note: You can do it another way, where you annotate the [Transacter] with the readerQualifer defined by
   *   [HibernateModule], which will use the read only replica as the datasource.
   */
  ...
  /**
   * Creates a new transacter that produces read only sessions. This does not mean the underlying datasource is read
   * only, only that the session produced won't modify the database.
   */
  ...
  @VisibleForTesting fun allowCowrites(): Transacter
  ...
  /**
   * Opens a Hibernate session and acquires a MySQL advisory lock with the name [lockKey]. It is safe to call
   * [Transacter.transaction] within [block] one or more times. The lock will be released only when this method return.
   *
 ...
 *
 * New objects must be persisted with an explicit call to [Session.save]. Updates are performed implicitly by modifying
 ...
 * }
 */
fun <T> Transacter.transaction(shard: Shard, block: (session: Session) -> T): T = transaction {
  it.target(Destination(shard = shard, tabletType = TabletType.PRIMARY)) { block(it) }
}
...
fun <T> Transacter.failSafeRead(block: (session: Session) -> T): T =
  try {
    transaction { block(it) }
  } catch (e: PersistenceException) {
    if (tabletDoesNotExists(e)) {
      replicaRead { block(it) }
    } else {
      throw e
    }
  }
