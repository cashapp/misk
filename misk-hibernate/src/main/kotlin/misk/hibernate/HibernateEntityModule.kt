...
abstract class HibernateEntityModule(private val qualifier: KClass<out Annotation>) : KAbstractModule() {
  ...
  protected fun addEntities(vararg entities: KClass<out DbEntity<*>>) {
    entities.forEach { entity -> addEntity(dbEntityClass = entity) }
  }

  /** Install entities with an AccessAnnotation to set run query permissions for Database-Query tab */
  protected fun addEntities(accessAnnotationClass: KClass<out Annotation>, vararg entities: KClass<out DbEntity<*>>) {
    entities.forEach { entity -> addEntity(dbEntityClass = entity, accessAnnotationClass = accessAnnotationClass) }
  }
  ...
  protected fun <T : DbEntity<T>> addEntity(
    ...
    multibind<HibernateEntity>(qualifier).toInstance(HibernateEntity(dbEntityClass))

    multibind<DatabaseQueryMetadata>()
      .toProvider(
  ...
  protected fun addHibernateEntities(vararg entities: KClass<out DbEntity<*>>) {
    entities.forEach { entity -> addHibernateEntity(dbEntityClass = entity) }
  }

  /** Bind just the HibernateEntity using the provided qualifier */
  protected fun <T : DbEntity<T>> addHibernateEntity(dbEntityClass: KClass<T>) {
    multibind<HibernateEntity>(qualifier).toInstance(HibernateEntity(dbEntityClass))
  }
  ...
  protected inline fun <
    reified T : DbEntity<T>,
    reified Q : Query<T>,
    reified AA : Annotation,
  ...
  protected fun <T> bindListener(
    ...
    val key =
      Key.get(Any::class.java, Names.named("HibernateEventListener@${nextHibernateEventListener.getAndIncrement()}"))

    // Create a multibinding for a ListenerRegistration that uses the above key.
    multibind<ListenerRegistration>(qualifier).toInstance(ListenerRegistration(type, getProvider(key), policy))
