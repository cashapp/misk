...
internal fun Session.tryAcquireLock(lockKey: String): Boolean {
  ...
  return when (dialect) {
    "org.hibernate.dialect.PostgreSQL95Dialect" ->
      acquirePostgresLock(lockKey)
    "org.hibernate.dialect.MySQL8Dialect",
    "misk.hibernate.vitess.VitessDialect" ->
        acquireMySqlLock(lockKey)
    else -> throw NotImplementedError("Unsupported dialect: $dialect")
  }
}

private fun Session.acquireMySqlLock(lockKey: String): Boolean {
  require(lockKey.length <= 64) {
    "MySQL requires a lock key <= 64 characters, was ${lockKey.length}"
  }

  val query = this.createNativeQuery("SELECT GET_LOCK(:lockKey, 0)")
...
private fun Session.acquirePostgresLock(lockKey: String): Boolean {
  val query = this.createNativeQuery("SELECT pg_try_advisory_lock(hashtext(:lockKey))")
    .setParameter("lockKey", lockKey)
...
internal fun Session.tryReleaseLock(lockKey: String) {
  ...
  return when (dialect) {
    "org.hibernate.dialect.PostgreSQL95Dialect" ->
      releasePostgresLock(lockKey)
    "org.hibernate.dialect.MySQL8Dialect",
    "misk.hibernate.vitess.VitessDialect" ->
      releaseMySqlLock(lockKey)
    else -> throw NotImplementedError("Unsupported dialect: $dialect")
  }
}

private fun Session.releaseMySqlLock(lockKey: String) {
  val query = this.createNativeQuery("SELECT RELEASE_LOCK(:lockKey)")
    .setParameter("lockKey", lockKey)
  ...
  when (result) {
    0 ->
      throw IllegalStateException("Attempting to release lock $lockKey owned by another session")
    1 -> {}
    else ->
      throw IllegalStateException("Unexpected result: $result releasing $lockKey")
  }
}

private fun Session.releasePostgresLock(lockKey: String) {
  val query = this.createNativeQuery("SELECT pg_advisory_unlock(hashtext(:lockKey))")
    .setParameter("lockKey", lockKey)
