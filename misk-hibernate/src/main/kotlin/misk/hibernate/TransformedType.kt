 ...
 * when a value is read from the table, or from cache. Disassemble is called when a value is about to be written to a
 * table or cache.
 *
 ...
 *
 * class AppendTransformer(val context: TransformerContext) { fun assemble(owner: Any?, value: Serializable): Any =
 ...
 * @TransformedType(transformer = AppendTransformer::class, targetType = String::class) annotation class
 *   SuffixedString(val suffix: String)
 *
 * Then, annotate an entity field:
 *
 * @Entity
 * @Table(name="table_name") class DbEntity : DbUnsharded(DbEntity) { // ...
 *
 *     @Column("some_column")
 *     @SuffixedString(suffix="_suffix") var suffixedString : String = ""
 *
 * // ... }
 */
