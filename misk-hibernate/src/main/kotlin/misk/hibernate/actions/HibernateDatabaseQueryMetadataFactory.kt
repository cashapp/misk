...
@Singleton
internal class HibernateDatabaseQueryMetadataFactory
@Inject
constructor(private val accessAnnotationEntries: List<AccessAnnotationEntry>) {
  fun <T : DbEntity<T>> fromQuery(
    ...
    val queryTypesDynamicToInclude =
      if (queryClass != null) {
    ...
    val queryType =
      if (isDynamic) {
        ...
        Type(fields = queryTypes.map { (name, _) -> Field(name = name, repeated = false, type = name) })
      }

    val queryWebActionPath =
      if (isDynamic) {
  ...
  private fun <T : DbEntity<T>> addStaticQueryMethodMetadata(
    ...
    if (queryClass != null) {
      for (function in queryClass.criteriaFunctions()) {
        ...
        when {
          constraint != null -> constraintsResult += constraintMetadata(queryClass, function, constraint)
          order != null -> ordersResult += orderMetadata(queryClass, function, order)
          select != null -> selectsResult += selectMetadata(queryClass, function, select, function.returnType)
        }
      }
    }
  }
  ...
  private fun constraintMetadata(
    ...
    val field =
      onlyParameter?.name?.let { name -> createField(onlyParameter.type, name, onlyParameter.isVararg) }
        ?: Field(name = "Add Constraint", type = Boolean::class.simpleName!!, repeated = false)
    return Type(fields = listOf(field)) to
  ...
  private fun selectMetadata(
    ...
    val paths =
      if (select.path == "") {
        val maybeProjectionClass =
          (selectTargetReturnType.arguments.firstOrNull()?.type?.classifier as KClass<*>?)?.supertypes?.firstOrNull {
            it.classifier as KClass<*> == Projection::class
          }
        if (maybeProjectionClass != null) {
          // TODO (adrw) this projection class path parsing isn't working
          val paths =
            (maybeProjectionClass.classifier as KClass<*>)
              .members
              .map { member ->
                member.annotations
                  .map {
    ...
    val field =
      onlyParameter?.name?.let { name ->
        if (onlyParameter.type.classifier as KClass<*> == Session::class) {
          Field(name = "Add Select (paths=$paths)", type = Boolean::class.simpleName!!, repeated = false)
        } else {
          createField(onlyParameter.type, name, onlyParameter.isVararg)
        }
      }

    return Type(fields = field?.let { listOf(field) } ?: listOf()) to
  ...
  private fun createField(fieldType: KType, fieldName: String, repeated: Boolean): Field? {
  ...
  companion object {
    ...
    data class DynamicQuerySelect(val paths: List<String>? = listOf())

    data class QueryConfig(val maxRows: Int?)
    ...
    private fun getQueryConfigType() =
      listOf(QUERY_CONFIG_TYPE_NAME to Type(fields = listOf(Field(name = "maxRows", repeated = false, type = "Int"))))

    fun makeDynamicQueryTypes(dbEntityClass: KClass<out DbEntity<*>>): List<Pair<String, Type>> =
      getQueryConfigType() +
        listOf(
          DYNAMIC_CONSTRAINT_TYPE_NAME to
            Type(
              fields =
                listOf(
          ...
          QUERY_TYPE to
            Type(
              fields =
                listOf(
                  Field(name = QUERY_CONFIG_KEY, repeated = false, type = QUERY_CONFIG_TYPE_NAME),
                  Field(name = DYNAMIC_CONSTRAINTS_KEY, repeated = true, type = DYNAMIC_CONSTRAINT_TYPE_NAME),
