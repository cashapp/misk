 ...
 *
 * For general usage with Gradle, it's recommended to instead use the
 * [Vitess database Gradle plugin](misk/misk-vitess-database-gradle-plugin/README.md).
 */
class DockerVitess(
  /**
   * Whether to enable scatter queries. It's recommended to disable scatter queries by default in tests (i.e. set
   * `enableScatters` = `false`), and opt-in queries via the Vitess query hint to allow scatters (see
