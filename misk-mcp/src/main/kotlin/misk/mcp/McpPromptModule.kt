 ...
 *
 * This module registers MCP prompts that will be available through the configured MCP server. Prompts provide reusable
 * templates that AI models and other MCP clients can discover and use.
 *
 * ## Usage
 *
 ...
 *
 * Prompts can be organized into groups using [BindingQualifier] annotations. This allows multiple MCP servers to expose
 * different sets of prompts. Define a qualifier annotation and use it when creating both the server and prompt modules:
 * ```kotlin
 * @Qualifier
 ...
 *
 * Prompts are made available through the MCP server configured via [McpServerModule]. All registered prompts will be
 ...
 */
@ExperimentalMiskApi
class McpPromptModule<P : McpPrompt>
private constructor(private val promptClass: KClass<P>, private val qualifier: BindingQualifier?) : KAbstractModule() {

  override fun configure() {
    // Bind the MCP prompt to the named server's prompt set
    multibind<McpPrompt>(qualifier).to(promptClass.java)
  }
  ...
  companion object {
    ...
    inline fun <reified P : McpPrompt> create(groupAnnotation: Annotation?) =
      create(promptClass = P::class, groupAnnotation = groupAnnotation)
  }
}
