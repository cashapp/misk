 ...
 *
 * Adapts Misk's WebSocket infrastructure to the MCP Kotlin SDK transport interface. Provides persistent bidirectional
 * communication with MCP subprotocol validation. Unlike SSE transport, WebSockets maintain persistent connections
 * without the need for cross call session management.
 *
...
internal class MiskWebSocketServerTransport(override val call: HttpCall, private val webSocket: WebSocket) :
  MiskServerTransport() {
  ...
  init {
    ...
    require(subprotocol == MCP_SUBPROTOCOL) { "Invalid subprotocol: $subprotocol, expected $MCP_SUBPROTOCOL" }
