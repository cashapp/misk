...
internal class ToolAnnotationBuilder {
   ...
   * interaction is closed. For example, the world of a web search tool is open, whereas that of a memory tool is not.
   *
   * Default: true
   */
...
internal fun Tool.Companion.build(name: String, inputSchema: ToolSchema, builder: ToolBuilder.() -> Unit) =
  ToolBuilder(name, inputSchema).apply(builder).build()

internal fun ToolAnnotations.Companion.build(builder: ToolAnnotationBuilder.() -> Unit) =
