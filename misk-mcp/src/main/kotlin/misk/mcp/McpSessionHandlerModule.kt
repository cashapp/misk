 ...
 *
 * This module registers an [McpSessionHandler] implementation with the dependency injection framework, enabling
 * automatic session management for MCP servers using StreamableHTTP transport (StreamableHTTP-based communication via
 * `@McpPost`, `@McpGet`, and `@McpDelete` endpoints).
 *
 * ## Transport Compatibility
 *
 * **StreamableHTTP Transport Only**: This module is designed exclusively for StreamableHTTP transport using Server-Sent
 * Events. Session handlers are not used with WebSocket transport (`@McpWebSocket`), which maintains connection state
 ...
 * ```
 *
 * Once installed, the framework will automatically handle session lifecycle for StreamableHTTP requests and include
...
class McpSessionHandlerModule<T : McpSessionHandler>(
  ...
  companion object {
     ...
     *
     * This is the base factory method that accepts a [KClass] for both the session handler and the group annotation.
     * Use the reified generic version for more convenient type-safe creation.
     *
     ...
     *
     * This is the recommended way to register session handlers. The handler type is specified using reified generics
     ...
     * @return A configured McpSessionHandlerModule instance
     */
    inline fun <reified T : McpSessionHandler> create(groupAnnotationClass: KClass<out Annotation>? = null) =
      McpSessionHandlerModule(mcpSessionHandlerClass = T::class, groupAnnotationClass = groupAnnotationClass)
  }
}
