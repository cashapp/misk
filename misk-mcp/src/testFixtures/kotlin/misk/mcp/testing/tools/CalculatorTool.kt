...
data class CalculatorToolInput(
  ...
  @Description("first term, an integer") val first_term: Int,
  @Description("second term, an integer") val second_term: Int,
) {
...
class CalculatorTool @Inject constructor() : StructuredMcpTool<CalculatorToolInput, CalculatorToolOutput>() {
  ...
  override suspend fun handle(input: CalculatorToolInput): ToolResult {
    ...
    return when (operation) {
      Operation.ADD -> ToolResult(CalculatorToolOutput(result = input.first_term + input.second_term))

      Operation.SUBTRACT -> ToolResult(CalculatorToolOutput(result = input.first_term - input.second_term))

      Operation.MULTIPLY -> ToolResult(CalculatorToolOutput(result = input.first_term * input.second_term))
      ...
      Operation.DIVIDE ->
        if (input.second_term != 0) {
          ...
          ToolResult(TextContent("Division by zero is not allowed."), isError = true)
        }
    }
  }
}

suspend fun Client.callCalculatorTool(first_term: Int, second_term: Int, operation: Operation) =
  callTool(
    name = "calculator",
    arguments = mapOf("operation" to operation.name, "first_term" to first_term, "second_term" to second_term),
  )
