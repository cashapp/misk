...
class VisibilityTimeoutCalculatorTest {
  ...
  fun `respects minimum visibility timeout`() {
    ...
    val result =
      calculator.calculateVisibilityTimeout(currentReceiveCount = 1, queueVisibilityTimeout = queueVisibilityTimeout)
  ...
  fun `increases timeout with receive count`() {
    ...
    val secondRetry =
      calculator.calculateVisibilityTimeout(currentReceiveCount = 2, queueVisibilityTimeout = queueVisibilityTimeout)
  ...
  fun `adds jitter to calculated timeout`() {
    val results =
      List(10) { calculator.calculateVisibilityTimeout(currentReceiveCount = 5, queueVisibilityTimeout = 1) }
