  ...
  internal abstract inner class QueueReceiver(queueName: QueueName) {
    ...
    protected fun fetchMessages(sqsBatchSize: Int, waitTimeSeconds: Int? = null): List<SqsJob> {
      ...
      for (message in messages) {
        try {
          val sentTimestamp = message.attributes()[MessageSystemAttributeName.SENT_TIMESTAMP]!!.toLong()
          val receiveCount = message.attributes()[MessageSystemAttributeName.APPROXIMATE_RECEIVE_COUNT]!!.toLong()
