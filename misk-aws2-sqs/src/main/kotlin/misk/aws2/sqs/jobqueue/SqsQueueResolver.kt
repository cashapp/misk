...
@Inject
constructor(private val sqsClientFactory: SqsClientFactory, private val sqsConfig: SqsConfig) {
  ...
  fun getQueueUrl(queueName: QueueName): String {
    return queueUrlCache.computeIfAbsent(queueName) {
      ...
      val retryQueueUrlRequest =
        GetQueueUrlRequest.builder().queueName(queueName.value).queueOwnerAWSAccountId(ownerAccountId).build()
