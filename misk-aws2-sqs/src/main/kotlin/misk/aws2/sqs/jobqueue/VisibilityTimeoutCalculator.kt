...
class VisibilityTimeoutCalculator @Inject constructor() {
  /**
   * Calculates exponential backoff visibility timeout based on the receive count.
   * At minimum, it should be equal to provided queue visibility timeout.
   * At maximum, it should be 12 hours, which is the highest visibility timeout
   * allowed by SQS.
   *
   * Calculated visibility timeout adds some jitter.
   */
  fun calculateVisibilityTimeout(
    currentReceiveCount: Int,
  ...
  companion object {
    ...
    /**
     * We calculate new visibility timeout for backoff based on the receive
     * count. Going beyond 17 will exceed MAX_JOB_DELAY anyway.
     * 2^17/2=65536 which is more than 43200 (12 hours)
     */
