...
## Leased SQS Compatibility

The `misk.aws2.sqs.jobqueue.leased` package exists only as a compatibility path for services that
are migrating from the previous `misk-aws` SQS implementation and are sure they need leases to limit
global SQS consumption.

Do not use the leased modules by default. Limiting consumption with leases is not recommended for new
...
If a service does need this compatibility path, install the explicitly named leased Guice modules:
`LeasedAwsSqsJobQueueModule`, `LeasedAwsSqsJobHandlerModule`, or `LeasedAwsSqsBatchJobHandlerModule`.
