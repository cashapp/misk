 ...
 * A Redis client.
 *
 * Note: special care must be taken if your Redis is running in cluster mode, as keys **must** belong to the same slot
 * in a single command operation like [lmove]. You can control which hash slot a key belongs to a certain degree by
 * making use of `{hashtags}` in the key name.
 *
 * See the [Redis Cluster Spec](https://redis.io/docs/reference/cluster-spec/#hash-tags) for more information.
 */
interface Redis {
  /**
   ...
   * @return 0 if none of the keys were deleted, otherwise a positive integer representing the number of keys that were
   *   deleted
   */
   ...
   * @return a list of String in the same order as the specified list of keys. For each key, a value will be returned if
   *   a key was found, otherwise null is returned.
   */
   ...
   *
   * @return The number of fields that were removed from the hash. If the key does not exist, it is treated as an empty
   *   hash and 0 is returned.
   */
   ...
   * @return a List<ByteString?> of the values for the specific fields requested, in the same order of the request. Null
   *   for missing fields
   */
   ...
   *
   * NB: Implementations using Jedis 4 or seeking to emulate Jedis should use [checkHrandFieldCount] to avoid surprising
   * behaviour like retrieving a result map which is smaller than requested by a completely random factor.
   */
   ...
   *
   * @param cursor The scan cursor. This should first be "0". Then subsequent cursor values will be taken from the
   *   returned ScanResults.
   * @param matchPattern A glob-like match pattern to filter keys by. If this is not provided, then all keys will be
   *   scanned.
   * @param count A hinted desired batch size to be returned in each ScanResult. Note that this is just a hint and there
   *   are no guarantees on the actual size of each ScanResult.
   * @return A ScanResult containing the next cursor and the current batch of keys. If the returned cursor is "0", then
   *   there are no more keys left in the iteration.
   */
   ...
   * @param field the field
   * @param value the value to set
   * @return The number of fields that were added. Returns 0 if all fields had their values overwritten.
   */
   ...
   *
   * Note: this is a string operation because Redis does not have a dedicated integer type. The string stored at the key
   * is interpreted as a base-10 64 bit signed integer to execute the operation.
   *
   * Redis stores integers in their integer representation, so for string values that actually hold an integer, there is
   * no overhead for storing the string representation of the integer.
   */
  ...
  /**
   * [blmove] is the blocking variant of [lmove]. When source contains elements, this command behaves exactly like
   * [lmove]. When used inside a MULTI/EXEC block, this command behaves exactly like [lmove]. When source is empty,
   * Redis will block the connection until another client pushes to it or until timeout (a double value specifying the
   * maximum number of seconds to block) is reached. A timeout of zero can be used to block indefinitely.
   *
   ...
   * like [rpoplpush]. When used inside a MULTI/EXEC block, this command behaves exactly like [rpoplpush]. When source
  ...
  /**
   * Atomically returns and removes the first/last element (head/tail depending on the [from] argument) of the list
   * stored at source, and pushes the element at the first/last element (head/tail depending on the [to] argument) of
   * the list stored at destination.
   *
   * For example: consider source holding the list a,b,c, and destination holding the list x,y,z. Executing LMOVE source
   * destination RIGHT LEFT results in source holding a,b and destination holding c,x,y,z.
   *
   * If source does not exist, the value nil is returned and no operation is performed. If source and destination are
   * the same, the operation is equivalent to removing the first/last element from the list and pushing it as first/last
   * element of the list, so it can be considered as a list rotation command (or a no-op if [from] is the same as [to]).
   *
   * Throws an error if using Redis Cluster and source and destination are not in the same hash slot
   *
  ...
  /**
   * Insert all the specified [elements] at the head of the list stored at [key]. If [key] does not exist, it is created
   * as empty list before performing the push operations. When [key] holds a value that is not a list, an error is
   * returned.
   *
  ...
  /**
   * Insert all the specified [elements] at the tail of the list stored at [key]. If [key] does not exist, it is created
   * as empty list before performing the push operations. When [key] holds a value that is not a list, an error is
   * returned.
   *
   * It is possible to push multiple elements using a single command call just specifying multiple arguments at the end
  ...
  /**
   * Blocking version of [lpop]. Pops an element from the first non-empty list in [keys], checking keys in the provided
   * order. If all lists are empty, blocks the connection until an [lpush] or [rpush] operation occurs on one of the
   * keys, or until [timeoutSeconds] expires.
   *
   ...
   *
   * The offset [start] and [stop] are zero-based indexes. These offsets can also be negative numbers indicating offsets
   * starting at the end of the list.
   */
  fun ltrim(key: String, start: Long, stop: Long)

  /**
   * Removes the first count occurrences of elements equal to element from the list stored at key. The count argument
   * influences the operation in the following ways: count > 0: Remove elements equal to element moving from head to
   * tail. count < 0: Remove elements equal to element moving from tail to head. count = 0: Remove all elements equal to
   * element. For example, LREM list -2 "hello" will remove the last two occurrences of "hello" in the list stored at
   * list.
   *
   ...
   *
   * @return true if the timeout has been removed. false if the key does not exist or does not have an associated
   *   timeout.
   */
   ...
   * [set], GETSET and all the *STORE commands. This means that all the operations that conceptually alter the value
   * stored at the key without replacing it with a new one will leave the timeout untouched. For instance, incrementing
   * the value of a key with [incr], pushing a new value into a list with LPUSH, or altering the field value of a hash
   ...
   *
   * If a key is overwritten by RENAME, like in the case of an existing key Key_A that is overwritten by a call like
   * RENAME Key_B Key_A, it does not matter if the original Key_A had a timeout associated or not, the new key Key_A
   * will inherit all the characteristics of Key_B.
   *
   * Note that calling [expire]/[pExpire] with a non-positive timeout or [expireAt]/[pExpireAt] with a time in the past
   * will result in the key being deleted rather than expired (accordingly, the emitted key event will be del, not
   * expired).
   *
   * @return true if the timeout was set. false if the timeout was not set. e.g. key doesn't exist, or operation skipped
   ...
   * representing the TTL (time to live), it takes an absolute Unix timestamp (seconds since January 1, 1970). A
  ...
  /**
   * Flushes all the previously watched keys for a transaction. If you call EXEC or DISCARD, there's no need to manually
   * call UNWATCH.
   */
  ...
  /**
   * Runs a block of Redis commands in a pipeline, for better performance. Pipelined command responses are not returned
   * until the block completes. If you need to use the results of each command immediately, either save the [Supplier]s
   * and call them later, or use non-pipelined operations.
   *
  ...
  /**
   * Adds the specified [member] with the specified [score] to the sorted set at the [key]. If a specified [member] is
   * already a member of the sorted set, the [score] is updated and the element reinserted at the right position to
   * ensure the correct ordering.
   *
   * If [key] does not exist, a new sorted set with the specified [member] as sole member is created, like if the sorted
   * set was empty. If the [key] exists but does not hold a sorted set, an error is returned.
   *
   * ZADD supports a list of [options], specified after the name of the key and before the first score argument. The
   * complete list of options can be found in [ZAddOptions].
   */
  fun zadd(key: String, score: Double, member: String, vararg options: ZAddOptions): Long

  /**
   * Adds all the specified members with the specified scores in [scoreMembers] to the sorted set at the [key]. If a
   * specified [member] is already a member of the sorted set, the [score] is updated and the element reinserted at the
   ...
   *
   * ZRANGE can perform different [type]s of range queries: by index (rank), by the score, or by lexicographical order.
   * Currently only index and score type range queries are supported. See [ZRangeType] for different types of range
   * queries.
   *
   * You can specify the [start] and [stop] of the range you want to filter by. Depending on the [type] you will have to
   * use the appropriate type of [ZRangeMarker].
   *
   * The order of elements is from the lowest to the highest score. Elements with the same score are ordered
   * lexicographically.
   *
   * Setting [reverse] reverses the ordering, so elements are ordered from highest to lowest score, and score ties are
  ...
  enum class ZRangeType {
    /**
     * The <start> and <stop> arguments represent zero-based indexes. These arguments specify an inclusive range.
     *
     * The indexes can also be negative numbers indicating offsets from the end of the sorted set, with -1 being the
     * last element of the sorted set and so on.
     *
     * Out of range indexes do not produce an error. If <start> is greater than either the end index of the sorted set
     * or <stop>, an empty list is returned. If <stop> is greater than the end index of the sorted set, Redis will use
     ...
     *
     * <start> and <stop> can be -inf and +inf, denoting the negative and positive infinities, respectively. This means
     * that you are not required to know the highest or lowest score in the sorted set to get all elements from or up to
     * a certain score.
     *
     * By default, the score intervals specified by <start> and <stop> are closed (inclusive). It is possible to specify
     * an open interval.
     *
  ...
  abstract class ZRangeMarker(val value: Any, val included: Boolean)

  data class ScanResult(val cursor: String, val keys: List<String>)

  data class ZRangeRankMarker(val longValue: Long) : ZRangeMarker(longValue, true)

  /**
   * To be used when [ZRangeType] is [ZRangeType.INDEX]. The [intValue] should be an integer specifying the index (start
   * or stop)
   */
  data class ZRangeIndexMarker(val intValue: Int) : ZRangeMarker(intValue, true)

  /**
   * To be used when [ZRangeType] is [ZRangeType.SCORE]. The [doubleValue] should be a double specifying the score
   * (start or stop) By default the range is included. Set [isIncluded] to false in order to exclude the start or stop.
   */
  data class ZRangeScoreMarker @JvmOverloads constructor(val doubleValue: Double, val isIncluded: Boolean = true) :
    ZRangeMarker(doubleValue, isIncluded) {
    override fun toString(): String {
      var ans =
        when (this.doubleValue) {
  ...
  /**
   * The limit argument in [zrange] and [zrangeWithScores] can be used to obtain a sub-range from the matching elements
   * similar to SELECT LIMIT offset, count in SQL. A negative [count] returns all elements from the [offset]. Keep in
   * mind that if <offset> is large, the sorted set needs to be traversed for <offset> elements before getting to the
   * elements to return, which can add up to O(N) time complexity.
   */
  data class ZRangeLimit(val offset: Int, val count: Int)

  /**
   * Options for ZADD. Not all options are compatible with one another. See the
   * [ZADD command documentation](https://redis.io/commands/zadd/) for more information.
   *
  ...
  enum class ZAddOptions {
     ...
     * not counted. Note: normally the return value of ZADD only counts the number of new elements added.
     */
    CH;
...
/**
 * Validates [count] is positive and non-zero. This is to avoid unexpected behaviour due to limitations in Jedis:
 * https://github.com/redis/jedis/issues/3017
 *
 * This check can be removed when Jedis v5.x is released with full support for the behaviours for negative counts that
 * are specified by Redis.
 *
...
inline fun checkHrandFieldCount(count: Long) {
  require(count > -1) { "This Redis client does not support negative field counts for HRANDFIELD." }
  require(count > 0) { "You must request at least 1 field." }
}
