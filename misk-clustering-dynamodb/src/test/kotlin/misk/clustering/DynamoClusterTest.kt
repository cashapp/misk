...
class DynamoClusterTest {
  ...
  @Test
  fun basic() {
    val enhancedClient = DynamoDbEnhancedClient.builder().dynamoDbClient(ddb).build()
  ...
  fun inspectAddedRecords() {
    ...
    assertTrue(
      table.scan().items().all { m -> m.expires_at!! <= expiredTimeUnit && m.expires_at!! > m.updated_at!! / 1000 }
    )
  }
