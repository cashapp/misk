...
open class ValidWebEntry
@JvmOverloads
constructor(val valid_slug: String = "", val valid_url_path_prefix: String = "/") {
  init {
  ...
  companion object {
    ...
    fun slugify(annotation: KClass<out Annotation>) = annotation.simpleName!!.slugify()
    ...
    fun String.slugify() =
      this.lowercase().replace(" ", "-").replace(".", "-").replace("_", "-").replace("/", "").replace(":", "")

    /** Slug must must only contain characters in ranges [a-z], [0-9] or '-'. */
    private fun String.requireValidSlug() = apply {
      require(
        CharMatcher.inRange('a', 'z').or(CharMatcher.inRange('0', '9')).or(CharMatcher.`is`('-')).matchesAllOf(this)
      ) {
    ...
    private fun String.requireValidUrlPathPrefix() = apply {
      require(
        this.startsWith("http") ||
          ...
          this.matches(Regex("(/[^/]+)*/")) ||
          // Ignores any trailing query parameters for service local links like /_admin/metadata/?q=config but still
          // enforces the trailing '/'
          if (this.contains("?")) this.split("?").dropLast(1).joinToString("?").matches(Regex("(/[^/]+)*/")) else false
      ) {
        "Invalid or unexpected [urlPathPrefix=$this]. " + "Must start with 'http' OR start and end with '/'."
      }
