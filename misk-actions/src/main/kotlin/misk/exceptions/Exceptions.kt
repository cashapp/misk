...
/**
 * Even though all kotlin exceptions are runtime exceptions. To ensure java inter-op all exception need to extend from
 * RuntimeException.
 */
...
open class WebActionException(
  ...
  val code: Int,
  /**
   * This is returned to the caller as is. Be mindful not to leak internal implementation details and possible
   * vulnerabilities in the response body.
   */
  ...
  /**
   * Details are used to enrich gRPC errors with additional proto-encoded messages. error_details.proto is a well-known
   * collection of details, but clients can define their own. This field is ignored for REST responses.
   *
  ...
  constructor(code: Int, message: String, cause: Throwable? = null) : this(code, message, message, cause)
...
/**
 * Base exception for when a server is acting as a gateway and gets invalid response from upstream. The message is not
...
/**
 * Base exception for when a server is acting as a gateway and cannot get a response in time. The message is not exposed
 * to the caller.
 */
...
private val GrpcStatus.isGrpcClientCode
  get() = true
