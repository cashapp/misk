...
/**
 * An [ExplicitReleaseBlockingQueue] is a [BlockingQueue] that only returns elements from [BlockingQueue.take],
 * [BlockingQueue.poll], and [BlockingQueue.peek] after a call to [ExplicitReleaseBlockingQueue.release]. Used by tests
 * that want to explicitly control when pollers receive queued items.
 */
class ExplicitReleaseBlockingQueue<T>
internal constructor(private val visible: BlockingQueue<T>, private val pending: BlockingQueue<T>) : BlockingQueue<T> {
  constructor() : this(LinkedBlockingQueue<T>(), LinkedBlockingQueue<T>())

  /**
   * releases up to n items from the pending queue, making them visible to [take], [poll], [peek], etc. This method does
   * not "extend credit"; if n > number of pending items on the queue, every currently pending item is made visible, but
   * any items added in the future continue to go onto the pending queue
   *
  ...
  override fun drainTo(c: MutableCollection<in T>?, maxElements: Int) = visible.drainTo(c, maxElements)
