...
fun KFunction<*>.asAction(
  ...
  val instanceParameter = instanceParameter ?: throw IllegalArgumentException("only methods may be actions")

  // Drop 'this' which is the function's first parameter.
  val actualParameters = parameters.drop(1)
  val actionName =
    instanceParameter.type.classifier?.let {
      when (it) {
  ...
  val acceptedMediaRange =
    when (dispatchMechanism) {
      DispatchMechanism.GRPC -> {
        ...
        listOf(MediaRange.parse(MediaTypes.APPLICATION_GRPC), MediaRange.parse(MediaTypes.APPLICATION_GRPC_PROTOBUF))
      }

      else ->
        findAnnotation<RequestContentType>()?.value?.flatMap { MediaRange.parseRanges(it) }?.toList()
