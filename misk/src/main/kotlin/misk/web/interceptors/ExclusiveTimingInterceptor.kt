 ...
 * 
 * Use this by installing via `install(ExclusiveTimingInterceptor.Module())`, and then inject a `ThreadLocal<DownstreamTime>` into places in your code where you want to exclude time.
 * 
 * You can also use a custom interceptor factory if you want to customize the metric name and tags. Install it via `install(ExclusiveTimingInterceptor.Module.of<MyExclusiveTimingInterceptorFactory>())` instead.
 */
class ExclusiveTimingInterceptor(
  ...
  override fun intercept(chain: NetworkChain) {
    ...
    try {
      ...
      val callingPrincipal = caller.get()?.service
        ?: caller.get()?.user?.let { "<user>" }
        ?: "unknown"

      val additionalTagValues = getAdditionalTagValues(downstreamTime).toTypedArray()
      
      metric
        .labels(actionName, callingPrincipal, statusCode, *additionalTagValues)
        .observe(exclusiveMillis)
    }
  }
  ...
  open class Factory @Inject constructor(
    ...
    override fun create(action: Action): NetworkInterceptor {
      return ExclusiveTimingInterceptor(excludedTime, requestDurationHistogram, caller, action.name, ::getAdditionalTagValues)
    }
  }
  ...
  class Module<T: Factory>(private var factoryClass: KClass<T>) : KAbstractModule() {    
    override fun configure() {
      ...
      bind(object : TypeLiteral<ThreadLocal<ExcludedTime>>() {})
        .toInstance(ThreadLocal.withInitial { ExcludedTime() })
    }
...
class ExcludedTime @JvmOverloads constructor(
  private var time: Duration = Duration.ZERO,
) {
  /**
   * Any additional data that you want to be available to your [ExclusiveTimingInterceptor.Factory].
   * This can be useful for creating additional metric tags.
   */
