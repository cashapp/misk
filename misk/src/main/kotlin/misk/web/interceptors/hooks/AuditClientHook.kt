...
/**
 * This hook is not installed by default but is best installed in the module which binds the implementation of the
 * [AuditClient].
 *
 * ```kotlin
...
private constructor(
  ...
  @Singleton
  class Factory @Inject constructor(private val auditClient: AuditClient) : RequestResponseHook.Factory {
    override fun create(action: Action): RequestResponseHook? {
      val annotation = action.function.findAnnotation<AuditRequestResponse>() ?: return null
  ...
  override fun handle(
    ...
    auditClient.logEvent(
      ...
      richDescription =
        if (annotation.richDescription.isBlank()) {
          description
        } else {
          annotation.richDescription + " " + description
        },
