...
@Singleton
internal class ThreadPoolMetrics
@Inject
internal constructor(metrics: Metrics, private val threadPool: MeasuredThreadPool) {
  val utilization =
    metrics.gauge(
  ...
  val busyThreads = metrics.gauge("jetty_thread_pool_busy", "current number of busy threads")

  val size = metrics.gauge("jetty_thread_pool_size", "current number of threads in the jetty thread pool")

  val queuedJobs = metrics.gauge("jetty_thread_pool_queued_jobs", "number of jobs queued in the jetty thread pool")
  ...
  private fun ratio(numerator: Double, denominator: Double) =
    if (denominator.isNaN() || denominator.isInfinite() || denominator == 0.0) Double.NaN else numerator / denominator
}
