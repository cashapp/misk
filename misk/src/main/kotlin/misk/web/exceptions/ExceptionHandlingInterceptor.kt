...
/**
 * Converts and logs application and component level dispatch exceptions into the appropriate response format. Allows
 * application and component code to control how exceptions are represented to clients; for example by setting the
 * status code appropriately, or by returning a specialized response format specific to the error. Components can
 * control how exceptions are mapped by installing [ExceptionMapper] via the [ExceptionMapperModule]
 *
...
private constructor(
  ...
  /**
   * Borrow behavior from [GrpcFeatureBinding] to send a gRPC error with an HTTP 200 status code. This is weird but it's
   * how gRPC clients work.
   *
   * One thing to note is for our metrics we want to pretend that the HTTP code is what we sent. Otherwise gRPC requests
   * that crashed and yielded an HTTP 200 code will confuse operators.
   */
  private fun sendGrpcFailure(httpCall: HttpCall, httpStatus: Int, response: GrpcErrorResponse) {
    ...
    httpCall.setResponseTrailer("grpc-status", response.status.code.toString())
    ...
    httpCall.takeResponseBody()?.use { responseBody: BufferedSink ->
      GrpcMessageSink(
        ...
        .use { messageSink -> messageSink.write(ByteString.EMPTY) }
    }
  }
  ...
  private fun unwrappedToResponse(th: Throwable, suppressLog: Boolean, mdcTags: Set<Tag>): Response<*> =
    ...
    mapperResolver.mapperFor(th)?.let {
      if (!suppressLog && it.isError(th)) {
        log.log(level = it.loggingLevel(th), th = th, tags = mdcTags.toTypedArray()) {
