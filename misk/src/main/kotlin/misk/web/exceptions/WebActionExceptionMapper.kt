...
/**
 * Maps [WebActionException]s into the appropriate status code. [WebActionException]s' response bodies are always
 * returned to the caller.
 */
internal class WebActionExceptionMapper @Inject internal constructor(val config: ActionExceptionLogLevelConfig) :
  ...
  override fun loggingLevel(th: WebActionException) =
    if (th.isClientError) config.client_error_level else config.server_error_level
  ...
  private companion object {
    ...
    val UNAUTHENTICATED_RESPONSE =
      Response("unauthenticated".toResponseBody(), HEADERS, HttpURLConnection.HTTP_UNAUTHORIZED)

    val UNAUTHORIZED_RESPONSE = Response("unauthorized".toResponseBody(), HEADERS, HttpURLConnection.HTTP_FORBIDDEN)
  }
}
