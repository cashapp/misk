...
fun WebAction.asChain(
  ...
  val callFunctionInterceptor =
    object : ApplicationInterceptor {
      override fun intercept(chain: Chain): Any {
        ...
        return if (!function.isSuspend) {
          ...
          runBlocking(context) {
            ...
            val grpcMessageSourceChannel =
              argsMap.values.mapNotNull { it as? GrpcMessageSourceChannel<*> }.singleOrNull()
            val grpcMessageSinkChannel = argsMap.values.mapNotNull { it as? GrpcMessageSinkChannel<*> }.singleOrNull()
            val responseSinkChannel = argsMap.values.mapNotNull { it as? ResponseSinkChannel<*> }.singleOrNull()
            ...
            try {
              (function as? FunctionWithOverrides)?.callSuspendBy(argsMap) ?: function.callSuspendBy(argsMap)
            } finally {
