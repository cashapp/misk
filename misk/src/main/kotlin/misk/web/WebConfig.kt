...
constructor(
  ...
  /**
   * If >= 0, By default, use a dedicated jetty thread pool for health checking. With health_dedicated_jetty_instance =
   * true, use a dedicated jetty instance.
   *
   * A dedicated thread pool ensures that health checks are not queued or rejected when the service is saturated and
   * queueing requests. If health checks are rejected and/or queued, the health checks may fail and k8s will kill the
   * container, even though it might be perfectly healthy. This can cause cascading failures by sending more requests to
   * other containers, resulting in longer queues and more health checks failures.
   */
   ...
   *
   * A dedicated jetty instance ensures the service readiness and liveness endpoints remain available during graceful
   * shutdown to prevent the container from being forcefully terminated and graceful failover from readiness check
   * failures via HTTP 503.
   *
   * A separate jetty instance allows the primary instance to allow in flight connections to complete before shutting
   * down and then shut down supporting services orderly until all are stopped and the health instance can shut down
   * safely.
   */
  @property:ExperimentalMiskApi val health_dedicated_jetty_instance: Boolean = false,

  /**
   * @ExperimentalMiskApi - this feature is still being verified in production.
   *
   * health_port must be >= 0, health_dedicated_jetty_instance must be true
   *
   * An interceptor which delays service shutdown for in flight requests to complete and for no new incoming requests to
   * have occurred during the idle time. Once shutdown has commenced, incoming requests will be redirected to healthy
   * pods by responding with 503 or the status code specified.
   *
   * This improves the ability for services to complete in-flight requests and safely terminate the underlying
   * connection.
   */
  @property:ExperimentalMiskApi val graceful_shutdown_config: GracefulShutdownConfig? = null,
  ...
  /**
   * Configuration to enable Jetty to listen for traffic on a unix domain socket being proxied through a sidecar (like
   * Envoy).
   */
   ...
   *
   * If 0, no queueing is used and requests are directly handed off to the thread pool. If a thread is not available
   * (i.e max threads in use) the request is rejected. Unfortunately Jetty rejects requests by closing the socket
   * instead of returning a 429. This can lead to confusing EOFExceptions for the client.
   */
  ...
  val concurrency_limiter: ConcurrencyLimiterConfig? =
  ...
  /**
   * The number of milliseconds a connection can be idling before commencing service shutdown. If zero, it is never
   * closed and may cause ungraceful shutdown.
   *
   * Note: There is an underlying strategy to determine the default shutdown idle timeout. Use this value only when
   * necessary.
   */
   ...
   *
   * Ensure that [readiness_refresh_interval_ms] + "readiness latency" is less than [readiness_max_age_ms] or readiness
   * will fail."
   */
  ...
  /**
   * Configurations to enable Jetty to listen for traffic on a unix domain socket being proxied through a sidecar (e.g.
   * envoy, istio)
   */
  val unix_domain_sockets: List<WebUnixDomainSocketConfig>? = null,

  /**
   * Config used by client and server interceptors installed by DeadlinePropagationModule Only applies if
   * DeadlinePropagationModule is installed, ignored otherwise
   */
   ...
   * https://jetty.org/docs/jetty/10/operations-guide/modules/standard.html#http2
   */
...
constructor(
  /**
   * The Unix Domain Socket to listen on. Will attempt to use the JEP-380 connector when supported (using Java 16+ and
   * file path)
   */
  val path: String,
...
constructor(
  ...
  val allowedHeaders: Array<String> = arrayOf("X-Requested-With", "Content-Type", "Accept", "Origin"),
...
constructor(
  ...
  /**
   * The level of log when concurrency shedding. Same as concurrency_limiter_log_level default for backwards
   * compatibility.
   */
  val log_level: Level = Level.ERROR,
)

data class GracefulShutdownConfig
@JvmOverloads
constructor(
  ...
  /**
   * The maximum amount of time to wait for no in-flight requests and the idle timeout to expire. If <= 0 the service
   * will not limit the amount of time to wait for idle, however it might be forcibly killed by an external source, like
   * kubernetes.
   */
  val max_graceful_wait: Long = 20_000,

  /**
   * The status code to use for rejecting incoming requests after shutdown has begun. If <= 0 the service will not
   * reject incoming requests and wait until new requests have processed and the idle_timeout is reached.
   */
 ...
 *
 * Request deadlines help prevent cascading failures by tracking how much time remains for a request chain to complete.
 * Deadlines are propagated through service boundaries and can be enforced at various points in the request lifecycle.
 *
 ...
 */
data class RequestDeadlinesConfig
@JvmOverloads
constructor(
  /**
...
enum class RequestDeadlineMode {
  /**
   * Only collect metrics on deadline violations without propagation or enforcement. Use this for safe rollout and
   * observability without behavioral changes.
   */
  METRICS_ONLY,

  /**
   * Propagate deadlines to downstream services but don't enforce them locally. Useful for establishing deadline chains
   * without risking service availability
   */
  PROPAGATE_ONLY,
