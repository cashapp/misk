...
/**
 * Binds a Kotlin class to a [FormData]. All fields are supplied to the class constructor which must be either named
 * directly or annotated [FormField].
 */
internal class FormAdapter<T : Any>
private constructor(private val constructor: KFunction<T>, private val fields: List<Field>) {
  internal fun fromFormData(formData: FormData): T {
    ...
    for (field in fields) {
      ...
      requireRequest(values.isNotEmpty() || field.nullable) { "${field.name} is a required value" }
  ...
  companion object {
    ...
    fun <T : Any> create(kClass: KClass<T>, stringConverterFactories: List<StringConverter.Factory>): FormAdapter<T>? {
    ...
    private fun KParameter.toField(stringConverterFactories: List<StringConverter.Factory>): Field {
      ...
      return Field(
        ...
        isList,
        converterFor(if (isList) type.arguments.first().type!! else type, stringConverterFactories),
      )
    }
  }
}
