...
@Singleton
internal class GracefulShutdownService @Inject constructor(webConfig: WebConfig, clock: Clock) : AbstractIdleService() {
  @OptIn(ExperimentalMiskApi::class)
  private val gracefulShutdownConfig = webConfig.graceful_shutdown_config ?: GracefulShutdownConfig(disabled = true)
  ...
  private val maxShutdownWait: Duration =
    if (gracefulShutdownConfig.max_graceful_wait <= 0) {
      Duration.INFINITE
    } else {
  ...
  /**
   * Tracker of in-flight requests from the interceptor. We will only shutdown when this reaches 0, along with the idle
   * timeout.
   */
  ...
  /**
   * NoOp until shutdown then GracefulReject. Even if we shut down we still want to the interceptor to continue
   * responding with 503.
   */
  ...
  /**
   * Report an in-flight request, this resets the idle timeout wait and will delay shutdown until reportRequestComplete
   * is called and total in-flight requests reach 0.
   */
  ...
  /**
   * Report an in-flight request has completed, if the in-flight requests count reaches 0 the service can proceed with
   * shutdown once incoming idle timeout is reached.
   */
  ...
  override fun shutDown() {
    ...
    while (inFlightRequestsTracker.get() != 0L || idleFor < idleTimeout) {
    ...
    withTags(
      ...
      logger.info { "Graceful Shutdown proceeding after $shutdownWait " + "with $inFlightRequests in-flight requests" }
    }
  }
  ...
  companion object {
    ...
    private fun Long.toNearest(multiple: Int): Long = multiple * (toDouble() / multiple).roundToLong()
