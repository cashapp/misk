 ...
 *
 * A binding can claim multiple features. [beforeCall] is invoked if any features are claimed, and [afterCall] is only
 * invoked if the return value is claimed.
 *
 * ## Creating Bindings
 *
 * Each factory is executed once for each endpoint at service startup time. Factories should interrogate the function
 * and claim whichever features that intend to bind. This happens early so Misk can validate that every feature is bound
 * exactly once.
 *
 * Factories that return null must make no claims. If no claims are made then no binding is executed.
 *
 * Misk will validate that every feature is claimed exactly once. If a feature is unclaimed that is a fatal error and
 * the service will not be started.
 *
 * ## Binding Execution
 *
 * Once a functions bindings have been created and validated, each binding will be executed once before and once after
 * every each HTTP call. In this method it must bind the features it claimed by providing parameters, reading the
 * request body, writing the response body, or taking the return value.
 */
