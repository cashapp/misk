...
@Singleton
class FakeCallerAuthenticator @Inject constructor(private val currentHttpCall: ActionScoped<HttpCall>) :
  MiskCallerAuthenticator {
  @Inject(optional = true) @DevelopmentOnly var developmentCaller: MiskCaller? = null
  ...
  override fun getAuthenticatedCaller(): MiskCaller? {
    ...
    return when {
      !(user == null && service == null) ->
        MiskCaller(service = service, user = user, capabilities = capabilities ?: setOf())
      development != null -> MiskCaller(development.service, development.user, capabilities = development.capabilities)
      else -> null
    }
  }
...
@Qualifier @Target(AnnotationTarget.FIELD, AnnotationTarget.FUNCTION) annotation class DevelopmentOnly
