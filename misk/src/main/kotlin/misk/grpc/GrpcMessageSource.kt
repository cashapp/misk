 ...
 *
 * This is derived from Wire's GrpcMessageSource.kt. https://github.com/square/wire/search?q=GrpcMessageSource&type=Code
 *
...
constructor(
  ...
  override fun read(): T? {
    ...
    val messageDecoding: GrpcDecoder =
      when {
        compressedFlag.toInt() == 0 -> GrpcDecoder.IdentityGrpcDecoder
        compressedFlag.toInt() == 1 -> {
          grpcEncoding?.toGrpcDecoding()
            ?: throw ProtocolException("message is encoded but message-encoding header was omitted")
        }
