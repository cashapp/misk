...
class HttpClientModule @JvmOverloads constructor(private val name: String, private val annotation: Annotation? = null) :
  KAbstractModule() {
  override fun configure() {
    val httpClientKey =
      if (annotation == null) Key.get(OkHttpClient::class.java) else Key.get(OkHttpClient::class.java, annotation)
    ...
    bind(httpClientKey).toProvider(HttpClientProvider(name)).asSingleton()
    ...
    requestInjection(
      object : Any() {
        @Inject
        fun injectHttpClientsConfig(httpClientsConfig: HttpClientsConfig) {
  ...
  private class ProtoMessageHttpClientProvider(
    ...
    override fun get(): ProtoMessageHttpClient {
      ...
      return ProtoMessageHttpClient(httpClientConfigUrlProvider.getUrl(endpointConfig), moshi, httpClient)
    }
  }
}
