...
internal class MiskCallerTest {
  ...
  @Test
  fun userNameIsRedactedFromToString() {
    assertThat("$testUser").doesNotContain(testUser.user)
  }

  @Test
  fun serviceNameIsNotRedactedFromToString() {
...
internal class MiskCallerTestModule : KAbstractModule() {
  override fun configure() {
    bind<MiskCaller>()
      .annotatedWith<TestUser>()
      .toInstance(MiskCaller(user = "Test user", capabilities = setOf("testing")))
    bind<MiskCaller>().annotatedWith<TestService>().toInstance(MiskCaller(service = "Test service"))
  }
}

@Qualifier @Target(AnnotationTarget.FIELD, AnnotationTarget.FUNCTION) annotation class TestUser

@Qualifier @Target(AnnotationTarget.FIELD, AnnotationTarget.FUNCTION) annotation class TestService
