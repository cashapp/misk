...
class ValidWebEntryTest {
  ...
  @Test fun `fails on bad url path prefix missing trailing slash`() {
    ...
    assertEquals("Invalid or unexpected url path prefix: '/test-path'. Must start with 'http' OR start and end with '/'.", errMsg.message)
  }
  ...
  @Test fun `fails on bad url path prefix not starting with http if absolute path`() {
    val errMsg = assertFailsWith<IllegalArgumentException> {
      class TestWebEntry : ValidWebEntry(slug = "test-slug", url_path_prefix = "www.cash.app/test-path")
      TestWebEntry()
    }
    assertEquals("Invalid or unexpected url path prefix: 'www.cash.app/test-path'. Must start with 'http' OR start and end with '/'.", errMsg.message)
  }
  ...
  @Test fun `fails on invalid slug characters`() {
    ...
    assertEquals("Slug contains invalid characters. Can only contain characters in ranges [a-z], [0-9] or '-'.", errMsg.message)
  }
}
