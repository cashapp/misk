...
class AuthenticationTest {
  ...
  @Test
  fun customServiceAccess_unauthenticated() {
    assertThat(executeRequest(path = "/custom_service_access")).isEqualTo("unauthenticated")
  }

  @Test
  fun customServiceAccess_unauthorized() {
    assertThat(executeRequest(path = "/custom_service_access", service = "customers")).isEqualTo("unauthorized")
  }
  ...
  @Test
  fun customCapabilityAccess_unauthenticated() {
    assertThat(executeRequest(path = "/custom_capability_access")).isEqualTo("unauthenticated")
  }

  @Test
  fun customCapabilityAccess_unauthorized() {
  ...
  fun customCapabilityAccess_authorized(cap: String) {
    ...
    assertThat(executeRequest(path = "/custom_capability_access", user = "sandy", capabilities = cap))
      .isEqualTo("$caller authorized with custom capability")
  }

  @Test
  fun testEmptyAuthenticatedWithUser() {
    val caller = MiskCaller(user = "sandy", capabilities = setOf("nothingfancy"))
    assertThat(executeRequest(path = "/empty_authorized_access", user = "sandy", capabilities = "nothingfancy"))
      .isEqualTo("unauthorized")
  }

  @ParameterizedTest
  @ValueSource(
    strings = ["widgeteer", "web-proxy", "access-proxy"]
  ) // web-proxy and access-proxy are both ExcludeFromAllowAnyService
  fun testEmptyAuthenticatedWithService(service: String) {
    val caller = MiskCaller(service = service)
    assertThat(executeRequest(path = "/empty_authorized_access", service = service)).isEqualTo("unauthorized")
  }

  @Test
  fun testEmptyAuthenticatedUnauthenticated() {
    assertThat(executeRequest(path = "/empty_authorized_access")).isEqualTo("unauthenticated")
  }

  @Test
  fun testEmptyAuthenticatedPlusCustomUnauthenticated() {
    assertThat(executeRequest(path = "/empty_authorized_and_custom_capability_access")).isEqualTo("unauthenticated")
  }

  @Test
  fun testEmptyAuthenticatedPlusCustomWithWrongCapability() {
    assertThat(
        executeRequest(
  ...
  @Test
  fun testEmptyAuthenticatedPlusCustomAsService() {
    assertThat(executeRequest(path = "/empty_authorized_and_custom_capability_access", service = "widgeteer"))
      .isEqualTo("unauthorized")
  }

  @Test
  fun checkAccessInterceptorFactoryLogs() {
    // We don't need to execute a request to check these logs, they're logged on installation of
    // the interceptor on the endpoint.

    assertThat(logCollector.takeEvents(AccessInterceptor::class).map { it.message })
  ...
  @Test
  fun testEmptyAuthenticatedPlusCustomWithRightCapability() {
    val caller = MiskCaller(user = "sandy", capabilities = setOf("admin"))
    assertThat(
        executeRequest(path = "/empty_authorized_and_custom_capability_access", user = "sandy", capabilities = "admin")
      )
  ...
  @ParameterizedTest
  @ValueSource(
    strings = ["web-proxy", "access-proxy"]
  ) // web-proxy and access-proxy are both ExcludeFromAllowAnyService
  fun testAllowAnyServiceWithExcludedServices(service: String) {
    assertThat(executeRequest(path = "/allow_any_service_access", service = service)).isEqualTo("unauthorized")
  }

  @Test
  fun testAllowAnyServiceNotExcluded() {
    val service = "widgeteer"
    val caller = MiskCaller(service = service)
    assertThat(executeRequest(path = "/allow_any_service_access", service = service))
      .isEqualTo("$caller authorized as any service")
  }

  @Test
  fun testAllowAnyServiceWithUser() {
    assertThat(executeRequest(path = "/allow_any_service_access", user = "sandy")).isEqualTo("unauthorized")
  }

  @Test
  fun testAllowAnyServiceWithAuthenticatedAsExcludedButExplicityAllowedService() {
    val service = "web-proxy"
    val caller = MiskCaller(service = service)
    assertThat(executeRequest(path = "/allow_any_service_plus_authenticated", service = service))
      .isEqualTo("$caller authorized as any service")
  }

  @Test
  fun testAllowAnyServiceWithAuthenticatedAsExcludedService() {
  ...
  @Test
  fun testAllowAnyServiceWithAuthenticatedAsAllowedUser() {
    val caller = MiskCaller(user = "sandy", capabilities = setOf("admin"))
    assertThat(
        executeRequest(
  ...
  @Test
  fun testAllowAnyServiceWithAuthenticatedAsNotAllowedUser() {
  ...
  @Test
  fun testAllowAnyUser() {
    val caller = MiskCaller(user = "sandy")
    assertThat(executeRequest(path = "/allow_any_user_access", user = caller.user))
      .isEqualTo("$caller authorized as any user")
  }

  @Test
  fun testAllowAnyUserDenyServiceOnly() {
    val caller = MiskCaller(service = "test")
    assertThat(executeRequest(path = "/allow_any_user_access", user = caller.user)).isEqualTo("unauthenticated")
  }

  private class MixesUnauthenticatedWithOtherAnnotations @Inject constructor() : WebAction {
    @Get("/oops") @Unauthenticated @Authenticated fun get() {}
  }
  ...
  fun `stacking @Authenticated with other access annotations is an error`() {
    ...
    assertThat(executeRequest(path = "/auth-and-custom-capability", service = unauthService.service))
      .isEqualTo("unauthorized")

    val authService = MiskCaller(service = "dingo")
    assertThat(executeRequest(path = "/auth-and-custom-capability", service = authService.service))
