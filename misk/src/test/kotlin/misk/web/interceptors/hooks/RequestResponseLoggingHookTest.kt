...
internal class RequestResponseLoggingHookTest {
  ...
  fun rateLimiting_includesBody() {
    ...
    assertThat(logCollector.takeMessages(RequestLoggingInterceptor::class))
      .containsExactly("RateLimitingIncludesBodyRequestLoggingAction principal=caller time=100.0 ms code=200")
  }
  ...
  fun rateLimiting_excludesBody() {
    ...
    assertThat(logCollector.takeMessages(RequestLoggingInterceptor::class))
      .containsExactly("RateLimitingRequestLoggingAction principal=caller time=100.0 ms code=200")
  ...
  fun noRateLimiting() {
    ...
    for (i in 0..10) {
      ...
      assertThat(messages).containsExactly("NoRateLimitingRequestLoggingAction principal=caller time=100.0 ms code=200")
    }
  }
  ...
  fun noNonErrorRequestLogging5xxAtError() {
    assertThat(invoke("/call/errorOnlyRequestLoggingAction/fail", "caller").code).isEqualTo(500)
    assertThat(invoke("/call/errorOnlyRequestLoggingAction/hello", "caller").isSuccessful).isTrue()
    val event =
      logCollector.takeEvents(RequestLoggingInterceptor::class, Level.ERROR, consumeUnmatchedLogs = false).single()
    assertThat(event.message).isEqualTo("errorOnlyRequestLoggingAction principal=caller time=100.0 ms failed")
  ...
  fun noNonErrorRequestLogging4xxAtWarn() {
    assertThat(invoke("/call/errorOnlyRequestLoggingAction/4xx", "caller").code).isEqualTo(404)

    assertThat(logCollector.takeMessages(RequestLoggingInterceptor::class, Level.ERROR, consumeUnmatchedLogs = false))
      .isEmpty()
    val event =
  ...
  fun noNonErrorRequestLoggingWrapped() {
    assertThat(invoke("/call/errorOnlyRequestLoggingAction/wrapped", "caller").code).isEqualTo(404)
  ...
  fun noHeadersByDefault() {
    assertThat(
        webTestClient
          .call("/call/withoutHeaders") { post("hello".toRequestBody(MediaTypes.APPLICATION_JSON_MEDIA_TYPE)) }
          .response
          .isSuccessful
      )
      .isTrue()
  ...
  fun `requestResponseBodyTransformer applies all relevant transformers`() {
    ...
    assertThat(messages[0]).contains("response=echo: Quokka (the happiest, most bestest animal) responseHeaders={}")
  }
  ...
  fun `requestResponseBodyTransformer contains explosions`() {
    ...
    assertThat(allLogs)
      .contains(
        "RequestLoggingTransformer of type [misk.web.interceptors.hooks.ThrowingTransformer] failed to transform: request=Oppenheimer-the-bestest response=echo: Oppenheimer-the-bestest"
      )

    // Regular request logging still happened, and [DontSayJerkTransformer] still ran
    val interceptorLogs =
      events.filter { it.loggerName == RequestLoggingInterceptor::class.qualifiedName }.map { it.message }
    assertEquals(1, interceptorLogs.size)
    assertThat(interceptorLogs[0])
      .contains(
        "LogEverythingAction principal=caller time=100.0 ms code=200 " +
          "request=Oppenheimer-the-bestest " +
          "requestHeaders={accept-encoding=[gzip], connection=[keep-alive]"
      )
    assertThat(interceptorLogs[0]).contains("response=echo: Oppenheimer-the-most bestest responseHeaders={}")
  }
  ...
  class TestModule : KAbstractModule() {
    override fun configure() {
      ...
      multibind<RequestLoggingConfig>()
        .toInstance(
          RequestLoggingConfig(
            mapOf(
              "ConfigOverrideAction" to
                ActionLoggingConfig(
                  ratePerSecond = 0,
      ...
      "wrapped" ->
        throw InvocationTargetException(
          UncheckedExecutionException(
            InvocationTargetException(UncheckedExecutionException(NotFoundException(message)))
          )
        )
      else -> "echo: $message"
    }
...
internal class QuokkaLovingTransformer @Inject constructor() : RequestLoggingTransformer {
  override fun transform(requestResponseBody: RequestResponseBody?): RequestResponseBody? {
    ...
    return requestResponseBody?.copy(
      response = responseString?.replace("Quokka", "Quokka (the happiest, bestest animal)")
    )
  }
}
