...
internal class SmartTagsExceptionHandlingInterceptorTest {
  ...
  fun serviceNotUsingSmartTagsShouldLogWithMdcContext() {
    ...
    val logs = logCollector.takeEvents(ServiceNotUsingSmartTagsShouldLogWithHandRolledMdcContext::class)
  ...
  class ServiceNotUsingSmartTagsShouldLogWithHandRolledMdcContext @Inject constructor() : WebAction {
    @Get(URL)
    @Unauthenticated
    ...
    fun call(): String {
      ...
      withTags(Tag("withTagsTag1", "withTagsValue1"), "withTagsTag2" to "withTagsValue2") {
  ...
  fun serviceNotUsingSmartTagsShouldLogExceptionWithoutMdcContext() {
    ...
    val serviceLogs =
      logCollector.takeEvents(
        ServiceUsingLegacyTaggedLoggerShouldLogExceptionWithoutMdcContext::class,
  ...
  class ServiceUsingLegacyTaggedLoggerShouldLogExceptionWithoutMdcContext @Inject constructor() : WebAction {
    ...
    data class LegacyTaggedLogger(val kLogger: KLogger, private val tags: Set<Tag> = emptySet()) {
  ...
  class LogMDCContextTestAction @Inject constructor() : WebAction {
    ...
    @ResponseContentType(MediaTypes.APPLICATION_JSON)
    fun call(): String {
      withSmartTags("testTag" to "SpecialTagValue123", "secondTag" to "SecondTagValue123") {
        logger.info { "Tester" }
  ...
  fun shouldHandleConcurrentThreadsUsingSmartTags() {
    ...
    val url = jettyService.httpServerUrl.newBuilder().encodedPath(NestedSmartTagsBothSucceed.URL).build()
    ...
    val callService: (String) -> Callable<String> = { identifier ->
      Callable {
        val request =
          okhttp3.Request.Builder().url(url).get().addHeader(NestedSmartTagsBothSucceed.IDENTIFIER_HEADER, identifier)

        val response = OkHttpClient().newCall(request.build()).execute()

        val responseText = response.body!!.source().use { source -> source.readUtf8Line() }
    ...
    val results =
      List(80) { index ->
        ...
        .mapIndexed { index, future -> println("Getting: $index with value ${future.get()}") } // Executing thread name
    ...
    val serviceLogsOrdered =
      serviceLogs
        .filter { it.message == "Non nested log message" }
    ...
    serviceLogsOrdered
      .groupBy { it.mdcPropertyMap[NestedSmartTagsBothSucceed.EXECUTION_IDENTIFIER] }
      .forEach { (_, logs) ->
        ...
        assertThat(logs[1].mdcPropertyMap)
          .doesNotContainKey(
            "testTag"
          ) // Context not shared into a newly spawned thread inside a tagged logger asContext
  ...
  class NestedSmartTagsBothSucceed @Inject constructor() : WebAction {
    ...
    fun call(@RequestHeaders headers: Headers): String {
      val result =
        withSmartTags("testTag" to "SpecialTagValue123", EXECUTION_IDENTIFIER to headers[IDENTIFIER_HEADER]) {
          ...
          Thread { result = AnotherClass().functionWithNestedSmartTags(headers[IDENTIFIER_HEADER]) }
            .also { it.start() }
            .join(1000)
    ...
    class AnotherClass() {
      fun functionWithNestedSmartTags(parentTag: String?): String {
        return withSmartTags(EXECUTION_IDENTIFIER to parentTag, "testTagNested" to "NestedTagValue123") {
  ...
  fun shouldHandleMixedUseOfWithTagsAndWithSmartTags() {
    ...
    val serviceLogs =
      logCollector.takeEvents(NestedWithTagsAndWithSmartTagsThrowsException::class, consumeUnmatchedLogs = false)
    val miskExceptionLogs = logCollector.takeEvents(ExceptionHandlingInterceptor::class)
  ...
  class NestedWithTagsAndWithSmartTagsThrowsException @Inject constructor() : WebAction {
    ...
    private fun functionWithNestedSmartTags(): String {
      return withTags(
        ...
        includeTagsOnExceptionLogs = true,
      ) { // aka "withSmartTags"
        logger.info { "Nested log message with two mdc properties" }
        throw NestedSmartTagsException("Nested logger test exception")
      }
  ...
  fun shouldHandleMixedUseOfWithSmartTagsAndWithTags() {
    ...
    val serviceLogs =
      logCollector.takeEvents(NestedWithSmartTagsAndWithTagsThrowsException::class, consumeUnmatchedLogs = false)
    val miskExceptionLogs = logCollector.takeEvents(ExceptionHandlingInterceptor::class)
  ...
  fun shouldCorrectlyResetThreadLocalWhenExceptionCaughtAndNoneThrown() {
    ...
    val serviceLogs =
      logCollector.takeEvents(NestedLoggersOuterExceptionHandledNoneThrown::class, consumeUnmatchedLogs = false)
    val miskExceptionLogs = logCollector.takeEvents(ExceptionHandlingInterceptor::class)
  ...
  fun shouldLeaveSmartTagsInThreadLocalWhenExceptionCaughtAndPeekedThenRethrown() {
    ...
    val serviceLogs =
      logCollector.takeEvents(NestedLoggersExceptionCaughtAndPeekedThenRethrown::class, consumeUnmatchedLogs = false)
    val miskExceptionLogs = logCollector.takeEvents(ExceptionHandlingInterceptor::class)
  ...
  fun invoke(url: String, asService: String? = null): okhttp3.Response {
    val url = jettyService.httpServerUrl.newBuilder().encodedPath(url).build()
