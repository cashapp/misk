package com.squareup.testing.gcloud.storage
...
import misk.cloud.gcp.storage.InMemoryStorageRpc
import misk.cloud.gcp.storage.blobIds
import misk.hash.asByteString
...
internal class InMemoryStorageRpcTest {
  private val storageRpc = InMemoryStorageRpc()
  private val storage = StorageOptions.newBuilder()
      .setCredentials(NoCredentials.getInstance())
      .setServiceRpcFactory { _ -> storageRpc }
      .build()
      .service
  ...
  fun listFolders() {
    ...
    assertThat(roots.blobIds).containsExactlyInAnyOrder(
        BlobId.of("my_bucket", "notes", 1),
        BlobId.of("my_bucket", "images", 1),
        BlobId.of("my_bucket", "images.txt", 1))

    val subDir = storage.list("my_bucket", prefix("notes/"), currentDirectory())
    assertThat(subDir.blobIds).containsExactlyInAnyOrder(
        BlobId.of("my_bucket", "notes/misc", 1),
        BlobId.of("my_bucket", "notes/storage", 1),
        BlobId.of("my_bucket", "notes/top.txt", 1),
        BlobId.of("my_bucket", "notes/top2.txt", 1))
  ...
  fun createEmpty() {
    ...
    val crc32 = Hashing.crc32c().hashBytes(ByteArray(0)).asByteString()
    ...
    assertThat(result.crc32c).isEqualTo(crc32.base64Url())
  ...
  fun progressiveDownload() {
    ...
    val download = ByteArray(blob.size.toInt())
    val buffer = ByteBuffer.wrap(download)
    blob.reader().use {
          // Use the minimum chunk size, which is smaller than the buffer size, to
