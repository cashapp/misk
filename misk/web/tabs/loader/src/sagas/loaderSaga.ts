...
function * handleGetAllComponents (action: IAction<IActionType, { adminTabs: IMiskAdminTabs }>) {
  try {
    let adminTabs: any = {}
    try {
...
function * watchLoaderSagas () {
  yield all([
    ...
    takeLatest(LOADER.GET_ALL_COMPONENTS, handleGetAllComponents),
    takeLatest(LOADER.GET_ALL_TABS, handleGetAllTabs),
    takeLatest(LOADER.GET_ALL_AND_TABS, handleGetAllAndTabs),
  ])
}
