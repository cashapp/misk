...
export const dispatchConfig = {
  failure: (error: any) =>
    createAction(CONFIG.FAILURE, { ...error, loading: false, success: false }),
  getAll: (url: string) =>
    createAction(CONFIG.GET_ALL, {
      url,
      loading: true,
      success: false,
      error: null
...
export function* watchConfigSagas() {
  yield all([takeLatest(CONFIG.GET_ALL, handleGetAll)])
}
