  ...
  if (metadata) {
    ...
    if (typescriptType === TypescriptBaseTypes.boolean) {
      ...
      if (
        ...
        const fieldGroup = (
          child: string,
          parentServerType: string,
          parentTypescriptType: string
        ) => {
          const {
          ...
          // Don't show intermediary wrapper parent nodes with redundant duplicate labels
          if (
            BaseFieldTypes.hasOwnProperty(childServerType) ||
            noParentGroupLabel ||
            (childServerType && childServerType.startsWith("Enum"))
          ) {
            return (
              <div key={child}>
                <FormFieldBuilderContainer
            ...
            <div>
              ...
              >
                <Card css={css(cssCard)}>
                  {idChildren.map((child: string) =>
                    fieldGroup(child, serverType, typescriptType)
                  )}
                </Card>
              </EditRawInput>
            </div>
