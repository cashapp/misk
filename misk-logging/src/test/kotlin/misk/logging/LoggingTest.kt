...
class LoggingTest {
  ...
  fun loggingWithTags() {
    ...
    assertThat(taggedEvents[0])
      .satisfies({
        assertThat(it.level).isEqualTo(Level.INFO)
        assertThat(it.message).isEqualTo("tagged info")
        assertThat(it.throwableProxy).isNull()
        assertThat(it.mdcPropertyMap).containsAllEntriesOf(mutableMapOf("user-id" to "blerb", "alias-id" to "d6F1EF53"))
      })
    assertThat(taggedEvents[1])
      .satisfies({
        assertThat(it.level).isEqualTo(Level.ERROR)
        ...
        assertThat(it.mdcPropertyMap).containsAllEntriesOf(mutableMapOf("sample-size" to "200"))
      })
    ...
    assertThat(defaultTaggedEvents[0])
      .satisfies({
        assertThat(it.level).isEqualTo(Level.WARN)
        assertThat(it.message).isEqualTo("inherited warn")
        assertThat(it.throwableProxy).isNull()
        assertThat(it.mdcPropertyMap)
          .containsAllEntriesOf(mutableMapOf("user-id" to "inherited-external", "context-id" to "overridden"))
      })
    assertThat(defaultTaggedEvents[1])
      .satisfies({
        assertThat(it.level).isEqualTo(Level.INFO)
        assertThat(it.message).isEqualTo("uses default tags")
        assertThat(it.mdcPropertyMap)
          .containsAllEntriesOf(mutableMapOf("user-id" to "inherited-external", "context-id" to "111111"))
      })
  }
}
