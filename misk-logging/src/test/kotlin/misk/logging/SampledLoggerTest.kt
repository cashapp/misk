...
class SampledLoggerTest {
  @MiskTestModule val testModule = Modules.combine(MiskTestingServiceModule(), LogCollectorModule())
  ...
  fun rateLimitedLogger() {
    ...
    assertThat(events[0])
      .satisfies({
        ...
        assertThat(it.mdcPropertyMap).containsEntry("user-id", "blah1")
      })
    assertThat(events[1])
      .satisfies({
        ...
        assertThat(it.mdcPropertyMap).containsEntry("user-id", "blah2")
      })
    assertThat(events[2])
      .satisfies({
        ...
        assertThat(it.throwableProxy).isNull()
        assertThat(it.mdcPropertyMap).containsEntry("user-id", "blah3")
      })

    // Sampled logger is rate-limited
    sampledLogger.info("user-id" to "blerb1") { "sampled test 1" }
    sampledLogger.error(NullPointerException("failed!"), "user-id" to "blerb2", "context-id" to "111111") {
      "sampled test 2"
    }
    ...
    assertThat(sampledEvents[0])
      .satisfies({
        assertThat(it.level).isEqualTo(Level.INFO)
        assertThat(it.message).isEqualTo("sampled test 1")
        assertThat(it.throwableProxy).isNull()
        assertThat(it.mdcPropertyMap).containsAllEntriesOf(mutableMapOf("user-id" to "blerb1"))
      })
    assertThat(sampledEvents[1])
      .satisfies({
        assertThat(it.level).isEqualTo(Level.ERROR)
        ...
        assertThat(it.mdcPropertyMap)
          .containsAllEntriesOf(mutableMapOf("user-id" to "blerb2", "context-id" to "111111"))
      })
    ...
    assertThat(sampledEvents2[0])
      .satisfies({
        assertThat(it.level).isEqualTo(Level.WARN)
        assertThat(it.message).isEqualTo("sampled test 4")
        assertThat(it.throwableProxy).isNull()
        assertThat(it.mdcPropertyMap).containsAllEntriesOf(mutableMapOf("user-id" to "blerb4"))
      })
  }
}
