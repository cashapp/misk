...
interface Sampler {
  ...
  companion object {
    ...
    fun percentage(samplePercentage: Int): Sampler =
      PercentSampler(samplePercentage) { ThreadLocalRandom.current().nextInt(0, 100) }
    ...
    fun rateLimiting(ratePerSecond: Long): Sampler =
      RateLimitingSampler(RateLimiter.Factory(Ticker.systemTicker(), Sleeper.DEFAULT).create(ratePerSecond))
...
@JvmOverloads
constructor(val samplePercentage: () -> Int, val random: () -> Int = { ThreadLocalRandom.current().nextInt(0, 100) }) :
  Sampler {
...
class RateLimitingSampler(private val rateLimiter: RateLimiter) : Sampler {
  constructor(
    ratePerSecond: Long
  ) : this(RateLimiter.Factory(Ticker.systemTicker(), Sleeper.DEFAULT).create(ratePerSecond))
