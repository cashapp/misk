...
sealed class MappedKeyManager<KeyT> constructor(private val injector: Injector, private val keyClass: Class<KeyT>) {
  private val keys: HashMap<String, KeyT> = LinkedHashMap()

  internal operator fun set(name: String, k: KeyT) = keys.set(name, k)

  operator fun get(name: String): KeyT = keys.getOrPut(name) { getKeyInstance(name) }
  ...
  internal open fun getKeyInstance(name: String): KeyT = getNamedInstance(keyClass, name)
}
 ...
 * ```
 *
 * Note: [Aead] instances provided by this module are **envelope Aead** instances. This means that all data is encrypted
 * with an ephemeral data encryption key (DEK), which is then protected by a key-encryption key (KEK) and stored inline
 * with ciphertext. This effectively means that ciphertext will be a bit larger than the plaintext, and that migrating
 * keys (KEKs) should not require the re-encryption of stored data.
 */
@Singleton
 ...
 * ```
 *
 * Note that DeterministicAead objects do not provide secrecy to the same level as AEAD do, since multiple plaintexts
 * encrypted with the same key will produce identical ciphertext. This behavior is desirable when querying data via its
 * ciphertext (i.e. equality will hold), but an attacker can detect repeated plaintexts.
 */
@Singleton
class DeterministicAeadKeyManager @Inject internal constructor(injector: Injector) :
 ...
 */
@Singleton
class MacKeyManager @Inject internal constructor(injector: Injector) : MappedKeyManager<Mac>(injector, Mac::class.java)

/**
 * Holds a map of every key name to its corresponding [PublicKeySign] and [PublicKeyVerify] primitives.
 *
 * Users may use this class to obtain a [PublicKeySign] to sign data or [PublicKeyVerify] to verify the integrity of a
 * message dynamically:
 * ```
...
/**
 * Holds a map of every [HybridEncrypt] key name to its corresponding primitive listed in the configuration for this
 * app.
 *
 ...
 * ```
 *
 * Note: Hybrid encryption is intentionally divided to 2 separate key managers, [HybridEncryptKeyManager] and
 * [HybridDecryptKeyManager], so that the public portion of the keyset could be exported to other services. This
 * configuration helps achieve the goal of allowing some services to **encrypt only** and other services to both encrypt
 * and decrypt.
 */
@Singleton
class HybridEncryptKeyManager @Inject internal constructor(injector: Injector) :
  MappedKeyManager<HybridEncrypt>(injector, HybridEncrypt::class.java)

/**
 * Holds a map of every [HybridDecrypt] key name to its corresponding primitive listed in the configuration for this
 * app.
 *
 ...
 */
@Singleton
class HybridDecryptKeyManager @Inject internal constructor(injector: Injector) :
  MappedKeyManager<HybridDecrypt>(injector, HybridDecrypt::class.java)

class KeyNotFoundException @JvmOverloads constructor(message: String? = null, cause: Throwable? = null) :
 ...
 * ```
 *
 * Note: [StreamingAead] is useful when the data to be encrypted is too large to be processed in a single step. Typical
 * use cases include encryption of large files or encryption of live data streams
 */
@Singleton
class StreamingAeadKeyManager @Inject constructor(injector: Injector) :
 ...
 */
@Singleton
class PgpEncrypterManager @Inject constructor(injector: Injector) :
 ...
 */
@Singleton
class PgpDecrypterManager @Inject constructor(injector: Injector) :
