...
object FilesystemLoaderBackend : ResourceLoader.Backend() {
  ...
  override fun watch(path: String, resourceChangedListener: (address: String) -> Unit) {
    ...
    directory.register(watcher, watchedEventKinds.toTypedArray())

    watchedDirectoryThreads[directory] = startWatcherThread(directory)
  }

  private fun startWatcherThread(directory: Path): Thread {
    val thread =
      Thread(threadGroup) {
        try {
          while (true) {
            ...
            key
              .pollEvents()
              .firstOrNull { event -> event.kind() in watchedEventKinds }
              ?.let { event ->
