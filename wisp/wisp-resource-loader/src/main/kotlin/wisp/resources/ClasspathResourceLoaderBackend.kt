...
object ClasspathResourceLoaderBackend : ResourceLoader.Backend() {
  ...
  override fun list(path: String): List<String> {
    ...
    return result.filter { !it.endsWith(".class") }.map { "/$checkPath/$it" }.toList()
  }

  /**
   * Returns a string like `/tmp/foo.jar` from a URL string like `jar:file:/tmp/foo.jar!/META-INF/MANIFEST.MF`. This
   * strips the scheme prefix `jar:file:` and an optional path suffix like `!/META-INF/MANIFEST.MF`.
   */
   ...
   *
   * @param pathPrefix a string like `wisp/resources/` that ends in a slash. This will return the simple names of the
   *   files and directories that are direct descendants of this path.
   */
