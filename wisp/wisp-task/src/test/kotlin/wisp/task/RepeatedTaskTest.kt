...
internal class RepeatedTaskTest {
  private val repeatedTaskConfig = RepeatedTaskConfig(timeBetweenRunsMs = 100L)

  private val collectorRegistry = CollectorRegistry()
  private val prometheusRegistry = PrometheusMeterRegistry(PrometheusConfig.DEFAULT, collectorRegistry, Clock.SYSTEM)

  private fun durationTest(status: Status, result: String) {
    val repeatedTask =
      RepeatedTask(name = "taskName", meterRegistry = prometheusRegistry, repeatedTaskConfig = repeatedTaskConfig) {
        _: String,
        _: TaskConfig ->
        status
      }
  ...
  private fun resultMetricTest(status: Status, counterName: String) {
    ...
    val counter = prometheusRegistry.get(counterName).counter()
  ...
  fun noRescheduleStopsRepeatedTask() {
    var counter = 0
    val repeatedTask =
      RepeatedTask(name = "taskName", repeatedTaskConfig = repeatedTaskConfig) { _: String, _: TaskConfig ->
        counter++
  ...
  data class MyTaskConfig(val foo: String, val allResults: MutableList<String> = mutableListOf()) : TaskConfig()
  ...
  fun `Using custom task config works`() {
    ...
    val repeatedTask =
      RepeatedTask(name = "taskName", repeatedTaskConfig = repeatedTaskConfig, taskConfig = myClassConfig) {
        _: String,
