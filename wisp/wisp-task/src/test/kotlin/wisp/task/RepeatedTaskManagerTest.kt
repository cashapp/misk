...
internal class RepeatedTaskManagerTest {
  ...
  fun createTask() {
    ...
    val newTask =
      manager.createTask(name = "taskName", repeatedTaskConfig = repeatedTaskConfig) { _: String, _: TaskConfig ->
  ...
  fun `Getting a Task that does not exists throws a NoTaskFoundException`() {
    val manager = RepeatedTaskManager()
    assertThrows<NoTaskFoundException> { manager.getTask("unknownTask") }
  }
  ...
  fun shutDown() {
    ...
    val repeatedTask2 =
      manager.createTask(name = "differentTask", repeatedTaskConfig = repeatedTaskConfig) { _: String, _: TaskConfig ->
        Thread.sleep(100L)
        Status.OK
      }
