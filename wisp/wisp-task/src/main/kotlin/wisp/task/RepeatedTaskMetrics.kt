...
class RepeatedTaskMetrics(meterRegistry: MeterRegistry) {
  ...
  internal fun taskDuration(meterRegistry: MeterRegistry, name: String, result: String): DistributionSummary =
  ...
  internal val successCount: Counter =
    Counter.builder(SUCCESS_COUNTER_NAME).description("count of successful repeated tasks").register(meterRegistry)

  internal val failedCount: Counter =
    Counter.builder(FAILED_COUNTER_NAME).description("count of repeated tasks failures").register(meterRegistry)

  internal val noWorkCount: Counter =
    Counter.builder(NO_WORK_COUNTER_NAME)
