...
class BaggageTest {
  ...
  fun `Span#setBaggageItems() can set baggage in bulk`() {
    // No baggage.
    tracer.traceWithSpan("no-baggage") { span -> span.setBaggageItems(mapOf()) }
    var spans = tracer.finishedSpans()
    assertTrue(spans.size == 1, "Expected exactly one span")
    assertTrue(spans.map { it.context().baggageItems() }.first().toList().isEmpty(), "Expected no baggage")

    tracer.reset()

    // With baggage.
    val baggage = mapOf("movie" to "star wars", "release-year" to 1977, "producer" to Person("George Lucas"))
    tracer.traceWithSpan("set-baggage") { span -> span.setBaggageItems(baggage) }
