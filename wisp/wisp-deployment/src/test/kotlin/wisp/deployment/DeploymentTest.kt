...
internal class DeploymentTest {
  ...
  fun defaultDeploymentEnvIsDevelopment() {
    val deployment = getDeploymentFromEnvironmentVariable(environmentVariableLoader = environmentVariableLoader)
  ...
  fun deploymentEnvIsSetProperly() {
    val props =
      listOf(Deployment::isProduction, Deployment::isStaging, Deployment::isTest, Deployment::isLocalDevelopment)
    val environmentMap =
      mapOf(
    ...
    for ((envVar, envProperty) in environmentMap) {
      ...
      props.filter { it != envProperty }.forEach { assertFalse(it.invoke(deployment)) }
    }
  }

  @Test
  fun `deployment maps to environment name`() {
    assertEquals("production", PRODUCTION.mapToEnvironmentName())
    assertEquals("staging", STAGING.mapToEnvironmentName())
    assertEquals("testing", TESTING.mapToEnvironmentName())
    assertEquals("development", DEVELOPMENT.mapToEnvironmentName())
  }
}
