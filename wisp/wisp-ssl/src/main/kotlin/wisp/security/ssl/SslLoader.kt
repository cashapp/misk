...
@Deprecated(
  ...
  replaceWith = ReplaceWith(expression = "SslLoader(resourceLoader)", imports = ["misk.security.ssl.SslLoader"]),
)
open class SslLoader constructor(private val resourceLoader: ResourceLoader) {
  @JvmOverloads
  ...
  fun loadCertStore(config: CertStoreConfig) = loadCertStore(config.resource, config.format, config.passphrase)
