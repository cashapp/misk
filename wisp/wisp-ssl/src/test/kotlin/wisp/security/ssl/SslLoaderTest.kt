...
internal class SslLoaderTest {
  ...
  fun loadKeystoreFromPEM() {
    val keystore = sslLoader.loadCertStore(clientComboPemPath, SslLoader.Companion.FORMAT_PEM, "password")!!.keyStore
    ...
    assertThat(keystore.getX509Certificate().subjectAlternativeNames.toList()[0][1]).isEqualTo("127.0.0.1")
  }

  @Test
  fun loadTrustFromPEM() {
    val keystore =
      sslLoader.loadTrustStore(clientTrustPemPath, SslLoader.Companion.FORMAT_PEM, "serverpassword")!!.keyStore
  ...
  fun loadFromJCEKS() {
    val keystore =
      sslLoader.loadTrustStore(serverKeystoreJceksPath, SslLoader.Companion.FORMAT_JCEKS, "serverpassword")!!.keyStore
    ...
    assertThat(certificateAndKey.certificate.subjectAlternativeNames.toList()[0][1]).isEqualTo("127.0.0.1")

    val certificateChain = keystore.getX509CertificateChain()
    assertThat(certificateChain.map { it.issuerX500Principal.name })
      .containsExactly("CN=misk-server,OU=Server,O=Misk,L=San Francisco,ST=CA,C=US")
  }

  @Test
  fun loadKeystoreFromJKS() {
    val keystore = sslLoader.loadCertStore(keystoreJksPath, SslLoader.Companion.FORMAT_JKS, "changeit")!!.keyStore
  ...
  fun loadTrustFromJKS() {
    val keystore = sslLoader.loadTrustStore(truststoreJksPath, SslLoader.Companion.FORMAT_JKS, "changeit")!!.keyStore
  ...
  fun loadKeystoreFromP12() {
    val keystore = sslLoader.loadCertStore(keystoreP12Path, SslLoader.Companion.FORMAT_PKCS12, "changeit")!!.keyStore
    assertThat(keystore.aliasesOfType<KeyStore.PrivateKeyEntry>()).containsExactly("combined-key-cert")
    assertThat(keystore.getPrivateKey("changeit".toCharArray())).isNotNull()
  ...
  fun loadTrustFromP12() {
    val keystore = sslLoader.loadTrustStore(truststoreP12Path, SslLoader.Companion.FORMAT_PKCS12, "changeit")!!.keyStore
    assertThat(keystore.aliases().toList()).containsExactly("ca")
    assertThat((keystore.getX509Certificate()).issuerX500Principal.name)
      .isEqualTo("CN=misk-client,OU=Client,O=Misk,L=San Francisco,ST=CA,C=US")
    assertThat(keystore.getX509Certificate().subjectAlternativeNames.toList()[0][1]).isEqualTo("127.0.0.1")
