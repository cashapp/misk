...
internal class WispConfigTest {
  ...
  data class MyConfig(val foo: Foo, val baz: String) : Config

  data class Foo(val enabled: Boolean, val bar: Int)

  data class AWSCredentials(val AWS_ACCESS_KEY_ID: Masked, val AWS_SECRET_ACCESS_KEY: Masked) : Config

  data class AWSConfigYaml(val aws: AWSCredentials) : Config

  data class ResourceLoaderPreprocessorContents(val content: Masked)

  @Test
  fun `config for a single config file loads`() {
    val builder = WispConfig.builder()
    builder.addWispConfigSources(listOf(ConfigSource("classpath:/a.yaml")))
  ...
  fun `config for a two config file loads with correct override`() {
    val builder = WispConfig.builder()
    builder.addWispConfigSources(listOf(ConfigSource("classpath:/b.yaml"), ConfigSource("classpath:/a.yaml")))
  ...
  fun `config for a multiple config files loads with correct override`() {
    ...
    builder.addWispConfigSources(
      listOf(ConfigSource("classpath:/c.yaml"), ConfigSource("classpath:/b.yaml"), ConfigSource("classpath:/a.yaml"))
    )
  ...
  fun `config for a aws credential yaml file loads with correct values and should be masked`() {
    val builder = WispConfig.builder()
    builder.addWispConfigSources(listOf(ConfigSource("classpath:/aws.yaml")))
  ...
  fun `config for a aws credential properties file loads with correct values and should be masked`() {
    val builder = WispConfig.builder()
    builder.addWispConfigSources(listOf(ConfigSource("classpath:/aws_credentials", "props")))
  ...
  fun `ResourceLoader preprocessor loads data`() {
    val builder = WispConfig.builder()
    builder.addWispConfigSources(listOf(ConfigSource("classpath:/d.yaml")))
