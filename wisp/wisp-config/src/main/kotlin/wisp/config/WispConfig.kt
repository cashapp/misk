 ...
 *
 * Config precedence is in order of sources added for loading, so if you want an environment config file to override
 * values in the default file, it has to be added first.
 *
 * Note that config values from Environment Variables, System Properties and User Settings are always loaded first, see:
 * https://github.com/sksamuel/hoplite#property-sources for details.
 *
...
object WispConfig {
   ...
   * Example use:
   *
   * val myConfig = WispConfig.builder() ... .addWispConfigSources(...) ... .build() .loadConfigOrThrow<MyConfig>() Note
   * that if you are passing configuration fragments within Wisp, MyConfig should implement [Config]
   *
...
/**
 * Add the config sources in the order supplied. If the config source location does not exist it is silently skipped,
 * i.e. the config locations are optional.
 */
fun ConfigLoaderBuilder.addWispConfigSources(
  ...
  configSources
    .filter { (resourceLoader.exists(it.configLocation)) }
    .forEach {
      val configContents = resourceLoader.utf8(it.configLocation)!!
      when (it.format) {
        "yml",
        "yaml" -> this.addSource(YamlPropertySource(configContents))
        "json" -> this.addSource(JsonPropertySource(configContents))
        "toml" -> this.addSource(TomlPropertySource(configContents))
        else ->
          this.addSource(
            InputStreamPropertySource(configContents.byteInputStream(Charsets.UTF_8), it.format, it.configLocation)
          )
      }
    }
