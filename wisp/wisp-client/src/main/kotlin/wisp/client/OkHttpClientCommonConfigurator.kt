...
@Deprecated(
  ...
  replaceWith =
    ReplaceWith(expression = "OkHttpClientCommonConfigurator", imports = ["misk.client.OkHttpClientCommonConfigurator"]),
)
class OkHttpClientCommonConfigurator {
  ...
  private fun configureFollowRedirects(builder: Builder, config: HttpClientEndpointConfig) {
    builder.followRedirects(config.clientConfig.followRedirect ?: followRedirects)
  }
