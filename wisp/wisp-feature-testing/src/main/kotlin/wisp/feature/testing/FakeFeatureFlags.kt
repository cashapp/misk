...
@Deprecated(
  ...
  replaceWith = ReplaceWith(expression = "FakeFeatureFlags()", imports = ["misk.feature.testing.FakeFeatureFlags"]),
)
open class FakeFeatureFlags
private constructor(val legacyFeatureFlags: FakeLegacyFeatureFlags, val strongFeatureFlags: FakeStrongFeatureFlags) :
  FeatureFlags,
  ...
  constructor(moshi: () -> Moshi) : this(FakeLegacyFeatureFlags(moshi), FakeStrongFeatureFlags())
  ...
  fun <T> override(feature: Feature, value: T, clazz: Class<T>) = legacyFeatureFlags.override(feature, value, clazz)

  fun overrideJsonString(feature: Feature, json: String) = legacyFeatureFlags.overrideJsonString(feature, json)
  ...
  override fun configure(config: FakeFeatureFlagsConfig) {
    config.featuresConfig.forEach {
      ...
      when (it.type) {
        "Boolean" -> overrideKey(Feature(it.featureName), key, it.value.toBoolean(), it.attributes)
        "Int" -> overrideKey(Feature(it.featureName), key, it.value.toInt(), it.attributes)
        "Double" -> overrideKey(Feature(it.featureName), key, it.value.toDouble(), it.attributes)
        "String" -> overrideKey(Feature(it.featureName), key, it.value, it.attributes)
        "ENUM" -> {
          TODO("Need to work out how to dynamically create an Enum of type X " + "with the String value")
        }
        "JSON" -> overrideKeyJsonString(Feature(it.featureName), key, it.value, it.attributes)
        else -> {
...
data class FakeFeatureFlagsConfig @JvmOverloads constructor(val featuresConfig: List<FeaturesConfig> = emptyList()) :
  Config
