...
class FakeStrongFeatureFlags : StrongFeatureFlags {
  ...
  private data class FeatureFlagConfig<T : Any>(val matchers: MutableList<FeatureMatcher<T>> = mutableListOf())

  private data class FeatureMatcher<T : Any>(val condition: (FeatureFlag<in T>) -> Boolean, val value: T)
  ...
  fun <T : Any, Flag : FeatureFlag<in T>> overrideAny(
    ...
    @Suppress("UNCHECKED_CAST") val typedMatcher = matcher as (FeatureFlag<in T>) -> Boolean
  ...
  private fun <T : Any> getAny(flag: FeatureFlag<T>): T {
    ...
    return featureConfig.matchers.lastOrNull() { it.condition(flag) }?.value ?: error("no flag match found for $flag")
  }
