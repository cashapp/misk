...
internal class FakeFeatureFlagsTest {
  ...
  data class TestNoAttributeBooleanFlag(val username: String = "test-boolean-username") : BooleanFeatureFlag {
  ...
  fun getJson() {
    ...
    assertThat(subject.getJson<JsonFeature>(FEATURE, "batman")).isEqualTo(JsonFeature("batman"))
    assertThat(subject.getJson<JsonFeature>(FEATURE, "batman", badBatmanNumericAttributes))
      .isEqualTo(JsonFeature("bad-batman"))
    assertThat(subject.getJson<JsonFeature>(FEATURE, "batman", sleepyBadBatmanNumericAttributes))
      .isEqualTo(JsonFeature("bad-batman"))
    // Provides the key level override when there is no match on attributes
  ...
  fun configureOverridesFeatures() {
    ...
    val config =
      WispConfig.builder().addWispConfigSources(listOf(configSource)).build().loadConfigOrThrow(clazz, emptyList())
  ...
  fun `strongly typed feature flags do not need to override attributes`() {
    ...
    subject.strongFeatureFlags.override<TestNoAttributeBooleanFlag>(true) { flag -> flag.attributes == Attributes() }
    assertThat(subject.strongFeatureFlags.get(booleanFlag)).isEqualTo(true)
  }

  /**
   * The heavy-lifying for strong feature flag testing lives in the other `TestFactory`-based tests below, but they use
   * a lot of generic magic to test every type of flag.
   *
   * To make sure we didn't miss anything, we also do a normal smoke test on String, if this works and all the below
   * tests also work we can be reasonably confident things are working well.
   */
  @Test
  fun `string feature flags should act as expected`() {
    ...
    // Can override with specific keys and attributes
    subject.override<TestStringFlag>("feature-joker-bad") { it.username == "joker" && it.segment == "bad" }
    subject.get(TestStringFlag()).shouldBe("feature")
    subject.get(TestStringFlag(username = "joker")).shouldBe("feature-joker")
  ...
  fun `strong feature flags should return overriden value`() = forAllStrongFlagTypes {
    val featureFlags = FakeFeatureFlags().scenarioOverride(scenarioValueOne)
  ...
  fun `strong feature flags should prefer last override value`() = forAllStrongFlagTypes {
    val featureFlags = FakeFeatureFlags().scenarioOverride(scenarioValueOne).scenarioOverride(scenarioValueTwo)
  ...
  fun `strong feature flags should use latest override if condition is met`() = forAllStrongFlagTypes {
    val featureFlags =
      FakeFeatureFlags().scenarioOverride(scenarioValueOne).scenarioOverride(scenarioValueTwo) { it.username == "vip" }
  ...
  fun `strong feature flags should work for non-key attributes`() = forAllStrongFlagTypes {
    val featureFlags = FakeFeatureFlags().scenarioOverride(scenarioValueOne) { it.segment == "pro-segment" }

    featureFlags.scenarioGet(scenarioFlag(segment = "pro-segment")).shouldBe(scenarioValueOne)
  }

  @TestFactory
  fun `strong feature flags should still use older overrides if newer ones don't match`() = forAllStrongFlagTypes {
    val featureFlags =
      FakeFeatureFlags().scenarioOverride(scenarioValueOne).scenarioOverride(scenarioValueTwo) { it.username == "vip" }
  ...
  fun `strong feature flags should not match if condition is not met`() = forAllStrongFlagTypes {
    val featureFlags = FakeFeatureFlags().scenarioOverride(scenarioValueOne) { it.username == "vip" }
