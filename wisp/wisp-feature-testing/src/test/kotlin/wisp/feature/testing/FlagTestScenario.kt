...
/**
 * Helper function that lets us write tests for all feature flag types, instead of duplcating tests 6 times.
 *
 * Within a test [FlagTestScenario] is in scope, providing access to `scenarioOverride`, `scenarioGet`, `scenarioTrack`
 * etc...
 *
 * Usage:
 * ```kotlin
...
fun forAllStrongFlagTypes(block: FlagTestScenario<FeatureFlag<Any>, Any>.() -> Unit): List<DynamicTest> =
  listOf(
...
interface FlagTestScenario<Flag : FeatureFlag<in T>, T : Any> {
   ...
   *
   * For example, if we are in [StringFlagTestScenario], this will call call `FakeFeatureFlags.get<TestStringFlag>()`
   */
  fun FakeFeatureFlags.scenarioGet(flag: Flag): T

  /**
   * Call the appropriate strongly typed [FakeFeatureFlags.override] method depending on the scenario.
   *
   ...
   */
  fun FakeFeatureFlags.scenarioOverride(value: T, matcher: (TestFlag<T>) -> Boolean = { _ -> true }): FakeFeatureFlags
...
object StringFlagTestScenario : FlagTestScenario<TestStringFlag, String> {
  ...
  override fun scenarioFlag(username: String, segment: String): TestStringFlag = TestStringFlag(username, segment)
...
object BooleanFlagTestScenario : FlagTestScenario<TestBooleanFlag, Boolean> {
  ...
  override fun scenarioFlag(username: String, segment: String): TestBooleanFlag = TestBooleanFlag(username, segment)
...
object IntFlagTestScenario : FlagTestScenario<TestIntFlag, Int> {
  ...
  override fun scenarioFlag(username: String, segment: String): TestIntFlag = TestIntFlag(username, segment)

  override fun FakeFeatureFlags.scenarioGet(flag: TestIntFlag): Int = this.get(flag)

  override fun FakeFeatureFlags.scenarioOverride(value: Int, matcher: (TestFlag<Int>) -> Boolean): FakeFeatureFlags =
    this.override<TestIntFlag>(value, matcher)
}
...
object DoubleFlagTestScenario : FlagTestScenario<TestDoubleFlag, Double> {
  ...
  override fun scenarioFlag(username: String, segment: String): TestDoubleFlag = TestDoubleFlag(username, segment)
...
object EnumFlagTestScenario : FlagTestScenario<TestEnumFlag, TestEnum> {
  ...
  override fun scenarioFlag(username: String, segment: String): TestEnumFlag = TestEnumFlag(username, segment)
...
object JsonFlagTestScenario : FlagTestScenario<TestJsonFlag, TestJsonObject> {
  ...
  override fun scenarioFlag(username: String, segment: String): TestJsonFlag = TestJsonFlag(username, segment)
