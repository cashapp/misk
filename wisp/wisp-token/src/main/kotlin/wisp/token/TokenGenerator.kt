...
/**
 * Generates an secure, unguessable, alphanumeric token for use as a universally unique ID. Tokens are encoded with a
 * [Crockford Base32 alphabet][https://www.crockford.com/wrmg/base32.html]. This alphabet contains a mix of lowercase
 * characters and digits and is resistant to decoding errors; for example `0`, `o`, and 'O' are equivalent.
 *
 * For strength similar to a [random UUID][java.util.UUID.randomUUID] (122 bits of entropy), most callers should use the
 * default length of 25 characters (125 bits). Using fewer characters risks collision, which may be acceptable for some
 * use-cases. There is no practical benefit to using more than 25 characters.
 *
 ...
 * ```
 *
 * In tests tokens are sequential and predictable. They are prefixed with an optional label that appears in the returned
 * string and can be used as a namespace. It is okay to hardcode expected tokens in test cases! These are some sample
 * testing tokens:
 * ```
...
@Deprecated(
  ...
  replaceWith = ReplaceWith(expression = "TokenGenerator()", imports = ["misk.tokens.TokenGenerator"]),
)
