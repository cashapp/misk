...
class SchemaMigratorPluginTest {
  @ParameterizedTest
  @ValueSource(
    strings = ["/schema-migrator-plugin-test", "/schema-migrator-plugin-test-with-declarative-migrations-format"]
  )
  fun `schema migrator plugin migrates schemas`(projectDir: String) {
    ...
    datasource.connection.use { connection ->
      connection.createStatement().use { statement -> statement.execute("DROP DATABASE IF EXISTS ${config.schema};") }
    }
    ...
    datasource.connection.use { connection ->
      connection.createStatement().use { statement ->
        statement.execute(
          "SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '${config.schema}' AND TABLE_NAME = 'people' AND COLUMN_NAME = 'nickname'"
        )
