...
class DashboardMetadataActionTest {
  ...
  @Test
  fun `admin dashboard unauthenticated tabs`() {
    val response = executeRequest(path = asDashboardPath<AdminDashboard>())
    assertEquals(0, response.dashboardMetadata.tabs.size)
  }

  @Test
  fun `admin dashboard unauthorized tabs`() {
    val response = executeRequest(path = asDashboardPath<AdminDashboard>(), user = "sandy", capabilities = "guest")
    assertEquals(0, response.dashboardMetadata.tabs.size)
  }

  @Test
  fun `admin dashboard authorized tabs`() {
    val response =
      executeRequest(path = asDashboardPath<AdminDashboard>(), user = "sandy", capabilities = "admin_access")
  ...
  @Test
  fun `test dashboard navbar items`() {
    val response =
      executeRequest(
        path = asDashboardPath<DashboardMetadataActionTestDashboard>(),
        user = "sandy",
    ...
    assertEquals("<a href=\"https://cash.app/\">Test Navbar Link</a>", response.dashboardMetadata.navbar_items.first())
  }

  @Test
  fun `test dashboard navbar status`() {
    val response =
      executeRequest(
        path = asDashboardPath<DashboardMetadataActionTestDashboard>(),
        user = "sandy",
  ...
  @Test
  fun `test dashboard custom theme`() {
    val response =
      executeRequest(
        path = asDashboardPath<DashboardMetadataActionTestDashboard>(),
        user = "sandy",
