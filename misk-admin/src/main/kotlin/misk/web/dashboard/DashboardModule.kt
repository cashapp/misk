...
constructor(
  ...
  override fun configure() {
    ...
    dashboardTabLoader?.let {
      multibind<DashboardTabLoaderEntry>().toInstance(DashboardTabLoaderEntry(it.urlPathPrefix, it))
    ...
    indexAccessBlocks.forEach { multibind<DashboardIndexAccessBlock>().toInstance(it) }

    indexBlocks.forEach { multibind<DashboardIndexBlock>().toInstance(it) }
  }
  ...
  companion object {
    ...
    inline fun <reified DA : Annotation, reified AA : Annotation> createMenuLink(
      ...
      return DashboardModule(dashboardTabProvider = dashboardTabProvider)
    }
    ...
    /**
     * @param slug: unique slug to identify the tab namespace, it must match the tab's corresponding [DashboardTab]
     *   multibinding
    ...
    inline fun <reified DA : Annotation, reified AA : Annotation> createHotwireTab(
      ...
      val dashboardTabLoader = DashboardTabLoader.HotwireTab(urlPathPrefix = urlPathPrefix)
      val dashboardTabProvider =
        DashboardTabProvider(
          slug = slug,
      ...
      return DashboardModule(dashboardTabProvider = dashboardTabProvider, dashboardTabLoader = dashboardTabLoader)
    }
     ...
     *   multibinding
     * @param urlPathPrefix: path prefix which when used in a user's navigation URL will route to this tab.
     * @param resourcePathPrefix: path prefix used for background network requests to get resources (HTML, CSS...) for
     *   the tab from a resource provider (classpath, filesystem, web proxy...).
     * @param iframePath: complete path including file and extension if necessary which is set as the iframe src
    ...
    inline fun <reified DA : Annotation, reified AA : Annotation> createIFrameTab(
      ...
      val dashboardTabLoader = DashboardTabLoader.IframeTab(urlPathPrefix = urlPathPrefix, iframePath = iframePath)
      val dashboardTabProvider =
        DashboardTabProvider(
          slug = slug,
          url_path_prefix = urlPathPrefix,
    ...
    /**
     * Installs a Misk-Web tab for a dashboard [DA] with access [AA]. The tab is identified by a unique [slug] and is
     * routed to by match on [urlPathPrefix]. In local development – when [isDevelopment] is true, the
     * [developmentWebProxyUrl] is used to resolve requests to [resourcePathPrefix]. In real environments, the
     * [classpathResourcePathPrefix] is used to resolve resource requests to files in classpath. The tab is included in
     * the dashboard navbar menu with [menuLabel] and in the menu group [menuCategory].
     */
    ...
    /**
     * Installs a Misk-Web app for a dashboard [DA] with access [AA]. The tab is identified by a unique [slug] and is
     * routed to by match on [urlPathPrefix]. In local development – when [isDevelopment] is true, the
     * [developmentWebProxyUrl] is used to resolve requests to [resourcePathPrefix]. In real environments, the
     * [classpathResourcePathPrefix] is used to resolve resource requests to files in classpath. The tab is included in
     * the dashboard navbar menu with [menuLabel] and in the menu group [menuCategory].
    ...
    fun addIndexAccessBlocks(vararg blocks: DashboardIndexAccessBlock) =
      DashboardModule(indexAccessBlocks = blocks.toList())

    /** Add access blocks to dashboard index. */
    fun addIndexBlocks(vararg blocks: DashboardIndexBlock) = DashboardModule(indexBlocks = blocks.toList())
  }
}
