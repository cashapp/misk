 ...
 *
 * In Development environment, requests are proxied through to a local running build server. In all other environments,
 * resource requests are served from the classpath.
 *
 ...
 *
 * @property slug A unique slug to identify the tab namespace, it must match the tab's corresponding [DashboardTab]
 *   multibinding
 * @property web_proxy_url a fully qualified url for the development build server (includes `http://` prefix)
 * @property url_path_prefix URL namespace used to determine where to route requests with that url prefix By Misk-Web
 *   convention, the prefix for tabs is `/_tab/{slug}/` to prevent namespace collisions with dashboard urls such as
 *   `/_admin/` or `/app/` since path routing is by url prefix.
...
constructor(
  ...
  override fun configure() {
    ...
    multibind<StaticResourceEntry>()
      .toInstance(StaticResourceEntry(url_path_prefix = url_path_prefix, resourcePath = resourcePath))
    if (isDevelopment) {
      web_proxy_url?.let {
        install(WebActionModule.createWithPrefix<WebProxyAction>(url_path_prefix = url_path_prefix))
        multibind<WebProxyEntry>()
          .toInstance(WebProxyEntry(url_path_prefix = url_path_prefix, web_proxy_url = web_proxy_url))
      }
    } else {
