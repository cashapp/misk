...
constructor(
  ...
  private var title: (appName: String, dashboardHomeUrl: DashboardHomeUrl?, dashboardTab: DashboardTab?) -> String =
    { appName: String, dashboardHomeUrl: DashboardHomeUrl?, dashboardTab: DashboardTab? ->
      "${dashboardTab?.menuLabel?.let { "$it | " } ?: ""}${dashboardTab?.menuCategory} on $appName ${dashboardHomeUrl?.dashboardAnnotationKClass?.titlecase() ?: ""}"
    }

  private val path by lazy { clientHttpCall.get().url.encodedPath }
  ...
  fun newBuilder(): DashboardPageLayout =
    DashboardPageLayout(
  ...
  fun build(
    tabBlock:
      TagConsumer<*>.(appName: String, dashboardHomeUrl: DashboardHomeUrl?, dashboardTab: DashboardTab?) -> Unit =
      { _, _, _ ->
        Unit
      }
  ): String {
    check(newBuilder) { "You must call newBuilder() before calling build() to prevent builder reuse." }
    newBuilder = false
    ...
    logger.info(
      "${callerProvider.get()} visited dashboard tab$analyticsTitle at $path [dashboard=${dashboardHomeUrl?.dashboardAnnotationKClass?.simpleName}]"
    )
    ...
    return buildHtml {
      HtmlLayout(
        ...
        Navbar(appName = appName, deployment = deployment, homeHref = homeUrl, menuSections = menuSections) {
          ...
          turbo_frame(id = "tab") { tabBlock(appName, dashboardHomeUrl, dashboardTab) }
        }
      }
    }
  }
    ...
    if (navbarItems.isNotEmpty()) {
      val links =
        navbarItems
          ...
          .map { Link(href = "", label = "", rawHtml = it.item) }
      if (links.isNotEmpty()) {
        listOf(MenuSection(title = "Links", links = links))
      } else {
        listOf()
      }
    } else {
      ...
      dashboardTabs
        .groupBy { it.menuCategory }
        .map { (sectionTitle, dashboardTabs) ->
          MenuSection(
            title = sectionTitle,
            links =
              dashboardTabs.map {
