...
@Singleton
internal class DashboardIFrameTabAction
@Inject
constructor(
    ...
    dashboardPageLayout
      ...
      .build { _, _, _ ->
        ...
        if (iframeTab != null) {
          ...
          if (iframeTab.iframePath.startsWith(MiskWebTabIndexAction.PATH)) {
            ...
            if (dashboardTab == null) {
              ...
              if (deployment.isLocalDevelopment) {
                ...
                val tabEntrypointJsResponse = webProxyAction.getResponse((hostname / tabEntrypointJs).toHttpUrl())
                if (tabEntrypointJsResponse.statusCode != 200) {
                  div("container mx-auto p-8") {
                    ...
                    AlertInfo(
                      "In local development, this can be from not having your local dev server (ie. Webpack) running or not doing an initial local Misk-Web build to generate the necessary web assets. Try running in your Terminal: \$ gradle buildMiskWeb OR \$ misk-web ci-build -e."
                    )
                  }
                }
              } else if (deployment.isReal) {
                // If real environment, only check static resource action and show limited 404 message
                val tabEntrypointJsResponse = staticResourceAction.getResponse((hostname / tabEntrypointJs).toHttpUrl())
                if (tabEntrypointJsResponse.statusCode != 200) {
                  logger.info(
                    "Failed to load Misk-Web tab: ${dashboardTab.menuCategory} / ${dashboardTab.menuLabel} Response: $tabEntrypointJsResponse"
                  )
                  div("container mx-auto p-8") {
                    AlertError("Failed to load Misk-Web tab: ${dashboardTab.menuCategory} / ${dashboardTab.menuLabel}")
                    AlertInfo(
                      "In real environments, this is usually because of a web build failure in CI. Try checking CI logs and report this bug to your platform team. If the CI web build fails or is not run, the web assets will be missing from the Docker context when deployed and fail to load."
                    )
                  }
                }
              }
            ...
            if (dashboardTab == null) {
              ...
              if (deployment.isLocalDevelopment) {
                // If local development, check web proxy action and show fuller development 404 message
                val proxyResponse = webProxyAction.getResponse((hostname / iframeTab.iframePath).toHttpUrl())
                val staticResponse = staticResourceAction.getResponse((hostname / iframeTab.iframePath).toHttpUrl())
                if (proxyResponse.statusCode != 200 && staticResponse.statusCode != 200) {
                  div("container mx-auto p-8") {
                    ...
                    if (iframeTab.iframePath == "/_tab/web-actions/index.html") {
                      AlertInfo(
                        "To use the Web Actions tab in local development, start the dev server (run in your terminal: \$ cd misk/misk-admin-web-actions/ && npm start) or run a build to generate the necessary web assets (run in your terminal: \$ gradle :misk:misk-admin:buildAndCopyWebActions)."
                      )
                    } else {
                      AlertInfo(
                        "In local development, this can be from not having your local dev server (ie. Webpack) running or not doing an initial local frontend build to generate the necessary web assets. Try running in your Terminal: \$ gradle buildMiskWeb OR \$ misk-web ci-build -e."
                      )
                    }
                  }
                }
                ...
                val tabEntrypointJsResponse =
                  staticResourceAction.getResponse((hostname / iframeTab.iframePath).toHttpUrl())
                if (tabEntrypointJsResponse.statusCode != 200) {
                  logger.info(
                    "Failed to load tab: ${dashboardTab.menuCategory} / ${dashboardTab.menuLabel} Response: $tabEntrypointJsResponse"
                  )
                  div("container mx-auto p-8") {
                    AlertError("Failed to load tab: ${dashboardTab.menuCategory} / ${dashboardTab.menuLabel}")
                    AlertInfo(
            ...
            try {
              OkHttpClient.Builder().build().newCall(Request((hostname / iframeSrc).toHttpUrl())).execute().use {
                response ->
                if (!response.isSuccessful) {
                  div("container mx-auto p-8") { AlertError("Failed to load tab at $iframeSrc") }
                }
              }
            } catch (e: Exception) {
              div("container mx-auto p-8") { AlertError("Failed to load tab at $iframeSrc") }
          ...
          iframe(classes = "h-full w-full") { src = iframeSrc }
        } else {
          div("container mx-auto p-8") {
  ...
  private operator fun String.div(other: String): String =
    StringBuilder().append(this.removeSuffix("/")).append("/").append(other.removePrefix("/")).toString()
