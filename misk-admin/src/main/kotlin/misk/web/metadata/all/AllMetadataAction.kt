...
constructor(
  ...
  private fun Map.Entry<String, @JvmSuppressWildcards Provider<Metadata>>.timedGet(): Metadata {
    val (duration, metadata) = timed { value.get() }
    val durationMillis = duration.toMillis()
    if (durationMillis > 10) {
      logger.info(
        "MetadataProvider [key=$key] took [loadTimeMs=${durationMillis}ms] to load [sizeChars=${metadata.toString().length}] by [class=${value}]."
      )
    }
    return metadata
  }
