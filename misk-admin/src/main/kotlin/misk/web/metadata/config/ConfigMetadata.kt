...
internal data class ConfigMetadata(val resources: Map<String, String?>) :
  Metadata(
    metadata = resources,
    prettyPrint =
      resources.entries.joinToString("\n\n") { "${it.key}\n---\n${it.value?.removePrefix("---")?.removePrefix("\n")}" },
  ) {
  override fun descriptionBlock(tagConsumer: TagConsumer<*>) =
    tagConsumer.apply {
      ToggleContainer(
...
internal class ConfigMetadataProvider : MetadataProvider<ConfigMetadata> {
  ...
  override fun get() = ConfigMetadata(resources = generateConfigResources(appName, deployment, config))

  private fun generateConfigResources(appName: String, deployment: Deployment, config: Config): Map<String, String?> {
