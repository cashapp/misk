 ...
 * ```
 *
 * @param modifierClass override of the icon's [defaultModifierClass]. For example if icon should have a different
 *   color, boldness...etc then a [modifierClass] can be provided which overrides the icon's [defaultModifierClass].
 */
fun TagConsumer<*>.heroicon(icon: Heroicons, modifierClass: String? = null, spanClass: String? = null) {
  span(spanClass) {
    //    """<!-- Heroicon name: ${it.id} -->"""
    unsafe {
      raw(
        icon.rawHtml(
          icon.svgClass +
            " " +
            (HeroiconProps(icon, modifierClass, spanClass).modifierClass ?: icon.defaultModifierClass)
        )
      )
    }
  }
...
@JvmOverloads
constructor(val icon: Heroicons, val modifierClass: String? = null, val spanClass: String? = null)
