...
data class CoordinatedServiceMetadata(val dependencies: Set<String>, val directDependsOn: Set<String>)

internal inline fun <reified T : Service> CoordinatedService(serviceProvider: Provider<T>) =
  CoordinatedService(key = T::class.toKey(), serviceProvider = serviceProvider)

internal class CoordinatedService(private val key: Key<*>, private val serviceProvider: Provider<out Service>) :
  AbstractService(), DelegatingService {

  /**
   * To avoid accessing the by lazy service property for toString, use key until the service is initiated by the service
   * graph. This will be replaced with service.toString() during by lazy initialization.
   */
  private val toStringLazy =
    Atomics.newReference<() -> String> {
      buildString {
  ...
  /**
   * Use care when accessing this value as it will instantiate the instance of the class at this time. We want the
   * instantiation to follow the Coordinated Service Graph.
   */
  override val service: Service by lazy {
    val realService =
      runCatching { serviceProvider.get() }
        .onFailure { this@CoordinatedService.notifyFailed(it) }
        .getOrThrow()
        .also { created ->
    ...
    toStringLazy.set { realService.toString() }
  ...
  /** Used to track internally if this [CoordinatedService] has invoked [startAsync] on the inner [Service] [service] */
  private val innerServiceStarted = AtomicBoolean(false)

  /** Marks every [coordinatedServices] as a dependency and marks itself as directDependsOn on each service. */
  ...
  private fun stopIfReady() {
    val canStopInner = state() == State.STOPPING && dependencies.all { it.isTerminatedOrFailed() }
  ...
  /**
   * Get the service name from the current string source. This uses the key until the service is Injected to avoid early
   * injecting the service out of band from the service graph.
   */
  override fun toString() = toStringLazy.get().invoke()

  /**
   * Traverses the dependency graph to detect cycles. If a cycle is found, then a list of services that forms this cycle
   * is returned.
   *
   * @param validityMap: A map that is used to track traversal of this service's dependency graph.
   */
  fun findCycle(validityMap: MutableMap<CoordinatedService, CycleValidity>): MutableList<CoordinatedService>? {
  ...
  companion object {
    ...
    private fun Service.checkNew(message: String = "Cannot add dependencies after the service graph has been built") {
