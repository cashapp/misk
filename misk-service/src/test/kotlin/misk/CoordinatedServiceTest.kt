...
class CoordinatedServiceTest {
  ...
  @RepeatedTest(100)
  fun fuzzCoordinatedServiceGraphStartAndStop() {
  ...
  @Test
  fun canShutDownGracefullyOnStartFailure() {
    ...
    val failOnStartService = (key("failOnStartService") to FailOnStartService()).toCoordinated()
    ...
    manager.startAsync()
    assertThatThrownBy { manager.awaitHealthy() }.isInstanceOf(IllegalStateException::class.java)
  ...
  @Test
  fun canShutDownGracefullyOnStopFailure() {
    ...
    val failOnStopService = (key("failOnStopService") to FailOnStopService()).toCoordinated()
  ...
  @Test
  fun cannotAddRunningServiceAsDependency() {
    val target = StringBuilder()
    val runningService = CoordinatedService { AppendingService(target, "I will be running") }
    val newService = CoordinatedService { AppendingService(target, "I will not run") }

    runningService.startAsync()

    assertFailsWith<IllegalStateException> { newService.addDependentServices(runningService) }
    assertFailsWith<IllegalStateException> { newService.addDependentServices(runningService) }
  ...
  @Test
  fun cannotWrapRunningService() {
    val target = StringBuilder()
    val service = AppendingService(target, "Running Service")
    service.startAsync()

    val failure =
      assertFailsWith<IllegalStateException> { CoordinatedService { service }.service.startAsync().awaitRunning() }
  ...
  private class FakeService(val name: String) : AbstractIdleService() {
    override fun startUp() {}
  ...
  private fun service(id: String): Pair<Key<*>, CoordinatedService> = (key(id) to FakeService(id)).toCoordinated()
  ...
  private fun Pair<Key<*>, Service>.toCoordinated() =
    this.first to CoordinatedService(this.first, Provider { this.second })
