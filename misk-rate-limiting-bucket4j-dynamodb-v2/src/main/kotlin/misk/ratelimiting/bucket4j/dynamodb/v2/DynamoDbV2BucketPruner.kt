...
class DynamoDbV2BucketPruner @JvmOverloads constructor(
  clock: Clock,
  ...
  private fun pruneLoop() {
    ...
    pager.items().forEach { item ->
      val stateBytes = item[STATE_ATTRIBUTE]?.b()?.asByteArray() ?: run {
        logger.warn {
          "Row ${item[HASH_ATTRIBUTE]?.s()} is missing its state attribute, " +
