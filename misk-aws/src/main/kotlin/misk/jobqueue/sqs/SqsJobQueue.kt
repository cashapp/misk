...
internal constructor(
        ...
        queue.call { client ->
          val sendRequest =
            SendMessageRequest().apply {
              ...
              attributes.forEach { (key, value) -> addMessageAttributesEntry(key, value.toMessageAttributeValue()) }
    ...
    executeWithTracingAndErrorHandling(queueName, jobs.size) { span: Span, queue: ResolvedQueue ->
        ...
        queue.call { client ->
          val messageEntries =
            jobs.map { message ->
              ...
              SendMessageBatchRequestEntry(message.idempotenceKey, message.body).apply {
                if (message.deliveryDelay != null) delaySeconds = getDelaySeconds(message.deliveryDelay)
                message.attributes.forEach { (key, value) ->
                  addMessageAttributesEntry(key, value.toMessageAttributeValue())
                }
          ...
          timed { client.sendMessageBatch(SendMessageBatchRequest(queue.url, messageEntries)) }
        }
      ...
      if (batchResult.failed.size > 0) {
        throw JobQueue.BatchEnqueueException(
          ...
          batchResult.failed.map { JobQueue.EnqueueErrorResult(it.id, it.isSenderFault, it.code, it.message) },
        )
      }
  ...
  private fun String.toMessageAttributeValue(): MessageAttributeValue =
    MessageAttributeValue().withDataType("String").withStringValue(this)
