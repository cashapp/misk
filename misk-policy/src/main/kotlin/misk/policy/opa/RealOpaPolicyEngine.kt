...
/**
 * Support for the Open Policy Engine (OPA). OPA provides a means to decouple policy from business logic. The resulting
 * query response may have arbitrary shapes.
 */
class RealOpaPolicyEngine
@Inject
constructor(
  ...
  private fun <T : OpaRequest, R : OpaResponse> evaluateInternal(
    ...
    val inputAdapter = moshi.adapter<Request<T>>(Types.newParameterizedType(Request::class.java, inputType))
   ...
   * perform a /v1/data/{document} POST. This consumes raw JSON for corner cases where developers need to do queries the
   * automatic JSON serialization doesn't support.
   *
   * @param document Name or Path of the OPA document to query.
  ...
  /**
   * Evaluate / Query a document with no additional input. This will connect to OPA via a retrofit interface and perform
   * a /v1/data/{document} POST.
   *
   * @param document Name or Path of the OPA document to query.
  ...
  private fun queryOpa(document: String, inputString: String = ""): retrofit2.Response<ResponseBody> {
    if (document.isEmpty()) {
      throw IllegalArgumentException("Must specify document")
    }
  ...
  private fun <R : OpaResponse> parseResponse(
    ...
    val outputAdapter = moshi.adapter<Response<R>>(Types.newParameterizedType(Response::class.java, returnType))

    val responseBody = response.body()?.string() ?: throw PolicyEngineException("OPA response body is empty")
    val extractedResponse =
      try {
        outputAdapter.fromJson(responseBody) ?: throw PolicyEngineException("Unmarshalled OPA response body is empty")
      } catch (e: Exception) {
