...
/**
 * ScheduledExecutorService for testing that runs in the current thread and is triggered using the `tick()` method. An
 * injected [Clock] is used to decide whether to execute a scheduled task.
 *
...
class FakeScheduledExecutorService @Inject constructor(private val clock: Clock) :
  ...
  override fun schedule(command: Runnable, delay: Long, unit: TimeUnit) = scheduleTask(delay, unit) { command.run() }

  override fun <V> schedule(callable: Callable<V>, delay: Long, unit: TimeUnit) =
  ...
  inner class ScheduledFutureTask<V>(
    ...
    constructor(executeAt: Long, clock: Clock, task: () -> V) : this(executeAt, 0L, clock, task)

    override fun compareTo(other: Delayed): Int =
      getDelay(TimeUnit.MILLISECONDS).compareTo(other.getDelay(TimeUnit.MILLISECONDS))

    override fun getDelay(unit: TimeUnit) = unit.convert(clock.millis() - executeAt, TimeUnit.MILLISECONDS)
    ...
    internal fun reschedule() {
      ...
      futures.add(
        ScheduledFutureTask(executeAt = executeAt + fixedDelay, fixedDelay = fixedDelay, clock = clock, task = task)
      )
    }
  }
}
