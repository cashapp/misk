...
open class FakeClock(epochMillis: Long = initialValue.toEpochMilli(), private val zone: ZoneId = ZoneId.of("UTC")) :
  Clock(), TestFixture {

  constructor(epochMillis: Long) : this(epochMillis, ZoneId.of("UTC"))

  // Explicit overloads constructors are used so that the 0 parameter constructor can be @Inject annotated for easy
  // injection
  ...
  /**
   * Advance the clock by the specified [Period]. Note that unlike adding a [Duration] the exact amount that is added to
   * the clock will depend on its current time and timezone. Not all days, months or years have the same length. See the
   * documentation for [Period].
   */
  fun add(p: Period) =
    millis.updateAndGet { millis -> Instant.ofEpochMilli(millis).atZone(zone).plus(p).toInstant().toEpochMilli() }
