...
internal class RendezvousClusterResourceMapperTest {
  @Test fun `throws an error when there are zero members`() {
    val hashRing =
      RendezvousClusterResourceMapper(members = setOf())
    assertThrows<NoMembersAvailableException> {
      hashRing["foo"]
    }
  }

  @Test fun `can hash with a single member`() {
    val zork = Cluster.Member("zork", "192.49.168.23")
    val hashRing =
      RendezvousClusterResourceMapper(members = setOf(zork))
    assertThat(listOf("foo", "bar", "zed").map { hashRing[it] }).containsExactly(zork, zork, zork)
  }

  @Test fun `hash is consistent when nodes are removed`() {
  ...
  @Test fun `evaluations are cached`() {
    val zork = Cluster.Member("zork", "192.49.168.23")
    val mork = Cluster.Member("mork", "192.49.168.24")
    val quark = Cluster.Member("quark", "192.49.168.25")
  ...
  @Test fun `hashes are uniformly distributed`() {
  ...
  @Test fun `assignments are uniformly distributed`() {
    val leaseCount = 1_024
    val memberCount = 128
