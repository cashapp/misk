...
internal class HashRingClusterResourceMapperTest {
  ...
  fun singleNode() {
    ...
    val hashRing = HashRingClusterResourceMapper(members = setOf(zork), hashFn = Hashing.murmur3_32_fixed(0))
  ...
  fun multipleNodes() {
    ...
    val hashRing1 =
      HashRingClusterResourceMapper(members = setOf(zork, mork, quark), hashFn = Hashing.murmur3_32_fixed(0))
    assertThat(listOf("foo", "bar", "zed", "abadidea").map { hashRing1[it] }).containsExactly(zork, quark, mork, zork)

    // Remove one of the members, only the resources mapped to that member should change
    val hashRing2 = HashRingClusterResourceMapper(members = setOf(zork, quark), hashFn = Hashing.murmur3_32_fixed(0))
    assertThat(listOf("foo", "bar", "zed", "abadidea").map { hashRing2[it] }).containsExactly(zork, quark, zork, zork)

    // Add a new member, should not remap resources unnecessarily
    val bork = Cluster.Member("bork", "192.49.168.26")
    val hashRing3 =
      HashRingClusterResourceMapper(members = setOf(zork, quark, bork), hashFn = Hashing.murmur3_32_fixed(0))
    assertThat(listOf("foo", "bar", "zed", "abadidea").map { hashRing3[it] }).containsExactly(zork, quark, zork, zork)
  }

  @Test
  fun zeroNodes() {
    val hashRing = HashRingClusterResourceMapper(members = setOf(), hashFn = Hashing.murmur3_32_fixed(0))
    assertThrows<NoMembersAvailableException> { hashRing["foo"] }
  }

  @Test
  fun resourceToRangeMapping() {
    /*
    ...
    */
    ...
    val hashRing =
      HashRingClusterResourceMapper(
        ...
        hashFn =
          FakeHashFn(
            mapOf("a 0" to 100, "b 0" to 200, "c 0" to 300, "foo" to 50, "bar" to 150, "zed" to 250, "zork" to 350)
          ),
        vnodesCount = 1,
      )
    assertThat(listOf("foo", "bar", "zed", "zork").map { hashRing[it] }).containsExactly(a, b, c, a)
  }

  /**
   * Hash function that uses pre-determined mapping to determine hashes for inputs.
   *
   * Does not actually delegate to sha256. It just uses delegation to avoid implementing the full interface.
   */
