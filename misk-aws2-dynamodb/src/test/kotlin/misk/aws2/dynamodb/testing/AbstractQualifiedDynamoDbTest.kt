...
abstract class AbstractQualifiedDynamoDbTest {
  ...
  @Test
  fun allModulesIsolated() {
    // Test that the three modules are isolated from each other
    ...
    val retrievedSecondaryMovie = secondaryMovieTable.getItem(
      Key.builder()
        .partitionValue("Secondary Isolation Test")
        .sortValue(LocalDate.of(2025, 2, 1).toString())
        .build()
    )
    assertThat(retrievedSecondaryMovie).isNotNull
    assertThat(retrievedSecondaryMovie.name).isEqualTo("Secondary Isolation Test")
  }
