...
abstract class AbstractQualifiedDynamoDbTest {
  ...
  fun unqualifiedModule() {
    val enhancedClient = DynamoDbEnhancedClient.builder().dynamoDbClient(unqualifiedClient).build()
    ...
    val actualMovie =
      movieTable.getItem(
        Key.builder().partitionValue("Unqualified Movie").sortValue(LocalDate.of(2024, 1, 1).toString()).build()
      )
  ...
  fun primaryQualifiedModule() {
    val enhancedClient = DynamoDbEnhancedClient.builder().dynamoDbClient(primaryClient).build()
    ...
    val actualCharacter =
      characterTable.getItem(Key.builder().partitionValue("Primary Movie").sortValue("Primary Character").build())
  ...
  fun secondaryQualifiedModule() {
    val enhancedClient = DynamoDbEnhancedClient.builder().dynamoDbClient(secondaryClient).build()
    val movieTable = enhancedClient.table("movies", MOVIE_TABLE_SCHEMA)
    ...
    val actualMovie =
      movieTable.getItem(
        Key.builder().partitionValue("Secondary Movie").sortValue(LocalDate.of(2025, 1, 1).toString()).build()
      )
    assertThat(actualMovie.name).isEqualTo(movie.name)
    assertThat(actualMovie.release_date).isEqualTo(movie.release_date)
  }
  ...
  fun allQualifiedClientsWork() {
    ...
    val unqualifiedEnhanced = DynamoDbEnhancedClient.builder().dynamoDbClient(unqualifiedClient).build()
    val primaryEnhanced = DynamoDbEnhancedClient.builder().dynamoDbClient(primaryClient).build()
    val secondaryEnhanced = DynamoDbEnhancedClient.builder().dynamoDbClient(secondaryClient).build()
    ...
    val retrievedUnqualifiedMovie =
      unqualifiedMovieTable.getItem(
        Key.builder().partitionValue("Unqualified Shared Test").sortValue(LocalDate.of(2024, 2, 1).toString()).build()
      )
    ...
    val retrievedPrimaryCharacter =
      primaryCharacterTable.getItem(
        Key.builder().partitionValue("Primary Shared Test").sortValue("Primary Character").build()
      )
    ...
    val retrievedSecondaryMovie =
      secondaryMovieTable.getItem(
        Key.builder().partitionValue("Secondary Shared Test").sortValue(LocalDate.of(2025, 2, 1).toString()).build()
      )
