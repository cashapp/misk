...
abstract class AbstractDynamoDbTest {
  ...
  fun happyPath() {
    ...
    val actualMovie =
      movieTable.getItem(
        Key.builder().partitionValue("Jurassic Park").sortValue(LocalDate.of(1993, 6, 9).toString()).build()
      )
    assertThat(actualMovie.name).isEqualTo(movie.name)
    assertThat(actualMovie.release_date).isEqualTo(movie.release_date)

    val actualCharacter =
      characterTable.getItem(Key.builder().partitionValue("Jurassic Park").sortValue("Ian Malcolm").build())
      ...
      QueryEnhancedRequest.builder()
        .queryConditional(
          QueryConditional.sortGreaterThanOrEqualTo {
            it.partitionValue("Steven Spielberg").sortValue(LocalDate.of(2010, 1, 1).toString())
          }
        )
