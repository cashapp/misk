    ...
    override fun configure() {
      ...
      install(
        HttpClientsConfigModule(
          HttpClientsConfig(endpoints = mapOf("slack" to HttpClientEndpointConfig("https://hooks.slack.com/")))
        )
      )
      install(HttpClientModule("slack", Names.named("slack")))
    }
  }

  companion object {
    val samplePostMessageJson =
      PostMessageRequest(
        channel = "#default-channel",
        blocks = listOf(Block(type = "section", text = Text(type = "mrkdwn", text = "this is a test"))),
      )
      ...
      UserData(
        ...
        enterprise_user =
          EnterpriseUser(
            ...
            is_admin = false,
            is_owner = false,
            teams = arrayOf("T1234567890"),
          ),
      )

    val sampleTopicRequest =
      SetConversationTopicRequest(channel = "#default-channel", topic = "This is a sample topic for testing")

    val sampleTopicResponse =
      SetConversationTopicResponse(
        ...
        channel =
          ConversationTopic(
            ...
            topic =
              TopicPurpose(value = "Apply topically for best effects", creator = "U12345678", last_set = 1649952691),
            purpose = TopicPurpose(value = "", creator = "", last_set = 0),
            previous_names = emptyList(),
          ),
      )

    val sampleInviteRequest = InviteRequest(channel = "#default-channel", users = "U1234567890,U9876543210")

    val sampleInviteResponse =
      InviteResponse(
        ok = true,
        channel =
          Channel(
    ...
    val sampleUserGroupRequest = UserGroupRequest(usergroup = "U7654321098", users = "U1234567890,U9876543210")

    val sampleChatPermalinkResponse =
      GetChatPermalinkResponse(
        ok = true,
        channel = "C012AB3CD",
        permalink = "https://test.slack.com/archives/C1H9RMHGA/p135854659800008",
      )

    val sampleUserGroupResponse =
      UserGroupResponse(
        ok = true,
        usergroup =
          UserGroup(
            ...
            deleted_by = null,
            prefs = Prefs(channels = emptyList(), groups = emptyList()),
            users = listOf("U060R4BJ4", "U060RNRCZ"),
            user_count = 1,
          ),
      )

    val sampleReactionRequest =
      AddReactionRequest(name = "checkmark", channel = "C012AB3CD", timestamp = "1405894322.002768")

    val sampleReactionResponse = AddReactionResponse(ok = true)
  }
}
