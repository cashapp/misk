...
@Singleton
class SlackSignedRequestsInterceptor @Inject constructor(private val clock: Clock, slackConfig: SlackConfig) :
  NetworkInterceptor {
  // Trim to remove errant newlines or whitespace that could be in the secret text.
  private val signingSecret =
    try {
  ...
  /**
   * The SlackSignedRequestsInterceptor verifies that the incoming request is authorized by using the signing secret
   * provided from Slack and cross-checking it against slack's incoming header with a signature
  ...
  @Singleton
  class Factory @Inject constructor(private val slackSignedRequestsInterceptor: SlackSignedRequestsInterceptor) :
    NetworkInterceptor.Factory {
    override fun create(action: Action): NetworkInterceptor? {
      return when {
        action.function.findAnnotation<SlackSignedRequestsOnly>() != null -> slackSignedRequestsInterceptor
