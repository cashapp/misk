...
data class Team(val id: String, val domain: String, val enterprise_id: String, val enterprise_name: String)
...
@JvmOverloads
constructor(val response_type: String? = "in_channel", val text: String)
...
constructor(
  ...
  /**
   * This user's ID - some Grid users have a kind of dual identity — a local, workspace-centric user ID as well as a
   * Grid-wise user ID, called the Enterprise user ID. In most cases these IDs can be used interchangeably, but when it
   * is provided, we strongly recommend using this Enterprise user id over the root level user id field.
   */
  val id: String? = null,
...
constructor(
  ...
  /**
   * A valid email address. It cannot have spaces, or be in use by another member of the same team. It must have an @
   * and a domain. Changing a user's email address will send an email to both the old and new addresses, and also post a
   * slackbot message to the user informing them of the change. You cannot update your own email using this method. This
   * field can only be changed by admins for users on paid teams.
   */
  val email: String?,
  /**
   * The user's first name. The name "slackbot" cannot be used. Updating first_name will update the first name within
   * real_name.
   */
  val first_name: String?,
  /**
   * The user's last name. The name "slackbot" cannot be used. Updating last_name will update the second name within
   * real_name.
   */
  ...
  val pronouns: String?,
  /**
   * The user's first and last name. Updating this field will update first_name and last_name. If only one name is
   * provided, the value of last_name will be cleared.
   */
...
constructor(
  ...
  /**
   * The display name the user has chosen to identify themselves by in their workspace profile. Do not use this field as
   * a unique identifier for a user, as it may change at any time. Instead, use id and team_id in concert.
   */
  val display_name: String? = null,
  /** The display_name field, but with any non-Latin characters filtered out. */
  val display_name_normalized: String? = null,
  /**
   * A valid email address for the user. It cannot have spaces and must have an @ and a domain. It cannot be in use by
   * another member of the same team. Changing a user's email address will send an email to both the old and new
   * addresses and post a slackbot to the user informing them of the change. This field can only be changed by admins
   * for users on paid teams.
   */
  ...
  /**
   * The Unix timestamp of when the status will expire. Providing 0 or omitting this field results in a custom status
...
/**
 * A user object contains information about a Slack workspace user. The composition of user objects can vary greatly
 * depending on the API being used, or the context of each Slack workspace. Data that has not been supplied may not be
 * present at all, may be null, or may contain an empty string.
 *
 * https://api.slack.com/types/user
 */
data class UserData
...
constructor(
  ...
  /**
   * This user has been deactivated when the value of this field is true. Otherwise, the value is false, or the field
  ...
  /**
   * Describes whether two-factor authentication is enabled for this user. Only visible if the user executing the call
   * is an admin.
   */
  val has_2fa: Boolean? = null,
  /**
   * Identifier for this workspace user. It is unique to the workspace containing the user. Use this field together with
   * team_id as a unique key when storing related data or when specifying the user in API requests. We recommend
   * considering the format of the string to be an opaque value, and not to rely on a particular structure.
   */
  val id: String? = null,
  ...
  /**
   * Indicates whether the user is actually a bot user. Bleep bloop. Note that Slackbot is special, so is_bot will be
   * false for it.
   */
  ...
  /**
   * Only present (and always true) when a user has been invited but has not yet signed in. Once the user signs in, this
  ...
  /**
   * Indicates whether the user is a guest user. Use in combination with the is_ultra_restricted field to check if the
   * user is a single-channel guest user.
   */
  val is_restricted: Boolean? = null,
  /**
   * If true, this user belongs to a different workspace than the one associated with your app's token, and isn't in any
   * shared channels visible to your app. If false (or this field is not present), the user is either from the same
   * workspace as associated with your app's token, or they are from a different workspace, but are in a shared channel
   * that your app has access to. Read our shared channels docs for more detail.
   */
  val is_stranger: Boolean? = null,
  /** Indicates whether the user is a single-channel guest. */
  val is_ultra_restricted: Boolean? = null,
  /**
   * Contains an IETF language code that represents this user's chosen display language for Slack clients. Useful for
   * localizing your apps.
   */
  val locale: String? = null,
  /**
   * Don't use this. It once indicated the preferred username for a user, but that behavior has fundamentally changed
   * since.
   */
  val name: String? = null,
  /**
   * The profile object contains the default fields of a user's workspace profile. A user's custom profile fields may be
   * discovered using users.profile.get.
   */
  ...
  /**
   * Indicates the type of two-factor authentication in use. Only present if has_2fa is true. The value will be either
   * app or sms.
   */
  ...
  /**
   * Indicates the number of seconds to offset UTC time by for this user's tz. Changes silently if changed due to
   * daylight savings.
   */
...
@JvmOverloads
constructor(val ok: Boolean, val error: String? = null, val user: UserData? = null)
 ...
 */
data class GetChatPermalinkResponse
@JvmOverloads
constructor(val ok: Boolean, val channel: String? = null, val error: String? = null, val permalink: String? = null)
...
@JvmOverloads
constructor(val ok: Boolean, val error: String? = null, val usergroup: UserGroup? = null)
...
@JvmOverloads
constructor(val ok: Boolean, val error: String? = null, val channel: Channel? = null)
 ...
 * https://slack.com/api/conversations.setTopic
 */
data class SetConversationTopicRequest
constructor(
 ...
 * https://slack.com/api/conversations.setTopic
 */
data class SetConversationTopicResponse
@JvmOverloads
constructor(val ok: Boolean, val error: String? = null, val channel: ConversationTopic? = null)
...
data class Prefs constructor(val channels: List<String>, val groups: List<String>)
...
data class TopicPurpose constructor(val value: String, val creator: String, val last_set: Long)

data class AddReactionRequest constructor(val name: String, val channel: String, val timestamp: String)

data class AddReactionResponse @JvmOverloads constructor(val ok: Boolean, val error: String? = null)
