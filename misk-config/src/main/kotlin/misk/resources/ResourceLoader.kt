...
class ResourceLoader @Inject constructor(val backends: Map<String, Backend>) {
  ...
  init {
    for (prefix in this@ResourceLoader.backends.keys) {
  ...
  fun open(address: String): BufferedSource? {
    ...
    val backend = this@ResourceLoader.backends[scheme] ?: return null
  ...
  fun put(address: String, data: ByteString) {
    ...
    val (scheme, path) = parseAddress(address)
    val backend = this@ResourceLoader.backends[scheme] ?: return
    backend.checkPath(path)
  ...
  fun exists(address: String): Boolean {
    ...
    val (scheme, path) = parseAddress(address)
    val backend = this@ResourceLoader.backends[scheme] ?: return false
    try {
      backend.checkPath(path)
    } catch (e: IllegalArgumentException) {
      return false
  ...
  fun list(address: String): List<String> {
    ...
    val (scheme, path) = parseAddress(address)
    val backend = this@ResourceLoader.backends[scheme] ?: return listOf()
    try {
      backend.checkPath(path)
    } catch (e: IllegalArgumentException) {
      return listOf()
...
internal class BackendAdaptor(val delegate: ResourceLoader.Backend) : wisp.resources.ResourceLoader.Backend() {
  ...
  override fun watch(path: String, resourceChangedListener: (String) -> Unit) {
    delegate.watch(path, resourceChangedListener)
  }

  override fun unwatch(path: String) {
    delegate.unwatch(path)
  }
