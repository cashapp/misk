...
constructor(
  ...
  /**
   * Pretty Print representation of the metadata used in the admin dashboard. Most metadata should create a Moshi JSON
   * adapter and use [toFormattedJson] to do this.
   */
  val prettyPrint: String =
    metadata
      .toString()
      // Improves readability of default data class toString() for admin dashboard if JSON or custom prettyPrint isn't
      // provided
      .split("),")
      .joinToString("),\n")
      .split(",")
  ...
  open fun descriptionBlock(tagConsumer: TagConsumer<*>) =
    tagConsumer.apply { AlertInfoHighlight(message = descriptionString) }

  /**
   * HTML block for the content. Can be overridden to show more complex UI than the default prettyPrint console output.
   */
  open fun contentBlock(tagConsumer: TagConsumer<*>) =
    tagConsumer.apply {
...
fun <T> JsonAdapter<T>.toFormattedJson(value: T): String {
  val json = serializeNulls().indent("  ").toJson(value)
