...
class CachedConfigurationFactoryTest {
  @Suppress("unused") @MiskTestModule private val module = ClientJooqTestingModule()

  @Inject private lateinit var clock: Clock

  @Inject @JooqDBReadOnlyIdentifier private lateinit var readerConfigurationFactory: ConfigurationFactory

  @Inject @JooqDBReadOnlyIdentifier private lateinit var readerDataSourceService: DataSourceService

  @Inject @JooqDBIdentifier private lateinit var writerConfigurationFactory: ConfigurationFactory

  @Inject @JooqDBIdentifier private lateinit var writerDataSourceService: DataSourceService
  ...
  fun `configuration factory should yield an identical configuration to old method`() {
    ...
    val legacyWriterConfig =
      buildOldConfiguration(
        clock,
  ...
  private fun buildOldConfiguration(
    ...
    return DSL.using(connectionProvider, SQLDialect.MYSQL, settings)
      .apply {
        configuration()
          .set(DefaultTransactionProvider(configuration().connectionProvider(), false))
          .apply {
