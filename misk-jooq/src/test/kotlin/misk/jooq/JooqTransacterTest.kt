...
internal class JooqTransacterTest {
  ...
  @Test
  fun `retries and succeeds in case of an optimistic lock exception`() {
    val movie =
    ...
    val updatedMovie = transacter.transaction { (ctx) -> ctx.selectFrom(MOVIE).where(MOVIE.ID.eq(movie.id)).fetchOne() }
  ...
  @Test
  fun `does not retry in case of no exception`() {
    var numberOfRetries = 0
    transacter.transaction { numberOfRetries++ }
    assertThat(numberOfRetries).isEqualTo(1)
  }

  @Test
  fun `does not retry in a noRetriesTransaction in case of any exception`() {
    var numberOfRetries = 0
    assertThatExceptionOfType(IllegalStateException::class.java).isThrownBy {
  ...
  @Test
  fun `should rollback in case there is an exception thrown`() {
    ...
    val numberOfRecords =
      transacter.transaction(noRetriesOptions) { (ctx) -> ctx.selectCount().from(MOVIE).fetchOne()!!.component1() }
    assertThat(numberOfRecords).isEqualTo(0)
  }

  /**
   * The way the code works is that, every time we call transacter.transaction { } it gets a new connection from the
   * connection pool. So even though it looks like a nested transaction, it's not. The inner transaction works on a new
   * connection and will commit and rollback based on what happens in its boundary. This test is to prove that this
   * works.
   */
  @Test
  fun `nested transactions work as they are not really nested transactions`() {
    transacter.transaction(noRetriesOptions) {
      val movie =
        transacter.transaction(noRetriesOptions) { (ctx) ->
          ctx
      ...
      val updatedMovie =
        transacter.transaction { (ctx) -> ctx.selectFrom(MOVIE).where(MOVIE.ID.eq(movie.id)).fetchOne() }
  ...
  @Test
  fun `pre commit hooks execute`() {
    var preCommitHook1Executed = false
    var preCommitHook2Executed = false
    transacter.transaction { session ->
      session.onPreCommit { preCommitHook1Executed = true }
      session.onPreCommit { preCommitHook2Executed = true }
    }
  ...
  @Test
  fun `an exception during a pre commit hook rolls back the transaction`() {
  ...
  fun `post commit hooks execute`() {
    ...
    transacter.transaction { session ->
      session.onPostCommit { postCommitHook1Executed = true }
      session.onPostCommit { postCommitHook2Executed = true }
    }
  ...
  @Test
  fun `an exception in a post commit does not rollback the transaction`() {
  ...
  @Test
  fun `session close hooks always execute regardless of exceptions thrown from anywhere`() {
    var sessionCloseHook1Called = false
    assertThatExceptionOfType(PostCommitHookFailedException::class.java).isThrownBy {
      transacter.transaction { session ->
        session.onPostCommit { throw RuntimeException() }
        session.onSessionClose { sessionCloseHook1Called = true }
      }
    }
    ...
    assertThatExceptionOfType(DataAccessException::class.java).isThrownBy {
      transacter.transaction(noRetriesOptions) { session ->
        session.onSessionClose { sessionCloseHook2Called = true }
  ...
  @Test
  fun `make sure read and write transacters connect to different dbs`() {
    transacter.transaction(noRetriesOptions) {
      transacter.transaction(noRetriesOptions) { (ctx) ->
        ctx
          .newRecord(MOVIE)
    ...
    /**
     * Since no migrations are run on the reader database, this will throw an exception saying Table
     * 'misk_jooq_testing_reader.movie' doesn't exist
     */
    assertThatExceptionOfType(DataAccessException::class.java).isThrownBy {
      readTransacter.transaction { (ctx) -> ctx.selectCount().from(MOVIE).fetch { it.component1() } }
    }
  }

  @Test
  fun `delete without where throws an error`() {
    transacter.transaction { (ctx) ->
      ctx
        .newRecord(MOVIE)
        .apply {
  ...
  inner class IsolationLevelTests {
    ...
    inner class CheckIsolationLevel {
      @Test
      fun `check isolation level is set to repeatable read by default`() {
        transacter.transaction { (ctx) ->
          ctx.connection {
      ...
      @Test
      fun `check isolation level is set to read committed when explicitly set`() {
        transacter.transaction(
    ...
    inner class EnsureIsolationLevelsWorkCorrectly {
      ...
      @Test
      fun `should see the same record as previously read - repeatable read`() {
        transacter.transaction(
          options =
            TransacterOptions(
          ...
          transacter.transaction { (ctx) ->
            ctx
              .selectFrom(MOVIE)
              .where(MOVIE.ID.eq(savedMovieRecord.id))
              .fetchOne()
              ?.apply { this.genre = Genre.HORROR.name }
              ?.also { it.store() }
          }
      ...
      @Test
      fun `should see the updated record - read committed`() {
        transacter.transaction(
          options = TransacterOptions(isolationLevel = TransactionIsolationLevel.READ_COMMITTED)
        ) { (ctx) ->
          var movie = ctx.selectFrom(MOVIE).where(MOVIE.ID.eq(savedMovieRecord.id)).fetchOne()!!
      ...
      @Test
      fun `should see the updated record - read uncommitted`() {
        transacter.transaction(
          options = TransacterOptions(isolationLevel = TransactionIsolationLevel.READ_UNCOMMITTED)
        ) { (ctx) ->
          var movie = ctx.selectFrom(MOVIE).where(MOVIE.ID.eq(savedMovieRecord.id)).fetchOne()!!
          assertThat(movie.genre).isEqualTo(Genre.COMEDY.name)
          val context1 = ctx
