...
class RecordSignatureListenerTest {
  ...
  @Inject @JooqDBIdentifier private lateinit var dataSourceService: DataSourceService

  @BeforeEach
  fun cleanDatabase() {
    transacter.transaction { session -> session.ctx.deleteFrom(RecordSignatureTest.RECORD_SIGNATURE_TEST) }
  }
  ...
  fun `signature column not null on insert`() {
    transacter.transaction { (ctx) ->
      ctx
        .newRecord(RECORD_SIGNATURE_TEST)
        .apply {
          this.name = "test"
          this.createdBy = "foo"
    ...
    transacter.transaction { (ctx) ->
      ctx.selectFrom(RECORD_SIGNATURE_TEST).fetchOne()!!.also { assertNotNull(it.recordSignature) }
    }
  }
  ...
  fun `signature updated when record is updated`() {
    ...
    transacter.transaction { (ctx) ->
      val record =
        ctx.selectFrom(RECORD_SIGNATURE_TEST).where(RECORD_SIGNATURE_TEST.ID.eq(originalRecord.id)).fetchOne()!!
    ...
    transacter.transaction { (ctx) ->
      ctx.selectFrom(RECORD_SIGNATURE_TEST).where(RECORD_SIGNATURE_TEST.ID.eq(originalRecord.id)).fetchOne()!!.also {
  ...
  fun `throws error when retrieving rows updated directly in database`() {
    ...
    assertThrows<DataIntegrityException> {
      transacter.transaction { (ctx) ->
        ...
        val result = ctx.selectFrom(RECORD_SIGNATURE_TEST).where(RECORD_SIGNATURE_TEST.ID.eq(record.id)).fetchOne()
  ...
  fun `different records with potentially colliding values have different signatures`() {
    ...
    transacter.transaction { (ctx) ->
      val retrieved1 = ctx.selectFrom(RECORD_SIGNATURE_TEST).where(RECORD_SIGNATURE_TEST.ID.eq(record1.id)).fetchOne()!!
      assertNotNull(retrieved1.recordSignature)

      val retrieved2 = ctx.selectFrom(RECORD_SIGNATURE_TEST).where(RECORD_SIGNATURE_TEST.ID.eq(record2.id)).fetchOne()!!
  ...
  fun `throws error when signature is updated to null`() {
    ...
    assertThrows<DataIntegrityException> {
      transacter.transaction { (ctx) ->
        ctx.selectFrom(RECORD_SIGNATURE_TEST).where(RECORD_SIGNATURE_TEST.ID.eq(record.id)).fetchOne()!!
      }
    }
  }
}
