...
class JooqTimestampRecordListenerTest {
  ...
  private val anHourBefore: LocalDateTime by lazy { clock.instant().minus(1, ChronoUnit.HOURS).toLocalDateTime() }

  private val aMinuteBefore: LocalDateTime by lazy { clock.instant().minus(1, ChronoUnit.MINUTES).toLocalDateTime() }

  @Test
  fun `should set the created and updated times if not already set`() {
    val movie =
    ...
    val fetchedMovie =
      transacter.transaction { (ctx) -> ctx.selectFrom(MOVIE).where(MOVIE.NAME.eq("Enter the dragon")).fetchOne()!! }
  ...
  @Test
  fun `should not set the created and updated times if already set`() {
    val movie =
      transacter.transaction { (ctx) ->
        ctx
          .newRecord(MOVIE)
  ...
  @Test
  fun `updates the updated at column during an update`() {
    transacter.transaction { (ctx) ->
      val record =
        ctx
          .newRecord(MOVIE)
  ...
  @Test
  fun `updated at will not be set if it has been changed before store is called`() {
    transacter.transaction { (ctx) ->
      val record =
        ctx
          .newRecord(MOVIE)
