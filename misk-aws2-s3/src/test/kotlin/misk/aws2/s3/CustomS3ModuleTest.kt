...
/**
 * Custom S3 module that demonstrates client configuration via lambda functions.
 * This shows how to configure LocalStack endpoint and path style via lambdas.
 */
...
class CustomS3ModuleTest {
  ...
  fun `lambda configuration is applied correctly`() {
    ...
    val syncResponse = s3Client.getObject(
      GetObjectRequest.builder()
        .bucket(bucketName)
        .key(key)
        .build()
    ...
    val asyncResponse = s3AsyncClient.getObject(
      GetObjectRequest.builder()
        .bucket(bucketName)
        .key(asyncKey)
        .build(),
