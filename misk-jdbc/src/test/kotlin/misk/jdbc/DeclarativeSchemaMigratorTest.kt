...
internal class DeclarativeSchemaMigratorTest {
  ...
  @MiskTestModule val deploymentModule = DeploymentModule(TESTING)

  @MiskTestModule val testingModule = MiskTestingServiceModule()

  @MiskTestModule val jdbcModule = JdbcModule(Movies::class, config)

  data class RootConfig(val mysql_data_source: DataSourceConfig) : Config
  ...
  fun failRequireAllForMissingColumn() {
    ...
    assertFailsWith<IllegalStateException>(
      message = "Error: Column \"ip_address\" for table table_1 is missing in the database."
    ) {
      declarativeSchemaMigrator.requireAll()
    }
  }
