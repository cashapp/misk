...
class MySQLEnforceWritableConnectionsTest {
  ...
  inner class ValidatingTransacterTest {
    @Test fun `marks connections invalid if the DB instance becomes read only`() {
  ...
  inner class TransacterTest {
    @Test fun `it does not mark connections invalid if the DB instance becomes read only`() {
      transacter.transactionWithSession { (connection) ->
        setGlobalReadOnly(connection, 1)
        assertThat(connection.isValid(VALID_CHECK_TIMEOUT_SECONDS)).isTrue()
      }
  ...
  private fun setGlobalReadOnly(connection: Connection, state: Int) {
    connection.createStatement().executeUpdate(
      "SET GLOBAL read_only = $state"
    )
    connection.createStatement().executeUpdate(
      "SET GLOBAL super_read_only = $state"
    )
  }
    ...
    override fun configure() {
      install(
        Modules.override(MiskTestingServiceModule()).with(
          FakeClockModule(), MockTracingBackendModule()
        )
      )
      ...
      install(
        JdbcModule(
          Movies2::class, config.mysql_data_source
        )
      )
    }
  }
