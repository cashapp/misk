...
object ScaleSafetyChecks {
  ...
  fun getLastLoggedCommand(connection: Connection): Timestamp? {
    return connection.createStatement().use { session ->
      session.executeQuery("SELECT MAX(event_time) FROM mysql.general_log").map { it.getTimestamp(1) }.singleOrNull()
    }
  }
    ...
    val explanations =
      connection.createStatement().use { statement ->
        try {
          ...
          if (message != null) null else throw e
        }
      }
  ...
  fun isDml(query: String): Boolean {
    val first = query.replace(COMMENT_PATTERN, "").trimStart().lowercase().takeWhile { !it.isWhitespace() }
