 ...
 *
 * This deletes the data in the tables but leaves the schema as-is. It also leaves the `schema_version` table as is.
 *
 * We truncate _before_ tests because that way we always have a clean slate, even if a preceding test wasn't able to
  ...
  private fun executeStatements(statements: List<String>, name: String) {
    ...
    transacterProvider.get().transaction { connection ->
      CheckDisabler.withoutChecks {
        for (s in statements) {
          connection.createStatement().use { statement -> statement.execute(s) }
        }
      }
    }
    ...
    if (statements.isNotEmpty()) {
      logger.info {
        "@${qualifier.simpleName} TruncateTablesService ran ${statements.size} $name " + "statements in $stopwatch"
      }
    }
  }
}
