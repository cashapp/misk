...
internal class ListAdapter(private val elementAdapter: JsonAdapter<Any?>) : JsonAdapter<List<Any?>>() {
  override fun fromJson(reader: JsonReader): List<Any?>? {
    val nextToken = reader.peek()
    if (nextToken == JsonReader.Token.NULL) return reader.nextNull()
    check(nextToken == JsonReader.Token.BEGIN_ARRAY) { "expected [; found ${reader.peek()}" }
