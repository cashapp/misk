...
class VitessDatabasePlugin : Plugin<Project> {
  override fun apply(project: Project) {
    ...
    val inputGetters = StartVitessDatabaseTask::class.java.methods.filter { it.isAnnotationPresent(Input::class.java) }

    project.tasks.withType(Test::class.java).configureEach { test ->
      for (getter in inputGetters) {
        val name = getter.name.removePrefix("get").replaceFirstChar { it.lowercase() }
        @Suppress("UNCHECKED_CAST")
        test.inputs.property("vitess.$name", startVitessDatabase.flatMap { getter.invoke(it) as Provider<Any> })
      }
    }
  }
}
